...
date: 2026-03-25
type: book-summary
tags: [book-summary, wealth, happiness, philosophy, startups, leverage, wisdom, Naval-Ravikant]
...
```mermaid
flowchart TD
    A["SPECIFIC KNOWLEDGE<br>(Can't be trained for —<br>found by pursuing<br>genuine curiosity)"] --> D["WEALTH<br>(Assets that earn<br>while you sleep)"]
    B["ACCOUNTABILITY<br>(Take business risks<br>under your own name)"] --> D
    C["LEVERAGE<br>(Code, Media, Capital,<br>Labour — multiply<br>your output)"] --> D
    D --> E["FREEDOM<br>(Control over<br>your time)"]
    E --> F["HAPPINESS<br>(A trainable skill,<br>not a destination)"]
    style A fill:#2980b9,color:#fff
    style D fill:#27ae60,color:#fff
    style F fill:#f9d77e,stroke:#d4a017
```
...
```mermaid
flowchart LR
    subgraph old ["OLD LEVERAGE (Permission Required)"]
        Labour["Labour<br>(People working for you)"]
        Capital["Capital<br>(Money working for you)"]
    end
    subgraph new ["NEW LEVERAGE (Permissionless)"]
        Code["Code<br>(Software working for you)"]
        Media["Media<br>(Content working for you)"]
    end
    Labour --> Output1["Linear scaling<br>More people = more output<br>But more management too"]
    Capital --> Output2["Moderate scaling<br>More money = more returns<br>But requires trust first"]
    Code --> Output3["Infinite scaling<br>One product serves millions<br>Marginal cost = zero"]
    Media --> Output4["Infinite scaling<br>One piece reaches millions<br>Marginal cost = zero"]
    style new fill:#27ae60,color:#fff
    style old fill:#f9d77e,stroke:#d4a017
```
...
```mermaid
flowchart TD
    A["Take accountability<br>(Year 1)"] --> B["Deliver with integrity"]
    B --> C["Reputation grows"]
    C --> D["Better opportunities arrive"]
    D --> E["Take accountability<br>(Year 5)"]
    E --> B
    C --> F["Trust compounds<br>over decades"]
    F --> G["Wealth follows<br>reputation"]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart LR
    A["I want X"] --> B["I am unhappy<br>(until I get X)"]
    B --> C["I get X"]
    C --> D["Brief satisfaction<br>(hedonic peak)"]
    D --> E["Adaptation<br>(new baseline)"]
    E --> F["I want Y<br>(new desire)"]
    F --> B
...
```mermaid
flowchart LR
    A["Wealth<br>(Financial assets)"] --> B["Freedom<br>(Control over time)"]
    B --> C["Space to pursue<br>happiness directly"]
    C --> D["Happiness<br>(A trainable skill)"]
    A -->|"Common mistake:<br>skipping freedom"| D
...
```mermaid
flowchart TD
    A["Traditional Retirement"] --> B["Work 40 years<br>at a job you tolerate"]
    B --> C["Save enough to<br>stop working"]
    C --> D["Stop working<br>at age 65"]
    D --> E["Often bored<br>and purposeless"]
    F["Naval's Retirement"] --> G["Find work you'd<br>do for free"]
    G --> H["Apply leverage<br>to make it pay"]
    H --> I["'Retire' at any age<br>(aligned work)"]
    I --> J["Energised<br>and purposeful"]
