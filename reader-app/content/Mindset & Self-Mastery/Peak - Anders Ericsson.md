...
date: 2026-03-25
type: book-summary
tags: [book-summary, deliberate-practice, expertise, performance, psychology, learning, neuroscience]
...
```mermaid
flowchart TD
    A["Sustained, focused<br>demand on the brain"] --> B["Brain encounters<br>current limits"]
    B --> C["Homeostasis disrupted:<br>brain must adapt or fail"]
    C --> D["New neural pathways<br>formed or strengthened"]
    D --> E["Mental representations<br>become richer"]
    E --> F["Performance improves"]
    F --> G["New, higher limits<br>become the baseline"]
    G --> A
...
```mermaid
flowchart TD
    A["Deliberate Practice<br>targeted, effortful,<br>with feedback"] --> B["Mental Representations<br>improve"]
    B --> C["Better performance:<br>see more, anticipate more,<br>detect errors faster"]
    C --> D["Can now practise at<br>a higher level"]
    D --> A
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    NP["Naive Practice<br>Just doing the activity"] --> P["OK Plateau<br>Performance stalls"]
    PP["Purposeful Practice<br>Goals + feedback + effort"] --> LI["Limited Improvement<br>Hits ceiling without guidance"]
    DP["Deliberate Practice<br>Expert-designed + targeted<br>+ at the edge of ability"] --> EP["Expert Performance<br>Continuous improvement"]
    P -.->|"Most people<br>stop here"| P
    LI -.->|"Better, but<br>still limited"| LI
    EP -->|"Cycle continues:<br>higher level practice"| DP
...
```mermaid
flowchart LR
    S1["Stage 1:<br>Starting Out"] --> S2["Stage 2:<br>Purposeful Phase"]
    S2 --> S3["Stage 3:<br>Deliberate Phase"]
    S3 --> S4["Stage 4:<br>Expert Performance"]
    S1 -.-> |"weeks to months"| S1
    S2 -.-> |"months to years"| S2
    S3 -.-> |"years to decades"| S3
    S4 -.-> |"ongoing"| S4
...
```mermaid
flowchart TD
    subgraph GLADWELL ["Gladwell's Model"]
        G1["Put in 10,000 hours"] --> G2["Become an expert"]
    end
    subgraph ERICSSON ["Ericsson's Model"]
        E1["Find expert coaching"] --> E2["Target specific weaknesses"]
        E2 --> E3["Practise at edge of ability"]
        E3 --> E4["Get immediate feedback"]
        E4 --> E5["Build mental representations"]
        E5 --> E6["Repeat for years"]
        E6 --> E2
    end
    style GLADWELL fill:#e74c3c,color:#fff
    style ERICSSON fill:#27ae60,color:#fff
```
...
```mermaid
flowchart LR
    subgraph HOME ["Homeostasis: No Growth"]
        H1["Current ability<br>matches demand"] --> H2["Brain stays<br>the same"] --> H3["OK Plateau"]
    end
    subgraph DP ["Deliberate Practice: Growth"]
        D1["Demand exceeds<br>current ability"] --> D2["Brain adapts to<br>meet new demand"] --> D3["New, higher<br>baseline"]
    end
    HOME -->|"Challenge<br>homeostasis"| DP
    DP -->|"Raise the bar<br>again"| DP
