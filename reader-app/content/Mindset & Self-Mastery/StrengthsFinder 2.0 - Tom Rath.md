...
date: 2026-03-25
type: book-summary
tags: [book-summary, strengths, talent, self-awareness, personal-development, gallup, psychology, taxonomy, assessment, engagement, teams]
author: Tom Rath
...
flowchart TB
    ...
    T["TALENT<br>(Natural recurring<br>pattern)"] --> S
    I["INVESTMENT<br>(Practice + Knowledge<br>+ Skill)"] --> S
    S --> E["ENGAGEMENT<br>(6x more likely)"]
    S --> Q["QUALITY OF LIFE<br>(3x more likely)"]
    S --> P["PERFORMANCE<br>(Exponential returns)"]
...
```mermaid
flowchart LR
    A[Assess<br>Top 5 Themes] --> B[Understand<br>Core Drives]
    B --> C[Notice<br>Strength Zones]
    C --> D[Invest<br>Deliberately]
    D --> E[Partner<br>for Weaknesses]
    E --> F[Build<br>Complementary Teams]
...
```mermaid
flowchart TB
    EX["EXECUTING DOMAIN<br>(Making things happen)"]
    EX --> ACH[Achiever]
    EX --> ARR[Arranger]
    EX --> BEL[Belief]
    EX --> CON[Consistency]
...
```mermaid
flowchart TB
    IN["INFLUENCING DOMAIN<br>(Moving others to act)"]
    IN --> ACT[Activator]
    IN --> CMD[Command]
    IN --> COM[Communication]
    IN --> CMP[Competition]
    IN --> MAX[Maximiser]
    IN --> SA[Self-Assurance]
    IN --> SIG[Significance]
    IN --> WOO[Woo]
    style IN fill:#2980b9,color:#fff
```
...
```mermaid
flowchart TB
    RB["RELATIONSHIP BUILDING DOMAIN<br>(Holding teams together)"]
    RB --> ADA[Adaptability]
    RB --> CNN[Connectedness]
    RB --> DEV[Developer]
    RB --> EMP[Empathy]
    RB --> HAR[Harmony]
    RB --> INC[Includer]
    RB --> IND[Individualisation]
    RB --> POS[Positivity]
    RB --> REL[Relator]
...
```mermaid
flowchart TB
    ST["STRATEGIC THINKING<br>(Analyse & Absorb)"] --> IN["INFLUENCING<br>(Sell & Persuade)"]
    IN --> EX["EXECUTING<br>(Make It Happen)"]
    EX --> RB["RELATIONSHIP BUILDING<br>(Hold Together)"]
    RB --> ST
...
flowchart LR
    subgraph "Assessment"
    A["177 Paired<br>Statements"] --> B["20 Seconds<br>Per Pair"]
    B --> C["Instinctive<br>Responses"]
    end
    subgraph "Results"
    C --> D["Top 5<br>Signature Themes"]
    D --> E["Development<br>Guide"]
    end
    subgraph "Action"
    E --> F["Invest in<br>Strengths"]
    E --> G["Manage<br>Weaknesses"]
    E --> H["Build<br>Partnerships"]
    end
    style A fill:#2980b9,color:#fff
