...
```mermaid
flowchart TD
    A[Initial Offer] --> B{Negotiate?}
    B -->|Yes| C[Higher Base Salary]
    B -->|No| D[Lower Base Salary]
    C --> E[% Raises on Higher Base]
    D --> F[% Raises on Lower Base]
    E --> G[Compounding Advantage]
    F --> H[Compounding Disadvantage]
...
```mermaid
flowchart LR
    A[Quality Work] --> C{Brand Impact}
    B[Active Marketing] --> C
    C -->|Both Present| D[Strong Brand]
    C -->|Quality Only| E[Invisible Excellence]
    C -->|Marketing Only| F[Credibility Collapse]
    style D fill:#27ae60,color:#fff
...
```mermaid
flowchart TD
    A[Unconscious Habit] --> B{Awareness}
    B -->|Recognition| C[See the Choice]
    C --> D{Deliberate Decision}
    D -->|Strategic| E[Play the Game]
    D -->|Authentic| F[Break the Rule]
    D -->|Contextual| G[Adapt to Situation]
    style A fill:#e74c3c,color:#fff
    style C fill:#2980b9,color:#fff
```
