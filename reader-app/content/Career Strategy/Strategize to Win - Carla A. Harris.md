...
date: 2026-03-25
type: book-summary
tags: [book-summary, career-strategy, positioning, sponsorship, performance-currency, relationship-currency, negotiation, corporate-navigation]
...
```mermaid
flowchart LR
    A[Starting Out] --> B[Stepping Up]
    B --> C[Starting Over]
    A -- "Performance Currency" --- D[Continuous Investment]
    A -- "Relationship Currency" --- D
    D --> B
    D --> C
...
```mermaid
flowchart TD
    A[Performance Currency] --> C{Promotion Decision}
    B[Relationship Currency] --> C
    C -->|Junior Level| D[Performance Dominant]
    C -->|Senior Level| E[Relationships Dominant]
    D --> F[Visible Metrics]
...
```mermaid
flowchart TD
    A[Positive Verbal Feedback] --> B{Match With Actions?}
    B -->|Yes: Stretch assignments,<br>promotions, inclusion| C[Genuine Signal]
    B -->|No: No new responsibilities,<br>no visibility, no advancement| D[Cushion, Not Signal]
    D --> E[Ask Direct Questions]
    E --> F[Track Actions Over Time]
    style C fill:#27ae60,color:#fff
...
```mermaid
flowchart TD
    A[Your Idea] --> B{Does it build on<br>what exists?}
    B -->|Yes| C[Evolutionary]
    B -->|No| D[Revolutionary]
    C --> E[Co-opt the original author]
    D --> F{Is the status quo<br>clearly broken?}
    F -->|Yes| G[Use negative incentive]
    F -->|No| H[Reframe as evolutionary]
    style C fill:#27ae60,color:#fff
    style D fill:#e74c3c,color:#fff
```
...
```mermaid
flowchart TD
    A[Career Dissatisfaction] --> B{Is it the Seat?}
    B -->|Yes| C[Change roles internally]
    B -->|No| D{Is it the House?}
    D -->|Yes| E[Leave the company]
    D -->|No| F{Is it a Person?}
    F -->|Yes| G[Build alternative<br>relationships]
    G --> H{Can you route<br>around them?}
    H -->|Yes| I[Stay and outlast]
    H -->|No| E
