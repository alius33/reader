...
date: 2026-03-25
type: book-summary
tags: [book-summary, promotion, visibility, sponsorship, timing, organisational-politics, self-promotion]
author: Donald Asher
title: "Who Gets Promoted, Who Doesn't, and Why"
year: 2007
---
...
```mermaid
flowchart LR
    A[Perceived Future Value] --> G[Promotion Decision]
    B[Low Replacement Cost] --> G
    C[Low Risk Profile] --> G
    D[Right Timing] --> G
    E[High Visibility] --> G
    F[Strong Sponsorship] --> G
...
```mermaid
flowchart TD
    A[Promotion Candidate] --> B{Benefits?}
    B -->|Strong| C{Costs?}
    B -->|Weak| F[Blocked]
    C -->|Acceptable| D{Risks?}
    C -->|Too High| F
    D -->|Manageable| E[Promoted]
    D -->|Tragic Flaw| F
...
```mermaid
flowchart TD
    A[Preparation] --> C{Opportunity Arrives}
    B[Nimbleness] --> C
    C --> D[Ready to Act]
    D --> E[Career Advance]
    F[Unprepared] --> C
    C --> G[Missed Opportunity]
    style E fill:#27ae60,color:#fff
    style G fill:#e74c3c,color:#fff
```
...
flowchart TB
    subgraph Foundation
        A[Investment Thesis] --> B[Three-Vector Test]
    end
    subgraph Positioning
        C[Visibility & Perception] --> D[Self-Promotion & Sales]
        D --> E[Ascension Plan]
    end
    subgraph Relationships
        F[Make Boss Look Good] --> G[Suction]
        G --> H[Guardian Angels]
    end
    subgraph Execution
        I[Mobility & Timing] --> J[Preparation + Luck]
    end
    B --> C
    E --> F
    H --> I
