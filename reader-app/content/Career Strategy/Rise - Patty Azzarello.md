...
date: 2026-03-25
type: book-summary
tags: [book-summary, career-advancement, leadership, visibility, delegation, priorities, personal-brand]
...
```mermaid
flowchart LR
    A[DO Better] --> D[Career Advancement]
    B[LOOK Better] --> D
    C[CONNECT Better] --> D
    A -- stories for visibility --> B
    B -- intelligence on priorities --> A
    C -- coaching on both --> A
    C -- advocacy --> B
```
...
```mermaid
flowchart TD
    A[You doing the work] --> B{Can you delegate to a person?}
    B -- Yes --> C[Delegate + coach + inspect]
    B -- No --> D[Delegate to a process]
    C --> E[Team builds capability]
    D --> E
    E --> F[You freed for strategic work]
    F --> G[Working ON the business]
```
...
```mermaid
flowchart LR
    A[Fearlessness] --> B[Leap into new role]
    B --> C[Learning on the job]
    C --> D[Experience]
    D --> E[Confidence]
    E -.-> |"Next level"| A
...
```mermaid
flowchart TD
    A[DO Better] -->|better stories| B[LOOK Better]
    B -->|intelligence on priorities| A
    C[CONNECT Better] -->|coaching| A
    C -->|advocacy| B
