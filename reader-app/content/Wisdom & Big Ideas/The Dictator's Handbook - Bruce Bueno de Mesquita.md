...
date: 2026-03-25
type: book-summary
tags: [political-theory, power, governance, corruption, democracy, autocracy, incentives, selectorate-theory, game-theory]
...
```mermaid
flowchart LR
    A[Selectorate<br>All Interchangeables] --> B[Influentials<br>Subset With Real Voice]
    B --> C[Essentials<br>Winning Coalition]
    C --> D[Leader<br>Stays or Falls]
    D -->|Pays Coalition| C
    D -->|Ignores| A
...
```mermaid
flowchart TD
    A[Leader Wants Power] --> B{Coalition Size?}
    B -->|Small| C[Private Rewards]
    B -->|Large| D[Public Goods]
    C --> E[Corruption & Kleptocracy]
    C --> F[Long Tenure]
    D --> G[Freedom & Prosperity]
    D --> H[Short Tenure]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart LR
    A[Incumbent Dies/Weakens] --> B[Coalition Uncertain]
    B --> C[Supporters Seek New Patron]
    C --> D[Challenger Seizes Treasury]
    D --> E[Pays Coalition First]
    E --> F[Purges Rivals]
    F --> G[Consolidates Power]
...
```mermaid
flowchart TD
    A[Government Revenue] --> B{Coalition Type}
    B -->|Small Coalition| C[High Tax Rate]
    B -->|Large Coalition| D[Moderate Tax Rate]
    C --> E[Maximum Extraction]
    C --> F[People Impoverished]
    D --> G[Balanced Extraction]
    D --> H[People Productive]
    E --> I[Leader Enriched]
    H --> J[Economy Grows]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A[Coalition Size] --> B{Large or Small?}
    B -->|Large| C[Good Building Codes]
    B -->|Large| D[Clean Water for All]
    B -->|Large| E[Universal Education]
    B -->|Small| F[Shoddy Construction]
    B -->|Small| G[Clean Water for Elites]
    B -->|Small| H[Primary Only Education]
    C --> I[Few Disaster Deaths]
    F --> J[Mass Casualties]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart LR
    A[Democratic Donor] -->|Cash/Aid| B[Autocratic Recipient]
    B -->|Policy Concession| A
    A -->|Coalition Happy| C[Leader Re-elected]
    B -->|Coalition Paid| D[Leader Survives]
    E[Recipient People] -->|Suffer| F[No Benefit]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A[Regime Weakens] --> B[Revenue Falls]
    B --> C[Coalition Payments Cut]
    C --> D[Military Loyalty Wavers]
    D --> E{Enough Defections?}
    E -->|Yes| F[Regime Falls]
    E -->|No| G[Crackdown Succeeds]
    G --> H[Opposition Crushed]
    F --> I[New Leader]
    I --> J[Shrinks Coalition<br>Cycle Restarts]
    style A fill:#e74c3c,color:#fff
...
```mermaid
flowchart TD
    A[War Begins] --> B{Coalition Size?}
    B -->|Large Coalition| C[Try Hard to Win]
    B -->|Small Coalition| D[Preserve Private Rewards]
    C --> E[High Spend per Soldier]
    C --> F[Surge if Losing]
    D --> G[Low Spend per Soldier]
    D --> H[Quit if Costly]
    E --> I[93% Win Rate When Initiating]
    G --> J[60% Win Rate When Initiating]
    style I fill:#27ae60,color:#fff
    style J fill:#e74c3c,color:#fff
```
...
```mermaid
flowchart TD
    A[Very Small Coalition] -->|Members prefer purges| B[Instability]
    A -->|If member survives purge| C[Greater Private Rewards]
    D[Medium Coalition] -->|Tension zone| E[Reform or Backslide?]
    F[Large Coalition] -->|Members prefer expansion| G[Stable Democracy]
    F -->|No one benefits from purge| H[Self-Reinforcing]
    style A fill:#e74c3c,color:#fff
    style G fill:#27ae60,color:#fff
