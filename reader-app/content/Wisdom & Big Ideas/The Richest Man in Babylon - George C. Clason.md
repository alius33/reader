...
date: 2026-03-25
type: book-summary
tags: [book-summary, finance, personal-finance, wealth, parables, saving, investing, timeless-principles, discipline, debt, compound-interest]
...
```mermaid
flowchart LR
    A["Earn Income"] --> B["Pay Yourself First<br>(save 10%+)"]
    B --> C["Control Expenses<br>(live on the rest)"]
    C --> D["Invest Wisely<br>(multiply savings)"]
    D --> E["Guard Against Loss<br>(protect principal)"]
    E --> F["Wealth<br>Accumulates"]
    F -->|"Compound<br>over time"| D
...
```chartjs
{
  "type": "bar",
  "data": {
    "labels": ["Pay Yourself First", "Control Expenses", "Multiply Gold", "Guard from Loss", "Own Dwelling", "Future Income", "Increase Earning"],
    "datasets": [{
      "label": "Adoption Rate in Modern Life (%)",
      "data": [30, 25, 20, 15, 40, 20, 35],
      "backgroundColor": ["#27ae60", "#2980b9", "#8e44ad", "#e74c3c", "#f39c12", "#3498db", "#2ecc71"],
      "borderColor": ["#1e8449", "#1f6fa3", "#6c3483", "#c0392b", "#d68910", "#2471a3", "#27ae60"],
      "borderWidth": 1
    }]
  },
  "options": {
    "plugins": {
      "title": {
        "display": true,
        "text": "The Seven Cures — Modern Adoption Rate"
      }
    },
    "scales": {
      "y": {
        "beginAtZero": true,
        "max": 50,
        "title": { "display": true, "text": "% of Adults Practicing" }
      }
    }
  }
}
...
```mermaid
flowchart TD
    A["Year 1: Save 10%"] --> B["Invest under<br>competent guidance"]
    B --> C["Returns earned"]
    C --> D{"Reinvest or<br>consume?"}
    D -->|"Reinvest"| E["Gold 'children'<br>earn their own returns"]
    E --> F["Compounding<br>accelerates"]
    F --> G["Wealth builds<br>exponentially"]
    D -->|"Consume"| H["Returns spent<br>on luxuries"]
    H --> I["Back to square one<br>every year"]
...
```chartjs
{
  ...
  "data": {
    ...
    "datasets": [{
      "label": "Wealth with 10% Saving at 7% Return ($K)",
      "data": [0, 7, 17, 32, 54, 85, 132],
      "borderColor": "#27ae60",
  ...
  "options": {
    "plugins": {
      "title": {
        "display": true,
        "text": "The Power of Compounding — Saving 10% at 7% Annual Return"
      }
    },
    "scales": {
      "y": {
        "beginAtZero": true,
        "title": { "display": true, "text": "Accumulated Wealth ($K)" }
      },
      "x": {
        "title": { "display": true, "text": "Years of Consistent Saving" }
      }
    }
  }
}
...
flowchart LR
    subgraph Y1["Year 1"]
        A1["Your savings"] --> B1["Small returns"]
    end
    subgraph Y5["Year 5"]
        A5["Your savings +<br>reinvested returns"] --> B5["Growing returns"]
    end
    subgraph Y20["Year 20"]
        A20["Returns now<br>outpace savings"] --> B20["Returns earning<br>their own returns"]
    end
    Y1 --> Y5 --> Y20
    style Y1 fill:#e8f5e9
    style Y5 fill:#c8e6c9
    style Y20 fill:#27ae60,color:#fff
```
...
```d3
{
  "type": "heatmap",
  "data": {
    "rows": ["Law 1: Gold Comes Gladly", "Law 2: Gold Works Diligently", "Law 3: Gold Clings to the Wise", "Law 4: Gold Slips from the Ignorant", "Law 5: Gold Flees the Greedy"],
    "columns": ["Saving", "Investing", "Diversifying", "Seeking Advice", "Avoiding Scams"],
    "values": [
      [10, 3, 2, 4, 3],
      [5, 10, 7, 6, 4],
      [4, 7, 6, 10, 8],
  ...
  "options": {
    "title": "Five Laws of Gold — Mapping to Modern Financial Behaviours",
    "colorScale": "blues",
    "width": 650,
    "height": 350
  }
...
```mermaid
flowchart TD
    A["Dabasir: drowning in debt"] --> B["Sira's question:<br>'Soul of free man<br>or slave?'"]
    B --> C["Escapes slavery,<br>returns to Babylon"]
    C --> D["Adopts 70/20/10<br>formula"]
    D --> E["Faces creditors<br>with honest plan"]
    E --> F["Consistent payments<br>build trust"]
    F --> G["Debts cleared<br>in two years"]
    G --> H["Savings compound<br>into wealth"]
...
```d3
{
  "type": "sankey",
  "data": {
    "nodes": ["Monthly Income", "Living Expenses (70%)", "Debt Repayment (20%)", "Savings (10%)", "Food & Shelter", "Clothing & Transport", "Creditor A", "Creditor B", "Creditor C", "Emergency Fund", "Investment Capital"],
    "links": [
      { "source": "Monthly Income", "target": "Living Expenses (70%)", "value": 70 },
      { "source": "Monthly Income", "target": "Debt Repayment (20%)", "value": 20 },
      { "source": "Monthly Income", "target": "Savings (10%)", "value": 10 },
      { "source": "Living Expenses (70%)", "target": "Food & Shelter", "value": 50 },
      { "source": "Living Expenses (70%)", "target": "Clothing & Transport", "value": 20 },
      { "source": "Debt Repayment (20%)", "target": "Creditor A", "value": 8 },
      { "source": "Debt Repayment (20%)", "target": "Creditor B", "value": 7 },
      { "source": "Debt Repayment (20%)", "target": "Creditor C", "value": 5 },
      { "source": "Savings (10%)", "target": "Emergency Fund", "value": 4 },
      { "source": "Savings (10%)", "target": "Investment Capital", "value": 6 }
    ]
  },
  "options": {
    "title": "Dabasir's 70/20/10 Income Allocation Formula",
    "width": 700,
    "height": 400
  }
}
...
```mermaid
flowchart TD
    A["Pay Yourself First<br>(Cure 1, Law 1)"] --> B["Control Expenses<br>(Cure 2)"]
    B --> C["Invest Wisely<br>(Cure 3, Law 2)"]
    C --> D["Seek Competent Counsel<br>(Cure 4, Law 3)"]
    D --> E["Avoid Bad Investments<br>(Laws 4 & 5)"]
    E --> F["Build Walls of Protection<br>(Walls parable, Cure 6)"]
    F --> G["Own Your Dwelling<br>(Cure 5)"]
    G --> H["Increase Earning Power<br>(Cure 7)"]
    H --> I["Act Decisively on<br>Opportunity (Luck parable)"]
    I --> J["Maintain Discipline<br>Even in Debt (Dabasir)"]
    J -->|"Cycle continues"| A
...
```mermaid
flowchart LR
    A["Abstract principle<br>(Save 10%)"] --> B["Easily forgotten<br>within days"]
    C["Parable<br>(Arkad's story)"] --> D["Emotionally encoded<br>in memory"]
    D --> E["Surfaces automatically<br>at decision points"]
    E --> F["Behaviour changes<br>without willpower"]
    style A fill:#e74c3c,color:#fff
