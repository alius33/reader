...
date: 2026-03-25
type: book-summary
tags: [book-summary, management, effectiveness, leadership, time-management, decision-making, knowledge-work, strengths, concentration, Drucker]
...
```mermaid
flowchart TD
    A["1. KNOW THY TIME<br>(Record, analyse, consolidate)"] --> B["2. FOCUS ON CONTRIBUTION<br>('What can I contribute?')"]
    B --> C["3. MAKE STRENGTH PRODUCTIVE<br>(Build on strengths, not weaknesses)"]
    C --> D["4. FIRST THINGS FIRST<br>(Concentrate on the vital few)"]
    D --> E["5. EFFECTIVE DECISIONS<br>(Few, important, systematic)"]
    E --> F["RESULTS"]
...
```mermaid
flowchart LR
    subgraph "What Does NOT Predict Effectiveness"
        A1[Intelligence]
        A2[Charisma]
        A3[Knowledge]
        A4[Personality]
    end
    subgraph "What DOES Predict Effectiveness"
        B1[Time management]
        B2[Contribution focus]
        B3[Strength deployment]
        B4[Concentration]
        B5[Decision quality]
    end
```
...
```mermaid
flowchart TD
    A["Ask: What can I contribute?"] --> B["Discover: What does the institution need?"]
    B --> C["Reframe: What must I deliver?"]
    C --> D["Communicate: What do others need from me?"]
    D --> E["Develop: What must others learn to contribute?"]
    E --> F["Result: Self-renewing organisation"]
    style A fill:#2980b9,color:#fff
    style F fill:#27ae60,color:#fff
```
...
```mermaid
flowchart TD
    A["10 priorities, divided attention"] --> B["All 10 receive<br>30 minutes/day"]
    B --> C["None completed<br>All mediocre"]
    D["1 priority, full concentration"] --> E["1 receives<br>5 hours/day"]
    E --> F["Completed brilliantly<br>Creates momentum"]
...
```mermaid
flowchart TD
    A["1. CLASSIFY<br>Is this generic or unique?"] --> B["2. DEFINE BOUNDARY CONDITIONS<br>What must the decision accomplish?"]
    B --> C["3. DECIDE WHAT IS RIGHT<br>(Not what is acceptable)"]
    C --> D["4. BUILD IN THE ACTION<br>Who does what by when?"]
    D --> E["5. TEST AGAINST RESULTS<br>Feedback: is it working?"]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart LR
    A[Time Management] -->|creates space for| B[Contribution Focus]
    B -->|reveals where to deploy| C[Strengths]
    C -->|determines what to| D[Concentrate On]
    D -->|enables| E[Effective Decisions]
    E -->|produces| F[Results]
    F -->|frees up| A
    style A fill:#2980b9,color:#fff
    style F fill:#27ae60,color:#fff
```
...
```mermaid
flowchart TD
    A[Time Management] -->|develops| A1[Self-Awareness]
    B[Contribution Focus] -->|develops| B1[Empathy]
    C[Strength Building] -->|develops| C1[Confidence]
    D[Concentration] -->|develops| D1[Courage]
    E[Decision-Making] -->|develops| E1[Judgement]
    A1 --> F[Character]
    B1 --> F
    C1 --> F
    D1 --> F
