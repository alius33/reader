...
date: 2026-03-28
type: book-summary
tags: [book-summary, management, innovation, disruption, strategy, technology, leadership, value-networks, resource-allocation, organizational-design, product-life-cycle, disk-drives, startups, case-studies]
...
```mermaid
flowchart TD
    A["SUSTAINING<br>TECHNOLOGY"] --> B["Improves performance along<br>dimensions mainstream<br>customers value"]
    A --> C["Established firms<br>ALWAYS lead"]
    D["DISRUPTIVE<br>TECHNOLOGY"] --> E["Underperforms on mainstream<br>dimensions but offers new<br>attributes: simpler, cheaper,<br>smaller, more convenient"]
    D --> F["Entrant firms<br>ALWAYS lead"]
    B --> G["Customers demand it<br>→ Gets funded<br>→ Succeeds"]
    E --> H["Customers reject it<br>→ Gets starved<br>→ Entrants find<br>new markets"]
    H --> I["Entrants improve faster<br>than market demands<br>→ Invade mainstream<br>from below"]
...
```mermaid
flowchart TD
    subgraph TRAJECTORY MAP
        direction LR
        A["14-inch drives<br>(mainframes)"] -.->|overshoots| B["8-inch drives<br>(minicomputers)"]
        B -.->|overshoots| C["5.25-inch drives<br>(desktops)"]
        C -.->|overshoots| D["3.5-inch drives<br>(laptops)"]
        D -.->|overshoots| E["2.5-inch<br>(portables)"]
    end
    F["Each smaller drive initially<br>UNDERPERFORMS for the<br>mainstream market above it"] --> G["But improves at 50%/year<br>while market grows<br>at only 25%/year"]
    G --> H["Eventually INVADES<br>the mainstream market<br>from below"]
...
```mermaid
flowchart TD
    A["Step 1: Engineers develop<br>disruptive prototype<br>INSIDE established firm"] --> B["Step 2: Marketing tests it<br>with existing customers"]
    B --> C["Step 3: Customers reject it<br>(doesn't meet their needs)"]
    C --> D["Step 4: Resources redirected<br>to sustaining projects<br>(bigger markets, higher margins)"]
    D --> E["Step 5: Entrant firms<br>find NEW markets that<br>value the disruption"]
    E --> F["Step 6: Entrants improve<br>drive faster than market demands<br>→ Invade mainstream<br>→ Incumbents lose"]
...
```mermaid
flowchart LR
    A["FUNCTIONALITY<br>Best features win"] --> B["RELIABILITY<br>Most dependable wins"]
    B --> C["CONVENIENCE<br>Easiest to use wins"]
    C --> D["PRICE<br>Cheapest wins"]
    E["Performance Oversupply"] -.->|triggers each shift| A
    E -.-> B
    E -.-> C
    style D fill:#e74c3c,color:#fff
...
```mermaid
flowchart TD
    A["YEAR 0-2<br>Disruptive technology emerges<br>Engineers at established firms<br>often develop it first"] --> B["YEAR 2-5<br>Entrant firms find small<br>emerging market; established<br>firms ignore or reject it"]
    B --> C["YEAR 5-8<br>Entrants improve technology<br>at 50%/yr; market demand<br>grows at only 25%/yr"]
    C --> D["YEAR 8-12<br>Disruptive technology's<br>trajectory intersects<br>mainstream demand"]
    D --> E["YEAR 12+<br>Mainstream market switches<br>rapidly; established firms<br>scramble but it's too late"]
    F["ESTABLISHED FIRMS<br>are most profitable<br>and most admired<br>during Years 5-8"] -.-> D
    style A fill:#27ae60,color:#fff
...
```mermaid
flowchart TD
    A["New Technology<br>Opportunity"] --> B{"Does it improve<br>performance on dimensions<br>valued by EXISTING<br>customers?"}
    B -->|Yes| C["SUSTAINING<br>INNOVATION"]
    B -->|No — offers different<br>attributes to different<br>users| D["DISRUPTIVE<br>INNOVATION"]
    C --> E["Manage within<br>mainstream organization<br>Listen to customers<br>Follow standard planning"]
    D --> F["Create autonomous<br>organization<br>Find new markets<br>Plan for learning<br>Start small, iterate fast"]
...
```mermaid
flowchart LR
    A["Good management<br>practices"] --> B["Listen to<br>best customers"]
    B --> C["Invest in higher-<br>margin products"]
    C --> D["Target larger<br>markets"]
    D --> E["Ignore small, low-<br>margin disruptions"]
    E --> F["FAIL when disruption<br>invades mainstream"]
    style A fill:#27ae60,color:#fff
    style F fill:#e74c3c,color:#fff
```
