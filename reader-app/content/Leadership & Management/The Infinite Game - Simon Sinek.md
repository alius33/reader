...
date: 2026-03-25
type: book-summary
tags: [book-summary, leadership, infinite-game, just-cause, trust, strategy, business, long-term-thinking, ethics, simon-sinek]
author: Simon Sinek
...
```mermaid
flowchart TD
    A[Just Cause] --> B[Infinite Mindset]
    C[Trusting Teams] --> B
    D[Worthy Rivals] --> B
    E[Existential Flexibility] --> B
    F[Courage to Lead] --> B
    B --> G[Stay in the Game]
    B --> H[Inspire Will]
    B --> I[Generate Resources]
...
flowchart LR
    subgraph Finite
    A1[Known Players] --> A2[Fixed Rules]
    A2 --> A3[Agreed Objective]
    A3 --> A4[Clear Winner]
    end
    subgraph Infinite
    B1[Shifting Players] --> B2[Changeable Rules]
    B2 --> B3[No Endpoint]
    B3 --> B4[Keep Playing]
    end
...
```mermaid
flowchart TD
    A[Just Cause] --> B[Inspires sacrifice]
    A --> C[Survives leaders]
    A --> D[Drives hard choices]
    E[False Cause] --> F[Inspires only when convenient]
    E --> G[Dies with the CEO]
    E --> H[Gets abandoned under pressure]
...
```mermaid
flowchart LR
    A[Just Cause] --> B[Generates Will]
    B --> C[Discretionary Effort]
    C --> D[Better Performance]
    D --> E[Generates Resources]
    E --> F[Funds the Cause]
    F --> A
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A[Finite Pressure] --> B[Small Shortcut]
    B --> C[Rationalisation]
    C --> D[New Normal]
    D --> E[Bigger Shortcut]
    E --> C
    C --> F[Ethical Collapse]
...
```mermaid
flowchart LR
    A[Current Strategy] --> B{Does it still<br>advance the cause?}
    B -->|Yes| C[Continue & Improve]
    B -->|No| D[Existential Flex]
    D --> E[New Strategy<br>Same Cause]
    E --> B
...
```mermaid
flowchart TD
    JC[Just Cause] -->|Provides direction for| TT[Trusting Teams]
    JC -->|Anchors| EF[Existential Flexibility]
    TT -->|Enables| EF
    TT -->|Creates safety for| WR[Worthy Rival]
    WR -->|Reveals gaps addressed by| EF
    EF -->|Demands| CL[Courage to Lead]
    CL -->|Protects| TT
    CL -->|Sustains commitment to| JC
    style JC fill:#2980b9,color:#fff
