...
```mermaid
flowchart LR
    A[Bar Raiser Veto] --> B[Only Strong Hires]
    B --> C[Higher Team Average]
    C --> D[Better Interview Loops]
    D --> E[Higher Candidate Bar]
    E --> A
...
```mermaid
flowchart TD
    subgraph Skills Forward
    S1[What do we have?] --> S2[What can we build?] --> S3[Hope customers want it]
    end
    subgraph Working Backwards
    W1[What does customer need?] --> W2[What must we invent?] --> W3[Build capabilities]
    end
...
flowchart TD
    ...
    E --> F[Launch AWS 2006]
    F --> G[Flywheel: Lower Prices --> More Users --> Scale --> Lower Prices]
