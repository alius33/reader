...
date: 2026-03-25
type: book-summary
tags: [leadership, coaching, teamwork, character, values, basketball, pyramid-of-success]
...
flowchart TB
    subgraph Foundation["Foundation Tier"]
        A[Industriousness] --- B[Friendship]
        B --- C[Loyalty]
        C --- D[Cooperation]
        D --- E[Enthusiasm]
    end
    subgraph Second["Second Tier"]
        F[Self-Control] --- G[Alertness]
        G --- H[Initiative]
        H --- I[Intentness]
    end
    subgraph Heart["Heart of the Pyramid"]
        J[Condition] --- K[Skill]
        K --- L[Team Spirit]
    end
    subgraph Near["Near the Top"]
        M[Poise] --- N[Confidence]
    end
    subgraph Apex["Apex"]
        O[Competitive Greatness]
    end
    Foundation --> Second
    Second --> Heart
    Heart --> Near
...
```mermaid
flowchart LR
    A[Faith & Patience<br/>The Mortar] --> B[Holds all<br/>blocks together]
    B --> C[Enables<br/>Competitive Greatness]
    C --> D[Sustained excellence<br/>under pressure]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A[Expert Knowledge] --> B{Can you teach it?}
    B -->|Yes| C[Effective Leader]
    B -->|No| D[Failed Leadership]
    C --> E[Team performs<br/>at its potential]
    D --> F[Knowledge trapped<br/>inside the leader]
...
```mermaid
flowchart LR
    A[Meticulous<br/>planning] --> B[Efficient<br/>practice]
    B --> C[Maximum learning<br/>per minute]
    C --> D[Better preparation<br/>than competitors]
    D --> E[Competitive<br/>advantage]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A[1962: Near-championship<br/>loss to Cincinnati] --> B[Wooden recognises<br/>self-imposed limits]
    B --> C[Reviews everything:<br/>notebooks, stats, methods]
    C --> D[Three major changes]
    D --> E[Starting 7 system]
    D --> F[Conserve energy<br/>before playoffs]
    D --> G[Keep it simple<br/>in postseason]
    E --> H[1964: First NCAA<br/>championship]
    F --> H
    G --> H
    H --> I[10 championships<br/>in 12 years]
...
```mermaid
flowchart TD
    A[Adversity strikes] --> B{Leader's response}
    B -->|Blame & self-pity| C[Team loses heart<br/>Excuses multiply]
    B -->|Accept & adapt| D[Team grows stronger<br/>New solutions emerge]
    C --> E[Organisation weakens]
    D --> F[Organisation thrives]
