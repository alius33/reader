...
date: 2026-03-25
type: book-summary
tags: [book-summary, power, leadership, organisational-behaviour, influence, Stanford, evidence-based]
...
```mermaid
flowchart LR
    A["Leadership Industry<br>Message"] -->|"Be authentic<br>Be humble<br>Be a servant"| B["Aspiration<br>(How we wish<br>leaders behaved)"]
    C["Pfeffer's Evidence-<br>Based Message"] -->|"Be strategic<br>Be visible<br>Be political"| D["Reality<br>(How successful<br>leaders actually behave)"]
    B --> E["Feel good<br>But ineffective"]
    D --> F["Feel uncomfortable<br>But effective"]
...
```mermaid
flowchart TD
    A["Limiting Belief<br>(e.g. 'merit is enough')"] --> B["Self-Limiting Behaviour<br>(e.g. avoid self-promotion)"]
    B --> C["Low Visibility"]
    C --> D["Fewer Opportunities"]
    D --> E["Confirms Belief<br>('the system is unfair')"]
    E --> A
    F["Break the Cycle:<br>Strategic Visibility"] -.->|Intervene here| C
...
```mermaid
flowchart TD
    A["Project confidence<br>(even if you don't feel it)"] --> B["Others respond<br>with deference"]
    B --> C["You feel more<br>confident (feedback)"]
    C --> D["You project even<br>more confidence"]
    D --> B
...
```mermaid
flowchart TD
    A["Clarity<br>(What you stand for)"] --> E["Powerful Brand"]
    B["Consistency<br>(Same message, every time)"] --> E
    C["Visibility<br>(Right people see it)"] --> E
    D["Evidence<br>(Proof it's real)"] --> E
    E --> F["Others tell your story<br>for you (reputation)"]
...
```mermaid
flowchart TD
    A["You have power<br>(position, authority, resources)"] --> B{"Do you use it?"}
    B -->|"Yes — decisively"| C["Others recognise<br>your authority"]
    C --> D["More power flows<br>to you (reinforcing)"]
    B -->|"No — you defer<br>and avoid"| E["Others learn<br>your authority is nominal"]
    E --> F["Power flows<br>to others (eroding)"]
    style C fill:#27ae60,color:#fff
...
```mermaid
flowchart TD
    A["Outcome: Success"] --> B["Methods retroactively<br>reinterpreted as:<br>bold, decisive, visionary"]
    C["Outcome: Failure"] --> D["Methods retroactively<br>reinterpreted as:<br>reckless, abusive, delusional"]
    E["Same Behaviour"] --> A
    E --> C
...
```mermaid
flowchart TD
    R1["Rule 1: Get Out<br>of Your Own Way"] --> R3["Rule 3: Appear<br>Powerful"]
    R1 --> R4["Rule 4: Build<br>a Brand"]
    R2["Rule 2: Break<br>the Rules"] --> R3
    R3 --> R6["Rule 6: Use<br>Your Power"]
    R4 --> R5["Rule 5: Network<br>Relentlessly"]
    R5 --> R6
    R6 --> R7["Rule 7: Success<br>Excuses Everything"]
    R7 -.->|"Success removes<br>self-doubt"| R1
    style R1 fill:#2980b9,color:#fff
    style R7 fill:#27ae60,color:#fff
```
