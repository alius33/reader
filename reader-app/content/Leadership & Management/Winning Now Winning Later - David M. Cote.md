...
date: 2026-03-25
type: book-summary
tags: [leadership, management, strategy, long-term-thinking, honeywell, turnaround, process-improvement, culture]
author: David M. Cote
...
```mermaid
flowchart TD
    A[Scrub to Reality] --> B[Generate Honest Data]
    B --> C[Make Better Decisions]
    C --> D[Hold Fixed Costs Constant]
    D --> E[Create Investment Surplus]
    E --> F[Fund Growth Initiatives]
    F --> G[Improved Performance]
    G --> H[More Cash to Invest]
    H --> E
...
```mermaid
flowchart LR
    A[Sales Growth] --> B[Variable Margin<br>30-80%]
    B --> C{Fixed Costs<br>Held Constant?}
    C -->|Yes| D[Increased Profits]
    C -->|No| E[Margin Consumed<br>by Cost Creep]
    D --> F[Fund Investment]
    D --> G[Return to Investors]
    F --> H[Better Products<br>& Processes]
    H --> A
...
```mermaid
flowchart TD
    A[Rigorous MRR Process] --> B[Identify Top Talent]
    B --> C[Give Leap Assignments]
    C --> D[Evaluate Performance]
    D --> E{Right Fit?}
    E -->|Yes| F[Develop & Retain]
    E -->|No| G[Transition Out]
    F --> H[Reduce Total Leaders<br>Pay Remaining More]
    H --> I[Stronger Performance<br>per Leader]
    G --> A
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart LR
    A[Build Pipeline] --> B[Kill Bad Deals]
    B --> C[Never Overpay]
    C --> D[Integrate Well]
    D --> E[Track Results]
    E --> A
...
```mermaid
flowchart TD
    A[Recession Hits] --> B{Two Paths}
    B -->|Conventional| C[Mass Layoffs]
    C --> D[Lose Institutional Knowledge]
    D --> E[Slow Recovery]
    B -->|Cote's Approach| F[Furloughs + Benefit Cuts]
    F --> G[Preserve Industrial Base]
    G --> H[Continue Growth Investments]
    H --> I[Prepare Suppliers for Recovery]
    I --> J[Explosive Post-Recession Growth]
    style C fill:#e74c3c,color:#fff
...
```mermaid
flowchart LR
    A[2007: Identify<br>~12 Candidates] --> B[2007-2014:<br>Leap Jobs & Testing]
    B --> C[2014: Narrow<br>to Finalists]
    C --> D[2015-16: Strategic Plan<br>Presentation to Board]
    D --> E[2016: Unanimous<br>Board Vote]
    E --> F[Year 1: COO<br>Transition]
    F --> G[Year 2: CEO<br>Transition]
    style A fill:#2980b9,color:#fff
    style G fill:#27ae60,color:#fff
```
