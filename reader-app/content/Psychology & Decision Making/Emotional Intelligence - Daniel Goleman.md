...
date: 2026-03-25
type: book-summary
tags: [book-summary, emotional-intelligence, EQ, psychology, self-awareness, empathy, leadership, neuroscience]
...
```mermaid
flowchart TD
    A["1. SELF-AWARENESS<br>(Know your emotions)"] --> B["2. SELF-REGULATION<br>(Manage your emotions)"]
    B --> C["3. MOTIVATION<br>(Drive yourself)"]
    C --> D["4. EMPATHY<br>(Read others' emotions)"]
    D --> E["5. SOCIAL SKILLS<br>(Manage relationships)"]
...
```mermaid
flowchart TD
    A["Stimulus<br>(e.g. someone shouts)"] --> B["Thalamus<br>(relay station)"]
    B -->|"FAST route<br>(12 milliseconds)"| C["Amygdala<br>(emotional reaction)"]
    B -->|"SLOW route<br>(several hundred ms)"| D["Neocortex<br>(rational analysis)"]
    D --> C
    C --> E["Body responds:<br>heart rate, hormones,<br>muscle tension"]
    D --> F["Mind responds:<br>'Oh, it's just my friend<br>across the street'"]
...
```mermaid
flowchart LR
    A["AMYGDALA<br>(Alarm system)"] <-->|"Dynamic balance"| B["PREFRONTAL CORTEX<br>(Regulation system)"]
    A -->|"When dominant"| C["Hijack<br>Impulsive reactions<br>Emotional flooding"]
    B -->|"When dominant"| D["Suppression<br>Emotional flatness<br>Disconnection"]
    A <-->|"When balanced"| E["EMOTIONAL<br>INTELLIGENCE<br>Feel fully,<br>choose wisely"]
    style E fill:#27ae60,color:#fff
    style C fill:#e74c3c,color:#fff
...
```mermaid
flowchart LR
    A["Too Little Arousal<br>(Bored, unfocused)"] --> B["Optimal Arousal<br>(Alert, energised,<br>focused)"]
    B --> C["Too Much Arousal<br>(Anxious, scattered,<br>frozen)"]
    A -->|"Low performance"| D["Underachievement"]
    B -->|"Peak performance"| E["Flow State"]
    C -->|"Declining performance"| F["Choking / Burnout"]
...
```mermaid
flowchart TD
    A["ENTRY LEVEL<br>Technical Skill = 80%<br>EQ = 20%"] --> B["MID-LEVEL<br>Technical Skill = 50%<br>EQ = 50%"]
    B --> C["SENIOR LEADERSHIP<br>Technical Skill = 15%<br>EQ = 85%"]
    style A fill:#2980b9,color:#fff
    style B fill:#f9d77e,stroke:#d4a017
...
```mermaid
flowchart TD
    A["Person A:<br>makes a statement"] --> B["Person B:<br>amygdala fires<br>(perceived threat)"]
    B --> C["Person B:<br>reacts defensively"]
    C --> D["Person A:<br>amygdala fires<br>(counter-threat)"]
    D --> E["Person A:<br>escalates"]
    E --> F["BOTH in hijack<br>No productive<br>dialogue possible"]
    F --> G["Original issue<br>forgotten — conflict<br>is now about emotions"]
    style F fill:#e74c3c,color:#fff
...
```mermaid
flowchart TD
    A["Positive Emotions"] --> B["Broadened Attention"]
    A --> C["Increased Cognitive Flexibility"]
    B --> D["More Creative Associations"]
    C --> D
    D --> E["Innovation & Problem-Solving"]
    F["Negative Emotions"] --> G["Narrowed Attention"]
    F --> H["Rigid Thinking Patterns"]
    G --> I["Stereotyped Responses"]
    H --> I
    I --> J["Risk Aversion & Stagnation"]
...
```mermaid
flowchart TD
    subgraph internal ["INTRAPERSONAL (Self)"]
        SA["SELF-AWARENESS<br>Know what you feel"]
        SR["SELF-REGULATION<br>Manage what you feel"]
        M["MOTIVATION<br>Use what you feel"]
    end
    subgraph external ["INTERPERSONAL (Others)"]
        E["EMPATHY<br>Feel what they feel"]
        SS["SOCIAL SKILLS<br>Manage relationships"]
    end
    SA --> SR --> M --> E --> SS
    SA -.->|"Foundation for<br>EVERYTHING"| E
    SS --> RESULT["OUTCOMES:<br>Better decisions<br>Stronger relationships<br>More effective leadership<br>Greater resilience<br>Better health"]
