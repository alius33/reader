...
date: 2026-03-25
type: book-summary
tags: [book-summary, emotions, emotional-literacy, vulnerability, connection, shame, empathy, psychology, taxonomy, brene-brown]
author: Brene Brown
...
```mermaid
flowchart LR
    A[Raw Feeling] --> B[Accurate Naming]
    B --> C[Understanding]
    C --> D[Regulation]
    D --> E[Communication]
    E --> F[Connection]
...
```mermaid
flowchart TD
    A[Something feels wrong] --> B{Can you name it?}
    B -->|"No — diffuse unease"| C[Anxiety]
    B -->|"Yes — specific concern"| D[Worry]
    B -->|"Yes — impending doom"| E[Dread]
    C --> F[Needs: grounding,<br>presence, breathing]
    D --> G[Needs: problem-solving<br>or acceptance]
    E --> H[Needs: naming,<br>support, agency]
...
```mermaid
flowchart TD
    A[Someone else's success] --> B{Self-worth intact?}
    B -->|Yes| C[Admiration / Freudenfreude]
    B -->|No| D{Named or unnamed?}
    D -->|Named as envy| E[Information:<br>What do I value?]
    D -->|Unnamed| F[Resentment]
    F --> G[Schadenfreude]
    C --> H[Deeper connection]
    E --> H
    G --> I[Eroded compassion]
...
```mermaid
flowchart TD
    A[Effort + Hope] --> B[Setback]
    B --> C{Response?}
    C --> D[Frustration:<br>blocked but<br>still trying]
    C --> E[Discouragement:<br>losing<br>confidence]
    E --> F{Support?}
    F -->|Yes| D
    F -->|No| G[Resignation:<br>nothing I do<br>matters]
    G --> H{Intervention?}
    H -->|Mistaken for acceptance| I[Stagnation]
    H -->|Correctly identified| J[Path back to effort]
...
```mermaid
mindmap
  root((Empathy vs Sympathy))
    Empathy
      Feels WITH the person
      Goes into the dark hole
      Accesses own pain
      Says "I'm here"
      Builds connection
    Sympathy
      ...
      Creates distance
    Near Enemies
      Pity looks like empathy
      But communicates superiority
      Advice looks like care
      But communicates distance
```
...
```mermaid
flowchart TD
    A[I made a mistake] --> B{How do I interpret it?}
    B -->|"I did a bad thing"| C[Guilt]
    B -->|"I am a bad person"| D[Shame]
    C --> E[Empathy for<br>the harmed party]
    E --> F[Repair:<br>apology, change]
    D --> G[Self-protection:<br>hide, deny, attack]
    G --> H[No repair,<br>escalating harm]
    style C fill:#27ae60,color:#fff
...
```mermaid
flowchart TD
    A[Desire for Connection] --> B{Strategy?}
    B -->|Assess group, adapt self| C[Fitting In]
    B -->|Show authentic self| D[Belonging]
    C --> E[Accepted by group<br>but self rejected]
    D --> F{Group response?}
    F -->|Accept you| G[True Belonging]
    F -->|Reject you| H[Wilderness:<br>Standing alone]
    H --> I[Still authentic:<br>Self-belonging intact]
    E --> J[Exhaustion,<br>inauthenticity]
...
```mermaid
flowchart TD
    A[Loneliness] --> B[Hypervigilance<br>for social threat]
    B --> C[Neutral cues read<br>as rejection]
    C --> D[Withdrawal and<br>self-protection]
    D --> E[Deeper isolation]
    E --> A
    F[Micro-moment of<br>genuine connection] -.->|Interrupts cycle| B
    style A fill:#e74c3c,color:#fff
    style F fill:#27ae60,color:#fff
```
...
```mermaid
flowchart LR
    A[Relief:<br>Danger passed] --> B[Calm:<br>Deliberate regulation]
    B --> C[Contentment:<br>Having enough]
    C --> D[Tranquility:<br>Effortless peace]
    A -.-> E[Joy:<br>Deep connection]
    E -.-> F[Foreboding Joy:<br>Fear of loss]
    F -.->|Gratitude practice| E
...
```mermaid
flowchart TD
    A[Emotional Challenge] --> B{Response?}
    B -->|Armoured Certainty| C[Deny complexity]
    C --> D[Suppress unfamiliar emotions]
    D --> E[Connection breaks down]
    B -->|Grounded Confidence| F[Name the emotion]
    F --> G[Sit with discomfort]
    G --> H[Communicate authentically]
    H --> I[Connection deepens]
    style E fill:#e74c3c,color:#fff
...
```mermaid
flowchart TD
    subgraph "PAINFUL TERRITORY"
    A[Uncertainty:<br>Stress, Anxiety, Dread]
    B[Comparison:<br>Envy, Resentment]
    C[Unmet Expectations:<br>Disappointment, Regret]
    F[Deep Pain:<br>Anguish, Grief, Despair]
    H[Falling Short:<br>Shame, Guilt]
    M[Feeling Wronged:<br>Anger, Contempt, Hate]
    end
    subgraph "CONNECTING TERRITORY"
    G[Being With Others:<br>Empathy, Compassion]
    I[Connection:<br>Belonging, Loneliness]
    J[Open Heart:<br>Love, Trust, Betrayal]
    end
    subgraph "EXPANSIVE TERRITORY"
    D[Transcendence:<br>Awe, Wonder, Curiosity]
    E[Paradox:<br>Bittersweetness, Nostalgia]
    K[Self-Assessment:<br>Pride, Humility]
    L[Life Is Good:<br>Joy, Gratitude, Calm]
    end
    A --> G
    F --> G
    H --> G
