...
date: 2026-03-25
type: book-summary
tags: [book-summary, neuroscience, cognitive-performance, productivity, SCARF]
author: David Rock
...
```mermaid
flowchart TD
    A[Information from Environment] --> B{Stage Gate}
    B -->|Selected| C["The Stage (PFC)<br>~4 actors max"]
    B -->|Blocked| D["The Audience<br>(Long-term memory,<br>automated routines)"]
    C -->|Rehearsed| D
    D -->|Retrieved| C
    E["The Director<br>(Meta-cognition)"] -->|Observes & manages| C
    E -->|Decides what<br>gets on stage| B
    style C fill:#f9d77e,stroke:#d4a017
    style E fill:#a8d8ea,stroke:#3a8fb7
```
...
```mermaid
flowchart TD
    A["Task A<br>(Conscious)"] --> B{Prefrontal Cortex<br>Single Bottleneck}
    C["Task B<br>(Conscious)"] --> B
    B -->|Sequential processing| D["Task A processed<br>then Task B"]
    B -->|Attempted parallel| E["Rapid switching<br>Errors increase 3-5x<br>IQ drops ~10 points"]
    F["Task C<br>(Automated/Basal Ganglia)"] --> G["Parallel OK<br>(Walking + Talking)"]
    A --> G
    style E fill:#f5a8a8,stroke:#c44040
    style G fill:#a8d8a8,stroke:#3a8f3a
```
...
```mermaid
flowchart LR
    A["Too Low<br>Bored, disengaged"] -->|Add novelty<br>Add urgency| B["Optimal Zone<br>Alert + Interested"]
    B -->|Task well-matched| C["Flow State<br>Peak Performance"]
    B -->|Arousal climbs| D["Too High<br>Anxious, scattered"]
    D -->|Calm breathing<br>Sensory grounding| B
    style B fill:#a8d8a8,stroke:#3a8f3a
    style A fill:#d4d4d4,stroke:#888888
    style D fill:#f5a8a8,stroke:#c44040
```
...
```mermaid
flowchart LR
    A["Awareness<br>Simplify the problem"] --> B["Reflection<br>Observe your thinking process"]
    B --> C["Insight<br>Gamma burst — aha moment"]
    C --> D["Action<br>Capture it before it fades"]
    D -->|New impasse| A
    style C fill:#f9d77e,stroke:#d4a017
```
...
```mermaid
flowchart TD
    A["Emotional Event"] --> B["Suppression<br>(Push it down)"]
    A --> C["Attention Shifting<br>(Look elsewhere)"]
    A --> D["Labeling<br>(Name it)"]
    A --> E["Reappraisal<br>(Reinterpret it)"]
    B --> F["Amygdala INCREASES<br>PFC drains continuously<br>Physiology worsens"]
    C --> G["Amygdala unchanged<br>Temporary relief only"]
    D --> H["Amygdala DECREASES<br>Minimal PFC cost<br>Immediate effect"]
    E --> I["Amygdala DECREASES most<br>High PFC cost<br>Most durable effect"]
    style F fill:#f5a8a8,stroke:#c44040
    style H fill:#a8d8a8,stroke:#3a8f3a
    style I fill:#a8d8a8,stroke:#3a8f3a
```
...
```mermaid
flowchart TD
    A[Expectation Formed] --> B{Outcome vs Expectation}
    B -->|Outcome > Expected| C["Dopamine Surge<br>(Reward response)"]
    B -->|Outcome = Expected| D["No change<br>(Neutral)"]
    B -->|Outcome < Expected| E["Dopamine Crash<br>(Threat response)"]
    C --> F[Improved PFC function<br>Better decisions<br>Virtuous cycle]
    E --> G[Degraded PFC function<br>Worse decisions<br>Vicious cycle]
    style C fill:#a8d8a8,stroke:#3a8f3a
    style E fill:#f5a8a8,stroke:#c44040
```
...
```mermaid
flowchart TD
    S["Status<br>Perceived position"] --- SCARF["SCARF Model<br>(Social Survival Needs)"]
    C["Certainty<br>Predictability"] --- SCARF
    Au["Autonomy<br>Sense of control"] --- SCARF
    R["Relatedness<br>Friend or foe"] --- SCARF
    F["Fairness<br>Equitable exchange"] --- SCARF
    SCARF -->|Threat in any domain| T["Amygdala activation<br>Same circuits as physical pain<br>Reduced PFC function"]
    SCARF -->|Reward in any domain| Rw["Dopamine release<br>Same circuits as physical pleasure<br>Enhanced PFC function"]
    style SCARF fill:#a8d8ea,stroke:#3a8fb7
    style T fill:#f5a8a8,stroke:#c44040
    style Rw fill:#a8d8a8,stroke:#3a8f3a
```
...
```mermaid
flowchart TD
    A["Insight<br>(Opens the window)"] --> B["Attention Density"]
    B --> C["Frequency<br>(How often)"]
    B --> D["Duration<br>(How long each time)"]
    B --> E["Intensity<br>(Emotional engagement)"]
    C --> F["Neural Pathway<br>Stabilisation"]
    D --> F
    E --> F
    F -->|Sufficient density| G["New Default Pattern<br>(Permanent change)"]
    F -->|Insufficient density| H["Reversion to<br>Old Pattern"]
    style G fill:#a8d8a8,stroke:#3a8f3a
    style H fill:#f5a8a8,stroke:#c44040
```
...
```mermaid
flowchart LR
    A["Stimulus"] --> B["Automatic Reaction<br>(Default)"]
    A --> C["Director Intervenes"]
    C --> D["Notice"]
    D --> E["Pause"]
    E --> F["Label"]
    F --> G["Choose"]
    G --> H["Deliberate Response"]
    H -->|Repeated practice| I["New Neural Pathway<br>(Neuroplasticity)"]
    B -->|No intervention| J["Old Pattern<br>Reinforced"]
    style C fill:#a8d8ea,stroke:#3a8fb7
    style I fill:#a8d8a8,stroke:#3a8f3a
    style J fill:#f5a8a8,stroke:#c44040
```
...
```mermaid
flowchart TD
    A["Director Practice"] --> B["Better Prioritisation"]
    B --> C["Conserved PFC Resources"]
    C --> D["Better Emotion Regulation"]
    D --> E["Clearer Thinking Under Pressure"]
    E --> F["Better Social Interactions"]
    F --> G["Fewer SCARF Threats"]
    G --> C
    style A fill:#a8d8ea,stroke:#3a8fb7
    style C fill:#a8d8a8,stroke:#3a8f3a
```
