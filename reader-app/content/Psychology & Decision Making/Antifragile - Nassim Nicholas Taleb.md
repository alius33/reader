...
date: 2026-03-25
type: book-summary
tags: [book-summary, risk, uncertainty, antifragility, philosophy, systems, optionality, probability, resilience]
...
flowchart LR
    subgraph triad [The Triad]
        F["FRAGILE<br>(Damocles)<br>Breaks under stress"]
        R["ROBUST<br>(Phoenix)<br>Resists stress"]
        A["ANTIFRAGILE<br>(Hydra)<br>Gains from stress"]
    end
    S["Stressor / Shock"] --> F
    S --> R
    S --> A
    style F fill:#e74c3c,color:#fff
    style R fill:#f9d77e,stroke:#d4a017
...
```mermaid
flowchart LR
    subgraph ORG ["Organic (Cat)"]
        S1["Stressor"] --> I1["Information"]
        I1 --> A1["Adaptation"]
        A1 --> ST1["Stronger"]
    end
    subgraph MECH ["Mechanical (Washing Machine)"]
        S2["Stressor"] --> W1["Wear"]
        W1 --> D1["Degradation"]
        D1 --> WK1["Weaker"]
    end
    style ST1 fill:#27ae60,color:#fff
    style WK1 fill:#e74c3c,color:#fff
```
...
```mermaid
flowchart TD
    subgraph BU ["Bottom-Up (Souk)"]
        M1["Merchant 1"] --- M2["Merchant 2"]
        M2 --- M3["Merchant 3"]
        M3 --- M4["Merchant 4"]
        M4 --- M1
    end
    subgraph TD2 ["Top-Down (Corporation)"]
        CEO["CEO"] --> VP1["VP 1"]
        CEO --> VP2["VP 2"]
        VP1 --> E1["Employee"]
        VP1 --> E2["Employee"]
    ...
    SHOCK["Shock"] --> BU
    SHOCK --> TD2
    BU --> R1["Small failures<br>System adapts"]
    TD2 --> R2["Single point of failure<br>System collapses"]
...
```mermaid
flowchart TD
    W["Which world?"] -->|Physical measurements| MED["MEDIOCRISTAN<br>Bell curves<br>Averages work<br>Outliers irrelevant"]
    W -->|Social/economic/info| EXT["EXTREMISTAN<br>Power laws<br>Averages mislead<br>Outliers dominate"]
    MED --> S1["Safe to use:<br>Mean, std deviation<br>Historical data"]
    EXT --> S2["Must use:<br>Payoff structure<br>Optionality<br>Barbell"]
...
```mermaid
flowchart LR
    A["Day 1<br>Turkey fed"] --> B["Day 100<br>Turkey fed"]
    B --> C["Day 500<br>Turkey fed"]
    C --> D["Day 999<br>Turkey confident"]
    D --> E["Day 1001<br>Thanksgiving"]
    style A fill:#27ae60,color:#fff
...
```mermaid
flowchart LR
    subgraph BB ["The Barbell"]
        L["SAFE SIDE<br>90% in T-bills<br>Zero risk of ruin"]
        R["WILD SIDE<br>10% in moonshots<br>Unlimited upside"]
    end
    L --- AVOID
    R --- AVOID
    AVOID["AVOID THE MIDDLE<br>Balanced portfolios<br>Moderate risk<br>Illusion of safety"]
...
```mermaid
flowchart TD
    V["Volatility / Stressor"] --> C{"Payoff Shape?"}
    C -->|Convex| G["Gains more<br>than it loses"]
    C -->|Linear| N["Gains = Losses<br>(Robust)"]
    C -->|Concave| L["Loses more<br>than it gains"]
    G --> AF["ANTIFRAGILE"]
    N --> RO["ROBUST"]
    L --> FR["FRAGILE"]
    style AF fill:#27ae60,color:#fff
    style RO fill:#f9d77e,stroke:#d4a017
...
```mermaid
flowchart LR
    D["Decision Maker"] -->|Upside| DM["Keeps profits<br>Keeps bonus<br>Keeps career"]
    D -->|Downside| TP["Taxpayer pays<br>Employees fired<br>Community suffers"]
...
```mermaid
timeline
    title The Lindy Effect — Expected Future Life by Age
    3000 BC : Fire (5,000+ years)
          : Writing
    500 BC : The Wheel
          : Democracy
    1 AD : Major Religions (2,000+ years)
    1450 : Printing Press (575 years)
    1876 : Telephone (150 years)
    1995 : The Internet (31 years)
    2007 : Smartphone (19 years)
    2022 : ChatGPT (4 years)
```
...
```mermaid
flowchart TD
    U["Fundamental Uncertainty"] --> Q{"How to respond?"}
    Q -->|Fragile approach| P["Predict and Plan"]
    Q -->|Antifragile approach| B["Position and Prepare"]
    P --> F1["Build models"]
    P --> F2["Hire experts"]
    P --> F3["Optimise for one scenario"]
    B --> A1["Maintain optionality"]
    B --> A2["Apply via negativa"]
    B --> A3["Ensure skin in the game"]
    F1 --> FAIL["Catastrophic failure<br>when prediction fails"]
    F2 --> FAIL
    F3 --> FAIL
    A1 --> WIN["Benefit from<br>whatever happens"]
    A2 --> WIN
    A3 --> WIN
...
```mermaid
flowchart TD
    AF["ANTIFRAGILE<br>LIFE"] --> O["Optionality<br>(Right, not obligation)"]
    AF --> VN["Via Negativa<br>(Remove the harmful)"]
    AF --> SG["Skin in the Game<br>(Bear consequences)"]
    AF --> BB["Barbell<br>(Extreme safe + extreme risk)"]
    AF --> LE["Lindy Effect<br>(Trust what survived)"]
    O --> R["RESULT:<br>Benefit from<br>whatever happens"]
    VN --> R
    SG --> R
    BB --> R
    LE --> R
    style AF fill:#2980b9,color:#fff
