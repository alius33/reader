...
date: 2026-03-28
type: book-summary
tags: [book-summary, negotiation, conflict-resolution, persuasion, communication, harvard, principled-negotiation, BATNA]
...
```mermaid
flowchart TD
    N["NEGOTIATION"] --> P{"What approach?"}
    P --> S["SOFT POSITIONAL<br/>Make concessions<br/>Trust others<br/>Yield to pressure"]
    P --> H["HARD POSITIONAL<br/>Demand concessions<br/>Distrust others<br/>Apply pressure"]
    P --> PR["PRINCIPLED<br/>Explore interests<br/>Invent options<br/>Use fair standards"]
    S --> R1["Get exploited<br/>Feel bitter"]
    H --> R2["Exhaust resources<br/>Damage relationships"]
    PR --> R3["Wise agreement<br/>Efficient process<br/>Preserved relationship"]
...
```mermaid
flowchart LR
    A["<b>PEOPLE</b><br/>Separate people<br/>from the problem"] --> B["<b>INTERESTS</b><br/>Focus on interests<br/>not positions"]
    B --> C["<b>OPTIONS</b><br/>Invent options for<br/>mutual gain"]
    C --> D["<b>CRITERIA</b><br/>Insist on<br/>objective standards"]
...
```mermaid
flowchart TD
    A["STEP 1: What's Wrong?<br/>(Specific Problem)"] --> B["STEP 2: Analysis<br/>(Diagnose in General)"]
    B --> C["STEP 3: Approaches<br/>(General Prescriptions)"]
    C --> D["STEP 4: Action Ideas<br/>(Specific, Feasible Steps)"]
    D -.->|"generates<br/>new problems"| A
...
```mermaid
flowchart TD
    A["Proposed Agreement"] --> B{"Compare to BATNA"}
    B -->|"Agreement > BATNA"| C["ACCEPT<br/>The deal is better than<br/>your best alternative"]
    B -->|"Agreement < BATNA"| D["WALK AWAY<br/>Your alternative<br/>is better"]
    B -->|"Agreement ≈ BATNA"| E["NEGOTIATE HARDER<br/>Push for better terms<br/>or improve your BATNA"]
...
```mermaid
flowchart TD
    DT["DIRTY TRICKS"] --> DD["DELIBERATE<br/>DECEPTION"]
    DT --> PW["PSYCHOLOGICAL<br/>WARFARE"]
    DT --> PP["POSITIONAL<br/>PRESSURE"]
    DD --> DD1["Phony facts"]
    DD --> DD2["Ambiguous authority"]
    DD --> DD3["Dubious intentions"]
    PW --> PW1["Stressful situations"]
    PW --> PW2["Personal attacks"]
    PW --> PW3["Good-guy / Bad-guy"]
    PW --> PW4["Threats"]
    PP --> PP1["Refusal to negotiate"]
    PP --> PP2["Extreme demands"]
    PP --> PP3["Escalating demands"]
    PP --> PP4["Lock-in tactics"]
    PP --> PP5["Hardhearted partner"]
    PP --> PP6["Calculated delay"]
    PP --> PP7["Take it or leave it"]
    style DT fill:#e74c3c,color:#fff
    style DD fill:#c0392b,color:#fff
    style PW fill:#c0392b,color:#fff
    style PP fill:#c0392b,color:#fff
```
...
```mermaid
flowchart TD
    PREP["<b>PREPARE</b><br/>List interests (yours & theirs)<br/>Research standards & criteria<br/>Develop your BATNA<br/>Draft a framework agreement"] --> ENGAGE["<b>ENGAGE</b><br/>Separate people from problem<br/>Build working relationship<br/>Establish side-by-side orientation"]
    ENGAGE --> EXPLORE["<b>EXPLORE</b><br/>Focus on interests, not positions<br/>Ask 'Why?' and 'Why not?'<br/>Identify shared & differing interests<br/>Acknowledge their concerns"]
    EXPLORE --> INVENT["<b>INVENT</b><br/>Brainstorm options freely<br/>Use the Circle Chart<br/>Dovetail differences<br/>Look for mutual gain"]
    INVENT --> EVALUATE["<b>EVALUATE</b><br/>Apply objective criteria<br/>Test reciprocal application<br/>Use fair procedures<br/>Yield to principle, not pressure"]
    EVALUATE --> COMMIT["<b>COMMIT</b><br/>Draft 'yesable propositions'<br/>Move gradually toward commitment<br/>Be generous at the end<br/>Ensure face-saving for both sides"]
    COMMIT --> REVIEW["<b>REVIEW</b><br/>Compare outcome to BATNA<br/>Verify implementation feasibility<br/>Maintain relationship for future"]
    style PREP fill:#3498db,color:#fff
    style ENGAGE fill:#2ecc71,color:#fff
    style EXPLORE fill:#e67e22,color:#fff
    style INVENT fill:#9b59b6,color:#fff
    style EVALUATE fill:#1abc9c,color:#fff
    style COMMIT fill:#e74c3c,color:#fff
    style REVIEW fill:#34495e,color:#fff
```
