...
date: 2026-03-25
type: book-summary
tags: [networking, relationships, social-capital, personal-brand, generosity, mentoring]
...
```mermaid
flowchart LR
    A[Give Generously] --> B[Build Trust]
    B --> C[Deepen Relationships]
    C --> D[Expand Network]
    D --> E[More to Give]
    E --> A
...
```mermaid
flowchart TD
    A[Before: Help Organise] --> B[Research Attendees]
    B --> C[Pre-Invite to Private Dinner]
    C --> D[During: Speak or Ask Great Questions]
    D --> E[Master the Deep Bump]
    E --> F[Host Conference Within Conference]
    F --> G[After: Follow Up Within 24 Hours]
...
```mermaid
flowchart LR
    A[World A: Tech] --- B((You: The Broker))
    B --- C[World B: Media]
    B --- D[World C: Finance]
    B --- E[World D: Government]
...
```mermaid
flowchart TD
    A[Diverse Activities] --> D[Serendipitous Encounters]
    B[Mixed Social Events] --> D
    C[Open Sharing of Goals] --> D
    D --> E[Novel Opportunities]
    E --> F[New Connections]
    F --> A
...
```mermaid
flowchart TD
    A[Identify Your Unique Proposition] --> B[Recruit a Credible Co-Founder]
    B --> C[Land an Anchor Figure]
    C --> D[Credibility Attracts Members]
    D --> E[You as Founder = Centre of Network]
    style A fill:#2980b9,color:#fff
    style E fill:#27ae60,color:#fff
```
...
```mermaid
flowchart LR
    A[Learn from Mentors] --> B[Apply Knowledge]
    B --> C[Achieve Results]
    C --> D[Mentor Others]
    D --> E[Learn from Mentees]
    E --> A
    style A fill:#2980b9,color:#fff
