...
date: 2026-03-25
type: book-summary
tags: [book-summary, psychopathy, corporate, leadership, manipulation, workplace, dark-psychology, office-politics]
...
```mermaid
flowchart TD
    A["Psychopathy Traits"] --> B["Interpersonal"]
    A --> C["Emotional"]
    A --> D["Behavioural"]
    B --> B1["Superficial charm"]
    B --> B2["Grandiosity"]
    B --> B3["Pathological lying"]
    B --> B4["Manipulation"]
    C --> C1["No remorse"]
    C --> C2["Shallow emotions"]
    C --> C3["Callousness"]
    C --> C4["No responsibility"]
    D --> D1["Impulsivity"]
    D --> D2["Irresponsibility"]
    D --> D3["Parasitic lifestyle"]
    D --> D4["Poor self-control"]
...
```mermaid
flowchart TD
    P["PSYCHOPATH"] -->|"Visionary persona"| CEO["CEO (Patron)"]
    P -->|"Team player persona"| PEERS["Peers"]
    P -->|"Charm + intimidation"| SUBS["Subordinates (Pawns)"]
    P -->|"Calculated friendship"| HR["HR Director (Pawn)"]
    P -->|"Pre-emptive discrediting"| THREAT["Perceptive VP (Threat)"]
    CEO -->|"Provides protection"| P
    HR -->|"Provides information"| P
    SUBS -->|"Do the actual work"| P
    THREAT -.->|"Isolated and discredited"| X["Marginalised"]
    style P fill:#e74c3c,color:#fff
    style CEO fill:#2980b9,color:#fff
    style THREAT fill:#95a5a6,color:#fff
```
...
```mermaid
flowchart LR
    A["Enter new role"] --> B["Map power structure"]
    B --> C["Build patron + pawn network"]
    C --> D["Honeymoon period: appear transformative"]
    D --> E["Cracks appear: team damage, complaints"]
    E --> F{"Exposed?"}
    F -->|"No"| G["Promoted / move up"]
    F -->|"Yes"| H["Attack accuser, activate patrons"]
    H --> I{"Accuser neutralised?"}
    I -->|"Yes"| G
    I -->|"No"| J["Move to new org, restart"]
    G --> A
    J --> A
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart LR
    subgraph GENUINE["Genuine Leader"]
        GL1["Confidence grounded in competence"]
        GL2["Empathy drives people-first decisions"]
        GL3["Vision shared with credit distributed"]
        GL4["Composure from emotional regulation"]
    end
    subgraph PSYCHOPATH["Corporate Psychopath"]
        CP1["Confidence from absence of self-doubt"]
        CP2["Absence of empathy enables ruthlessness"]
        CP3["Vision claimed exclusively, credit hoarded"]
        CP4["Composure from absence of feeling"]
    end
    style GENUINE fill:#27ae60,color:#fff
    style PSYCHOPATH fill:#e74c3c,color:#fff
```
