...
date: 2026-03-27
type: book-summary
tags: [book-summary, parenting, unconditional-love, discipline, rewards, punishment, intrinsic-motivation, autonomy, self-esteem]
author: Alfie Kohn
...
```mermaid
flowchart TB
    subgraph doing_to ["The 'Doing To' Continuum"]
        P1["Harsh punishment<br/>(spanking, hitting)"]
        P2["Mild punishment<br/>(time-outs, grounding)"]
        P3["Tangible rewards<br/>(stickers, treats)"]
        P4["Verbal rewards<br/>('Good job!')"]
    end
    P1 --> Core["All communicate:<br/>'My love depends on<br/>your behavior'"]
    P2 --> Core
    P3 --> Core
    P4 --> Core
    ...
    subgraph working_with ["The Alternative: 'Working With'"]
        ...
        W3["Respect for autonomy"]
        W4["Long-term perspective"]
    end
    W1 --> Good["Genuine empathy,<br/>intrinsic motivation,<br/>psychological health"]
    W2 --> Good
    W3 --> Good
    W4 --> Good
```
...
```mermaid
flowchart TB
    subgraph conditional ["Two Sides of Conditional Parenting"]
        LW["Love Withdrawal<br/>(punishment, time-out, ignoring)"]
        PR["Positive Reinforcement<br/>(praise, rewards, stickers)"]
    end
    LW --> Message["Both communicate:<br/>'You must earn my love'"]
    PR --> Message
    Message --> Effects["Contingent self-esteem,<br/>anxiety, reduced intrinsic<br/>motivation, shallow compliance"]
```
...
```mermaid
flowchart TB
    subgraph grades ["What Grades Do to Learning"]
        G1["Focus on grades<br/>(extrinsic motivation)"]
        G2["Lose interest in learning<br/>(intrinsic motivation erodes)"]
        G3["Choose easiest tasks<br/>(minimize risk of failure)"]
        G4["Think superficially<br/>(skim for what's testable)"]
    end
    G1 --> G2
    G1 --> G3
    G1 --> G4
    G2 --> Result["Students who are skilled<br/>at school but unskilled<br/>at learning"]
    G3 --> Result
    G4 --> Result
```
...
```mermaid
flowchart TB
    subgraph instead ["Instead of 'Good Job!'"]
        N["Say nothing<br/>(just be present<br/>and attentive)"]
        D["Describe what you see<br/>'You used a lot of blue<br/>in that painting'"]
        A["Ask about their experience<br/>'How did you come up<br/>with that idea?'"]
        E["Explain the effect<br/>'That made your brother<br/>smile'"]
    end
    N --> Result["Child develops<br/>internal standards,<br/>intrinsic motivation,<br/>genuine self-esteem"]
    D --> Result
    A --> Result
    E --> Result
```
...
```mermaid
flowchart TB
    subgraph check ["Am I Parenting Conditionally?"]
        Q1["Do I withdraw affection<br/>when my child misbehaves?"]
        Q2["Do I praise primarily<br/>when my child pleases me?"]
        Q3["Does my child seem to<br/>need constant approval?"]
        Q4["Do I use consequences<br/>to 'teach lessons'?"]
        Q5["Does my child ask<br/>'Are you mad at me?'<br/>after conflicts?"]
        Q6["Do I say 'Good job!'<br/>more than 'Tell me about it'?"]
    end
    Q1 --> |"Yes"| Warning["These are signs of<br/>conditional parenting"]
    Q2 --> |"Yes"| Warning
    Q3 --> |"Yes"| Warning
    Q4 --> |"Yes"| Warning
    Q5 --> |"Yes"| Warning
    Q6 --> |"Yes"| Warning
    Warning --> Shift["Shift toward<br/>unconditional acceptance"]
```
...
```mermaid
flowchart TB
    subgraph principles ["Three Core Practices"]
        Love["Unconditional Love<br/>Love is the starting point,<br/>not the reward"]
        Choice["Choices for Children<br/>Maximize autonomy<br/>within safe boundaries"]
        Perspective["Child's Perspective<br/>Imagine how it looks<br/>from their side"]
    end
    Love --> Relationship["Strong parent-child<br/>relationship"]
    Choice --> Autonomy["Genuine autonomy<br/>& intrinsic motivation"]
    Perspective --> Empathy["Mutual empathy<br/>& trust"]
    Relationship --> LongTerm["Long-term goals:<br/>compassionate, curious,<br/>confident adults"]
    Autonomy --> LongTerm
    Empathy --> LongTerm
```
...
```mermaid
flowchart LR
    Kohn["Kohn<br/>(the philosophy:<br/>WHY unconditional)"] --> Gopnik["Gopnik<br/>(the science:<br/>WHY children explore)"]
    Gopnik --> Siegel["Siegel<br/>(the brain science:<br/>HOW connection works)"]
    Siegel --> Lansbury["Lansbury<br/>(the practice:<br/>WHAT to do today)"]
    Lansbury --> Faber["Faber & Mazlish<br/>(the scripts:<br/>WHAT to say)"]
```
