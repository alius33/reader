...
date: 2026-03-26
type: book-summary
tags: [book-summary, parenting, cross-cultural, indigenous, maya, inuit, hadzabe, anthropology, cooperation, emotional-intelligence]
...
```mermaid
flowchart TB
    subgraph team ["TEAM Parenting Framework"]
        T["T — Togetherness<br/>(Maya)<br/>Include children in real life"]
        E["E — Encouragement<br/>(Universal)<br/>Motivate through belonging,<br/>not praise"]
        A["A — Autonomy<br/>(Hadzabe)<br/>Freedom to explore,<br/>take risks, self-organise"]
        M["M — Minimal Interference<br/>(Inuit)<br/>Less controlling, less<br/>directing, less anger"]
    end
    T --> Result["Children who are:<br/>Helpful • Cooperative •<br/>Emotionally regulated •<br/>Confident • Self-motivated"]
    E --> Result
    A --> Result
    M --> Result
```
...
```mermaid
flowchart LR
    subgraph western ["Western Approach to Misbehaviour"]
        Anger["Parent Shows Anger<br/>(yell, punish, threaten)"]
    end
    subgraph inuit ["Inuit Approach to Misbehaviour"]
        Calm["Parent Stays Calm<br/>(walk away if needed)"]
    end
    Anger --> TeachesAnger["Child Learns: Anger is<br/>how adults handle problems"]
    Calm --> TeachesCalm["Child Learns: Calm is<br/>how adults handle problems"]
    TeachesAnger --> MoreAggression["More aggression,<br/>more dysregulation"]
    TeachesCalm --> MoreRegulation["More emotional<br/>intelligence, more respect"]
```
...
```mermaid
flowchart TB
    subgraph western2 ["WEIRD Parenting"]
        Control["Parent Controls<br/>→ Child Resists<br/>→ Power Struggle"]
        Entertainment["Parent Entertains<br/>→ Child Depends<br/>→ Boredom Intolerance"]
        Praise["Parent Praises<br/>→ Child Performs for Approval<br/>→ Extrinsic Motivation"]
    end
    subgraph team2 ["TEAM Parenting"]
        Together2["Togetherness<br/>→ Child Belongs<br/>→ Voluntary Help"]
        Encourage2["Encouragement<br/>→ Child Contributes<br/>→ Intrinsic Motivation"]
        Autonomy2["Autonomy<br/>→ Child Explores<br/>→ Confidence"]
        Minimal2["Minimal Interference<br/>→ Child Self-Directs<br/>→ Resilience"]
    end
```
...
```mermaid
flowchart TB
    subgraph controlParadigm ["Western: Control Paradigm"]
        PC["Parent Controls?"] --> YesPC["→ Authoritarian"]
        CC["Child Controls?"] --> YesCC["→ Permissive"]
        Both["Both Fight for Control?"] --> PowerStruggle["→ Power Struggle"]
    end
    subgraph cooperationParadigm ["Indigenous: Cooperation Paradigm"]
        Neither["Neither Controls"] --> Cooperate["→ Both Cooperate"]
        Cooperate --> ChildBelongs["Child Belongs,<br/>Contributes, Grows"]
    end
    controlParadigm -->|"Produces"| Conflict["Conflict, Resistance,<br/>Anxiety, Power Struggles"]
    cooperationParadigm -->|"Produces"| Harmony["Cooperation, Helpfulness,<br/>Confidence, Calm"]
```
...
```mermaid
flowchart TB
    HGP["Hunt, Gather, Parent<br/>(Cross-cultural evidence)"] --> Validates["Validates & Enriches"]
    Validates --> Montessori4["Montessori<br/>(Practical life = Maya togetherness)"]
    Validates --> RIE3["RIE / Lansbury<br/>(Calm leadership = Inuit never-anger)"]
    Validates --> WholeBrain2["Whole-Brain / Siegel<br/>(Connection first = universal principle)"]
    Validates --> Kohn["Kohn / Unconditional<br/>(No rewards = cross-cultural norm)"]
    Validates --> SelfDriven["Self-Driven Child<br/>(Autonomy = Hadzabe freedom)"]
    HGP --> Adds["Adds What Others Miss"]
    Adds --> CrossCultural["Cross-cultural evidence<br/>that these approaches are<br/>not Western innovations —<br/>they are recovered universals"]
```
