...
date: 2026-03-25
type: book-summary
tags: [book-summary, creativity, strategy, reframing, advertising, simplicity, lateral-thinking, competitive-advantage, problem-solving, upstream-thinking]
author: Dave Trott
...
```mermaid
flowchart TD
    A["PREY THINKING<br>(Reactive)"] --> B["Accept the brief<br>as given"]
    A --> C["Follow industry<br>conventions"]
    A --> D["Compete on same<br>ground as rivals"]
    E["PREDATORY THINKING<br>(Proactive)"] --> F["Redefine the<br>problem first"]
    E --> G["Break the<br>expected format"]
    E --> H["Fight where no<br>one else is"]
    B --> I["Incremental<br>improvement"]
    F --> J["Fundamental<br>advantage"]
...
```mermaid
flowchart TD
    A["Grand Strategy<br>(Redefine the game)"] --> B["Strategy<br>(Choose the battle)"]
    B --> C["Tactics<br>(Fight the battle)"]
    A -->|"Leverage: 10x"| D["Change the<br>competitive landscape"]
    B -->|"Leverage: 2x"| E["Pick the right<br>problem to solve"]
    C -->|"Leverage: 1.1x"| F["Execute the<br>existing plan better"]
...
```mermaid
flowchart LR
    A["SAME FACTS"] --> B{"How are they<br>framed?"}
    B -->|Convention| C["Expected<br>meaning"]
    B -->|Reframing| D["New<br>meaning"]
    C --> E["Ignored<br>(noise)"]
    D --> F["Noticed<br>(signal)"]
...
stateDiagram-v2
    [*] --> PreyMode
    PreyMode --> AcceptBrief: Default behaviour
    ...
    Reframe --> FightElsewhere: Create new ground
    FightElsewhere --> FundamentalAdvantage: Asymmetric win
```
...
```mermaid
flowchart TD
    A["APPARENT PURPOSE<br>Destroy the runway"] --> B["MEASURE<br>Bombs on target"]
    B --> C["VERDICT<br>Failure — 1 hit<br>out of 21 bombs"]
    D["REAL PURPOSE<br>Change Argentine<br>strategic thinking"] --> E["MEASURE<br>Enemy response"]
    E --> F["VERDICT<br>Success — fighters<br>pulled back to<br>defend mainland"]
...
```mermaid
flowchart TD
    A["Identify the<br>assumption"] --> B{"Is the<br>assumption<br>tested?"}
    B -->|No| C["VULNERABILITY<br>Strategy built<br>on faith"]
    B -->|Yes| D{"Is it<br>still valid?"}
    D -->|No| E["REFRAME<br>Ask a different<br>question"]
    D -->|Yes| F["PROCEED<br>Build strategy<br>on foundation"]
    C --> G["Maginot Line<br>Thinking"]
    E --> H["Predatory<br>Thinking"]
    style C fill:#e74c3c,color:#fff
...
```mermaid
flowchart LR
    A["Deep<br>Preparation"] --> B["Clear<br>Objective"]
    B --> C["Contact with<br>Reality"]
    C --> D{"Plan still<br>valid?"}
    D -->|Yes| E["Execute<br>the plan"]
    D -->|No| F["Adapt using<br>preparation depth"]
    F --> E
    style A fill:#2980b9,color:#fff
    style F fill:#27ae60,color:#fff
    style E fill:#27ae60,color:#fff
...
```mermaid
flowchart TD
    A["QUESTION<br>THE BRIEF"] --> B["FIND THE<br>ASSUMPTION"]
    B --> C["REFRAME THE<br>PROBLEM"]
    C --> D["SIMPLIFY"]
    D --> E["EXECUTE<br>WITH CRAFT"]
    F["OBSERVE<br>(don't research)"] --> B
    G["PREPARE<br>DEEPLY"] --> E
    H["HAVE COURAGE<br>TO BE WRONG"] --> E
    style A fill:#2980b9,color:#fff
