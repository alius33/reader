...
date: 2026-03-25
type: book-summary
tags: [book-summary, communication, data, numbers, persuasion, translation, framing, statistics, presentation, cognitive-psychology]
...
```mermaid
flowchart LR
    A["Raw Number"] --> B["Identify<br>the Point"]
    B --> C["Find Familiar<br>Anchor"]
    C --> D["Build<br>Comparison"]
    D --> E["Test for<br>Intuition"]
    E --> F["Audience<br>Understands"]
...
```mermaid
flowchart TD
    A["Raw Data<br>7:1 disease-to-combat<br>mortality ratio"] --> B{"Translation<br>Needed?"}
    B -->|"Present raw"| C["Tables of numbers<br>→ Politicians ignore"]
    B -->|"Translate"| D["Coxcomb Chart<br>→ Visual argument"]
    D --> E["Parliament funds<br>hospital reforms"]
    C --> F["No action taken<br>Soldiers keep dying"]
...
```mermaid
flowchart TD
    A["Static Total<br>600,000 per year"] --> B["Per Day<br>1,644"]
    A --> C["Per Hour<br>68"]
    A --> D["Per Minute<br>~1.1"]
    A --> E["Per Second<br>1 every 53 sec"]
    B --> F["Factual<br>briefing tone"]
    E --> G["Urgent<br>persuasive tone"]
...
```mermaid
flowchart LR
    A["8 Billion People"] --> B["100-Person<br>Village"]
    C["$6 Trillion Budget"] --> D["$60,000<br>Household"]
    E["4.5 Billion Years"] --> F["1 Calendar<br>Year"]
...
```mermaid
flowchart TD
    A["1. Avoid the Number<br>(Use a qualitative statement)"] --> B["2. Use a 1-in-X Ratio<br>(Personal and vivid)"]
    B --> C["3. Round to Friendly Number<br>(Thirds, halves, multiples of 10)"]
    C --> D["4. Use a Concrete Comparison<br>(Football fields, dollar bills)"]
    D --> E["5. Convert to a Process<br>(Per second, per day)"]
    E --> F["6. Pair with a Story<br>(Name, face, narrative)"]
...
```mermaid
quadrantChart
    title Number Scale vs. Brain Response
    x-axis Small Scale --> Large Scale
    y-axis Low Comprehension --> High Comprehension
    quadrant-1 Intuitive grasp
    quadrant-2 Translated successfully
    quadrant-3 Numbers numb
    quadrant-4 Need anchors
    1-7 items: [0.1, 0.95]
    8-100 items: [0.25, 0.7]
    100-10K items: [0.45, 0.4]
    10K-1M items: [0.6, 0.2]
    1M-1B items: [0.8, 0.1]
    1B+ items: [0.95, 0.05]
```
...
```mermaid
flowchart TD
    A["Raw Number"] --> B{"Ethical Check"}
    B -->|"Serves audience<br>understanding"| C["Translate<br>Illuminate<br>Calibrate"]
    B -->|"Serves your<br>agenda"| D["Manipulate<br>Obscure<br>Distort"]
    C --> E["Trust built<br>Decisions improved"]
    D --> F["Trust destroyed<br>Decisions corrupted"]
