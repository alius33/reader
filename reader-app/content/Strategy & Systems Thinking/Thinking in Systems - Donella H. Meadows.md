...
```mermaid
flowchart LR
    A[Gap Detected] --> B[Corrective Action]
    B --> C[Stock Changes]
    C --> D[Gap Narrows]
    D -->|Loop continues| A
...
```mermaid
stateDiagram-v2
    [*] --> Equilibrium: Balancing loop dominates
    [*] --> ExponentialGrowth: Reinforcing loop dominates
    Equilibrium --> Oscillation: Delays in balancing loop
    ExponentialGrowth --> Overshoot: Reinforcing hits limit
    Oscillation --> DampedOscillation: Strong balancing
    Oscillation --> GrowingOscillation: Weak balancing
    Overshoot --> Collapse: No recovery capacity
    Overshoot --> OscillatingDecline: Partial recovery
    DampedOscillation --> Equilibrium: System stabilizes
    GrowingOscillation --> Collapse: System destabilizes
...
```mermaid
flowchart TD
    A[System Archetypes] --> B[Goal Problems]
    A --> C[Competition Problems]
    A --> D[Resource Problems]
    B --> B1[Policy Resistance]
    B --> B2[Drift to Low Performance]
    B --> B3[Seeking Wrong Goal]
    B --> B4[Rule Beating]
    C --> C1[Escalation]
    C --> C2[Success to the Successful]
    D --> D1[Tragedy of the Commons]
    D --> D2[Shifting the Burden]
    style A fill:#2980b9,color:#fff
```
...
```mermaid
flowchart LR
    A[Actor A Escalates] --> B[Actor B Perceives Threat]
    B --> C[Actor B Escalates More]
    C --> D[Actor A Perceives Threat]
    D --> A
...
```mermaid
flowchart TD
    A[Problem] --> B[External Fix Applied]
    B --> C[Symptom Relieved]
    C --> D[No Pressure for Root Fix]
    D --> E[Self-Correction Atrophies]
    E --> F[Problem Worsens]
    F --> G[More External Fix Needed]
    G --> B
    style A fill:#e74c3c,color:#fff
...
```mermaid
flowchart LR
    subgraph Physical ["Physical Layer (12-10)"]
        A[Numbers] --> B[Buffers] --> C[Structures]
    end
    subgraph Feedback ["Feedback Layer (9-7)"]
        D[Delays] --> E[Balancing Loops] --> F[Reinforcing Loops]
    end
    subgraph Design ["Design Layer (6-4)"]
        G[Information] --> H[Rules] --> I[Self-Organisation]
    end
    subgraph Purpose ["Purpose Layer (3-1)"]
        J[Goals] --> K[Paradigms] --> L[Transcendence]
    end
    Physical --> Feedback --> Design --> Purpose
    style A fill:#e74c3c,color:#fff
    style L fill:#27ae60,color:#fff
```
