...
date: 2026-03-25
type: book-summary
tags: [critical-thinking, reasoning, egocentrism, intellectual-standards, fair-mindedness, decision-making, sociocentrism]
...
```mermaid
flowchart TD
    A[Critical Thinking Framework] --> B[Analysis<br>8 Elements of Reasoning]
    A --> C[Assessment<br>9 Intellectual Standards]
    A --> D[Character<br>7 Intellectual Virtues]
    B --> E[Break thinking into parts]
    C --> F[Measure quality of each part]
    D --> G[Ensure honest application]
    E --> H[Improved Thinking<br>& Better Life Outcomes]
    F --> H
    G --> H
...
```mermaid
flowchart TD
    A[Critical Thinking Skills] --> B{Used How?}
    B -->|Selfishly| C[Weak-Sense / Sophistry]
    B -->|Fairly| D[Strong-Sense / Fair-Mindedness]
    C --> E[Wins arguments<br>Manipulates others<br>Hides from truth]
    D --> F[Seeks truth<br>Respects all views<br>Changes mind when wrong]
...
```mermaid
flowchart LR
    T[Thinking<br>Creates Meaning] <--> F[Feeling<br>Evaluates Meaning]
    F <--> W[Wanting<br>Allocates Energy]
    W <--> T
...
```mermaid
flowchart LR
    S1[Stage 1<br>Unreflective] --> S2[Stage 2<br>Challenged]
    S2 --> S3[Stage 3<br>Beginning]
    S3 --> S4[Stage 4<br>Practicing]
    S4 --> S5[Stage 5<br>Advanced]
    S5 --> S6[Stage 6<br>Master]
...
```mermaid
flowchart TD
    P[Purpose] --> Q[Question]
    Q --> I[Information]
    I --> INF[Inferences]
    INF --> A[Assumptions]
    A --> C[Concepts]
    C --> IMP[Implications]
    IMP --> POV[Point of View]
    POV --> P
    style P fill:#2980b9,color:#fff
    style POV fill:#2980b9,color:#fff
```
...
```mermaid
flowchart TD
    A[Egocentric Mind] --> B[Seeks Gratification]
    A --> C[Seeks Self-Validation]
    B --> D[Dominating Ego<br>Forces others to comply]
    B --> E[Submissive Ego<br>Manipulates through servility]
    C --> F[Rationalises unethical behaviour]
    C --> G[Ignores contradicting evidence]
    D --> H[Short-term power<br>Long-term isolation]
    E --> I[Short-term security<br>Long-term resentment]
...
```mermaid
flowchart TD
    E[Ethics<br>Universal principles<br>about human welfare] --- R[Religion<br>Culturally relative<br>spiritual beliefs]
    E --- S[Social Conventions<br>Arbitrary cultural<br>practices]
    E --- L[The Law<br>Enforced rules<br>that may be unjust]
    E --- T[Sexual Taboos<br>Cultural norms<br>not ethical principles]
...
```mermaid
flowchart TD
    A[Key Idea 1<br>Thoughts, Feelings, Desires<br>are Interdependent] --> B[Key Idea 2<br>Everything Has a Logic<br>You Can Figure Out]
    B --> C[Key Idea 3<br>Assess Thinking Using<br>Intellectual Standards]
    C --> D[Key Idea 4<br>Egocentrism is the<br>Default Mechanism]
    D --> E[Key Idea 5<br>Be Sensitive to Others'<br>Egocentrism]
    E --> F[Key Idea 6<br>Mind Overgeneralises<br>Immediate Experience]
    F --> G[Key Idea 7<br>Ego Appears Rational<br>to Itself]
    G --> H[Key Idea 8<br>Ego is Automatic<br>and Unconscious]
    H --> I[Key Idea 9<br>Power Through Domination<br>or Submission]
    I --> J[Key Idea 10<br>Humans are Naturally<br>Sociocentric]
    J --> K[Key Idea 11<br>Rationality<br>Requires Work]
    style A fill:#2980b9,color:#fff
