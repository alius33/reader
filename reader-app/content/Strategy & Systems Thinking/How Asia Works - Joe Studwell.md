...
date: 2026-03-25
type: book-summary
tags: [book-summary, economics, development, industrial-policy, asia, land-reform, trade-policy, finance]
author: Joe Studwell
...
```mermaid
flowchart TD
    A[Land Reform] --> B[Smallholder Ownership]
    B --> C[Intensive Cultivation]
    C --> D[Higher Yields per Hectare]
    D --> E[Agricultural Surplus]
    E --> F[Rural Savings]
    E --> G[Food for Urban Workers]
    E --> H[Domestic Demand]
    F --> I[Capital for Industry]
    G --> I
    H --> I
    I --> J[Industrial Takeoff]
...
```mermaid
flowchart LR
    subgraph "Failed Model"
    A1[Collective Farming] --> B1[No Individual Incentive]
    B1 --> C1[Low Yields]
    C1 --> D1[Rural Poverty]
    D1 --> E1[No Surplus for Industry]
    end
    subgraph "Successful Model"
    A2[Household Responsibility] --> B2[Family Keeps Surplus]
    B2 --> C2[Intensive Cultivation]
    C2 --> D2[Yield Surge]
    D2 --> E2[Rural Surplus Funds Industry]
    end
...
```mermaid
flowchart TD
    A[Government Support] --> B{Can the Firm Export?}
    B -->|Yes| C[More Support + Expansion]
    C --> D[Genuine Competitiveness]
    B -->|No| E[Support Withdrawn]
    E --> F[Restructuring or Failure]
    D --> G[Move to Higher-Value Manufacturing]
    F --> H[Resources Redirected to Winners]
    H --> A
...
```mermaid
flowchart TD
    A[Premature Financial Liberalisation] --> B[Short-Term Foreign Capital Inflows]
    B --> C{Where Does Money Go?}
    C -->|With Export Discipline| D[Productive Manufacturing]
    C -->|Without Export Discipline| E[Real Estate & Speculation]
    D --> F[Crisis Impact: Recoverable]
    E --> G[Crisis Impact: Devastating]
    F --> H[Export Earnings Fund Recovery]
    G --> I[No Export Base to Drive Recovery]
...
```mermaid
flowchart LR
    A[Step 1: Land Reform] --> B[Agricultural Surplus]
    B --> C[Step 2: Industrial Policy]
    C --> D{Export Discipline?}
    D -->|Yes| E[Competitive Manufacturing]
    D -->|No| F[Crony Capitalism]
    E --> G[Step 3: Directed Finance]
    G --> H[Capital to Strategic Industry]
    H --> I[Sustained Growth]
    F --> J[Stagnation & Inequality]
    I --> K[Eventual Liberalisation from Position of Strength]
    style A fill:#27ae60,color:#fff
...
```mermaid
flowchart TD
    subgraph "What the Washington Consensus Prescribed"
    W1[Liberalise Early] --> W2[Free Markets Allocate Capital]
    W2 --> W3[Comparative Advantage Drives Growth]
    W3 --> W4[Prosperity]
    end
    subgraph "What Actually Worked in Asia"
    A1[Land Reform] --> A2[Directed Industrial Policy]
    A2 --> A3[Export Discipline]
    A3 --> A4[Financial Repression]
    A4 --> A5[Sustained Growth]
    A5 --> A6[Liberalise After Development]
    end
