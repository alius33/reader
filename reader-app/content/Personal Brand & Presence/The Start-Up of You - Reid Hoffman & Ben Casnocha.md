...
date: 2026-03-25
type: book-summary
tags: [book-summary, career-startup, networking, adaptability, risk, entrepreneurship]
...
```mermaid
flowchart LR
    A[Pre-Industrial Era] -->|"Everyone self-employed"| B[Industrial Revolution]
    B -->|"Career escalator created"| C[20th Century Stability]
    C -->|"Escalator breaks"| D[Post-2008 Reality]
    D -->|"Return to entrepreneurial default"| E[Permanent Beta]
...
```mermaid
flowchart TD
    A[Assets] --> D{Competitive Advantage}
    B[Aspirations] --> D
    C[Market Realities] --> D
    D --> E[Your Direction]
...
```mermaid
flowchart TD
    Y[You - 170 connections] --> A[1st Degree - 170 people]
    A --> B[2nd Degree - ~29,000 people]
    B --> C[3rd Degree - ~2,000,000 people]
    C -.->|"Trust breaks down"| D[4th Degree - No quality filter]
...
```mermaid
flowchart LR
    A[Domain Experts] --> D{You as Synthesiser}
    B[Close Confidants] --> D
    C[Smart Outsiders] --> D
...
```mermaid
flowchart TD
    A[Individual Capability] -->|"amplified by"| B[Network Relationships]
    B -->|"sustained by"| C[Paying It Forward]
    C -->|"strengthens"| B
    B -->|"produces"| D[Career Outcomes]
    D -->|"reinvested into"| C
    style A fill:#2980b9,color:#fff
    style D fill:#27ae60,color:#fff
```
