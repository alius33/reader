...
date: 2026-03-25
type: book-summary
tags: [book-summary, manipulation, covert-aggression, psychology, self-defence, personality-disorders, boundaries, character-disturbance]
...
```mermaid
flowchart TD
    A["Aggression"] --> B["Overt"]
    A --> C["Covert"]
    B --> D["Openly hostile<br>Easy to identify<br>You know you're in a fight"]
    C --> E["Disguised as something else<br>Charm, concern, helpfulness<br>You DON'T know you're in a fight"]
    E --> F["The covert-aggressive<br>WINS before you realise<br>you were playing"]
...
```mermaid
flowchart LR
    A["Neurotic"] --- B["Normal"] --- C["Character-Disturbed"]
    A --> D["Too much conscience<br>Excessive guilt<br>Self-doubt"]
    C --> E["Too little conscience<br>Entitlement<br>Externalises blame"]
    B --> F["Balanced<br>Self-aware<br>Accountable"]
...
```mermaid
flowchart TD
    A["Covert-Aggressive Tactics"] --> B["Confusion Tactics"]
    A --> C["Emotional Weapons"]
    A --> D["Reality Distortion"]
    A --> E["Identity Attacks"]
    B --> B1["Diversion"]
    B --> B2["Evasion"]
    B --> B3["Lying by omission"]
    C --> C1["Guilt-tripping"]
    C --> C2["Shaming"]
    C --> C3["Covert intimidation"]
    D --> D1["Minimisation"]
    D --> D2["Denial"]
    D --> D3["Rationalisation"]
    E --> E1["Vilifying the victim"]
    E --> E2["Projecting blame"]
    E --> E3["Playing the victim"]
    A --> F["Role-Playing"]
    F --> F1["Playing the servant"]
    F --> F2["Seduction"]
```
...
```mermaid
flowchart TD
    A["Harmful act committed"] --> B["Confronted?"]
    B -->|No| C["Continue unchecked"]
    B -->|Yes| D["Deny it happened"]
    D -->|Denial holds| C
    D -->|Denial fails| E["Minimise it"]
    E -->|Works| C
    E -->|Fails| F["Rationalise it"]
    F -->|Works| C
    F -->|Fails| G["Divert the conversation"]
    G -->|Works| C
    G -->|Fails| H["Guilt-trip the accuser"]
    H -->|Works| C
    H -->|Fails| I["Play the victim"]
    I -->|Works| C
    I -->|Fails| J["Vilify the accuser to others"]
...
```mermaid
flowchart LR
    A["Observe behaviour<br>(not words)"] --> B["Identify the tactic<br>(name it)"]
    B --> C["Refuse to engage<br>(no JADE)"]
    C --> D["Set clear boundary<br>(concrete, specific)"]
    D --> E["Enforce consistently<br>(no exceptions)"]
    E --> F["Accept reality<br>(they won't change)"]
...
```mermaid
flowchart TD
    subgraph OLD ["Old Pattern (Victim Mindset)"]
        A1["Observe harmful behaviour"] --> A2["Ask: WHY are they doing this?"]
        A2 --> A3["Generate sympathetic explanations"]
        A3 --> A4["Give benefit of the doubt"]
        A4 --> A5["Nothing changes"]
        A5 --> A1
    end
    subgraph NEW ["New Pattern (Empowered Mindset)"]
        B1["Observe harmful behaviour"] --> B2["Ask: WHAT are they doing?"]
        B2 --> B3["Name the tactic"]
        B3 --> B4["Set and enforce boundary"]
        B4 --> B5["Evaluate: did behaviour change?"]
        B5 -->|No| B6["Accept reality and adjust"]
        B5 -->|Yes| B7["Genuine progress"]
    end
