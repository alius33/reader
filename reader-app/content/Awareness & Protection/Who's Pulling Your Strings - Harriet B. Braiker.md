...
date: 2026-03-25
type: book-summary
tags: [manipulation, people-pleasing, vulnerability, self-protection, psychology, resistance-tactics, personality-disorders, emotional-control]
...
```mermaid
flowchart LR
    A[Legitimate Influence] -->|Open, honest| B[Mutual benefit]
    C[Manipulation] -->|Covert, coercive| D[Manipulator's gain]
    A -->|Reward-based| B
    C -->|Threat-based| D
...
```mermaid
flowchart TD
    A[Valerie speaks up] --> B[Jay gets angry]
    B --> C["See? This is why I can't marry you"]
    D[Valerie stays silent] --> E[No marriage discussed]
    E --> F[Status quo: Jay's preferred outcome]
    C --> F
...
```mermaid
flowchart TD
    A[People-pleasing] --> H[Soft Target]
    B[Approval addiction] --> H
    C[Emotophobia] --> H
    D[Inability to say no] --> H
    E[Blurry identity] --> H
    F[Low self-reliance] --> H
    G[External locus of control] --> H
    H --> I[Manipulator exploits vulnerabilities]
    I --> J[Compliance reinforces cycle]
    J --> H
...
```mermaid
flowchart LR
    A[Promise of Gain] -->|"Carrot phase"| B[Target Complies]
    B -->|"Hook is set"| C[Shift to Threat of Loss]
    C -->|"Stick phase"| D[Target Complies Out of Fear]
    D --> E[Escalating Control]
    style A fill:#27ae60,color:#fff
    style C fill:#e74c3c,color:#fff
```
...
```mermaid
flowchart TD
    A[Recognise Manipulation] --> B{Stay or Leave?}
    B -->|Stay| C[Deploy Resistance Tactics]
    B -->|Leave| D[Extraction]
    C --> E[Manipulator Adapts or Escalates]
    E -->|Adapts| F[Healthier Dynamic]
    E -->|Escalates| G{Tolerable?}
    G -->|Yes| C
    G -->|No| D
