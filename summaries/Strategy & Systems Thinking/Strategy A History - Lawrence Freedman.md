...
date: 2026-03-25
type: book-summary
tags: [book-summary, strategy, military-history, political-strategy, business-strategy, game-theory, decision-making, clausewitz, sun-tzu, machiavelli, nuclear-strategy, nonviolence, narrative, systems-thinking, guerrilla-warfare, revolution, management-theory]
...
```mermaid
flowchart TD
    A["Strategic Challenge"] --> B{"Starting Balance of Power"}
    B -->|"Favourable"| C["Strategy of Force<br>(Direct, Decisive)"]
    B -->|"Unfavourable"| D["Strategy of Guile<br>(Indirect, Adaptive)"]
    C --> E["Risk: Overconfidence<br>Ignoring friction"]
    D --> F["Risk: Cleverness without mass<br>Self-deception"]
    E --> G["Adapt or Fail"]
    F --> G
    G --> H["Sequential Decisions<br>Revise after each move"]
...
```mermaid
timeline
    title Evolution of Strategic Thought Across Freedman's Four Domains
    section Ancient Origins
        500 BC : Sun Tzu — Art of War
        400 BC : Thucydides — Peloponnesian War
        1513 : Machiavelli — The Prince
    section Military Revolution
        1832 : Clausewitz — On War
        1929 : Liddell Hart — Indirect Approach
        1961 : Schelling — Strategy of Conflict
        1976 : Boyd — OODA Loop
    section Strategy from Below
        1902 : Lenin — What Is To Be Done
        1937 : Mao — Guerrilla Warfare
        1930 : Gandhi — Nonviolent Resistance
        1963 : MLK — Birmingham Campaign
    section Strategy from Above
        1962 : Chandler — Strategy and Structure
        1980 : Porter — Competitive Strategy
        1994 : Mintzberg — Rise and Fall of Planning
        1997 : Christensen — Innovator's Dilemma
```
...
```mermaid
flowchart LR
    A["Greek Strategic Duality"] --> B["Bie — Force<br>(Achilles)"]
    A --> C["Metis — Cunning<br>(Odysseus)"]
    B --> D["Direct confrontation<br>Overwhelming power"]
    C --> E["Deception, manoeuvre<br>Indirect approach"]
    D --> F["Wins when:<br>Power gap is decisive"]
    E --> G["Wins when:<br>Opponent is rigid or arrogant"]
    F --> H["Risks:<br>Attrition, overreach"]
    G --> I["Risks:<br>Clever without mass"]
    style A fill:#2980b9,color:#fff
```
...
```mermaid
flowchart LR
    A["Clausewitz's Trinity"] --> B["Violence & Passion<br>(The People)"]
    A --> C["Chance & Probability<br>(The Military)"]
    A --> D["Political Purpose<br>(The Government)"]
    B --> E["Provides the energy<br>for war"]
    C --> F["Shapes how war<br>is fought"]
    D --> G["Defines why war<br>is fought"]
    style A fill:#2980b9,color:#fff
```
...
```mermaid
flowchart TD
    A["Nuclear Weapons Exist"] --> B{"Can war be won?"}
    B -->|"No — MAD"| C["Strategy = Deterrence<br>Prevent war, don't fight it"]
    B -->|"Maybe — Kahn"| D["Strategy = Survival<br>Prepare to fight and endure"]
    C --> E["Requires mutual vulnerability<br>Defence undermines stability"]
    D --> F["Requires civil defence<br>First-strike capability"]
    E --> G["Schelling: Threat that leaves<br>something to chance"]
    F --> H["Arms race escalation"]
    G --> I["Cold War stability<br>(precarious but real)"]
    H --> I
...
```mermaid
flowchart TD
    A["Initial Commitment"] --> B["Setback or<br>slower-than-expected progress"]
    B --> C{"Escalate or<br>withdraw?"}
    C -->|"Withdraw"| D["Admit failure<br>Lose credibility"]
    C -->|"Escalate"| E["Deeper commitment<br>Higher stakes"]
    E --> F["Further setback"]
    F --> C
    D --> G["Painful but<br>learning possible"]
    E -->|"Eventually"| H["Catastrophic failure<br>Forced withdrawal"]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart LR
    A["Observe<br>(Gather information)"] --> B["Orient<br>(Make sense of it)"]
    B --> C["Decide<br>(Choose action)"]
    C --> D["Act<br>(Execute)"]
    D --> A
    B -.->|"Implicit guidance<br>& control"| D
    B -.->|"Feedback"| A
...
```mermaid
flowchart TD
    A["The Left's Strategic Dilemma"] --> B["Reform<br>(Bernstein)"]
    A --> C["Revolution<br>(Lenin)"]
    A --> D["Mass Strike<br>(Luxemburg)"]
    B --> E["Win elections<br>Legislate change"]
    C --> F["Vanguard party<br>Seize crisis moments"]
    D --> G["Organic workers' struggle<br>Learn by doing"]
    E --> H["Risk: Co-optation<br>System absorbs dissent"]
    F --> I["Risk: Dictatorship<br>Party replaces class"]
    G --> J["Risk: Suppression<br>No organisation to resist"]
    style A fill:#2980b9,color:#fff
```
...
```mermaid
flowchart TD
    A["Strategic Intentions"] --> B["Deliberate Strategy<br>(What you planned)"]
    A --> C["Unrealised Strategy<br>(Plans that were abandoned)"]
    D["Emergent Strategy<br>(Responses to surprises)"] --> E["Realised Strategy<br>(What actually happened)"]
    B --> E
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A["Business Strategy Frameworks"] --> B["Porter: Five Forces<br>(Analyse competitive position)"]
    A --> C["Christensen: Disruption<br>(Watch for threats from below)"]
    A --> D["Kim & Mauborgne: Blue Ocean<br>(Create new market space)"]
    A --> E["Prahalad & Hamel:<br>Core Competence<br>(Leverage what you do best)"]
    B --> F["Static analysis<br>Good for established markets"]
    C --> G["Dynamic analysis<br>Good for technology transitions"]
    D --> H["Creative analysis<br>Good for value innovation"]
    E --> I["Capability analysis<br>Good for diversification"]
    style A fill:#2980b9,color:#fff
```
...
```mermaid
flowchart LR
    A["Strategy From Below"] --> B["Start weak<br>Build coalitions"]
    B --> C["Change the rules<br>Refuse enemy's terms"]
    C --> D["Exploit opponent's<br>rigidity"]
    D --> E["Patience &<br>endurance"]
    E --> F["Win through<br>adaptation"]
    G["Strategy From Above"] --> H["Start strong<br>Defend position"]
    H --> I["Optimise existing<br>systems"]
    I --> J["Risk: Rigidity &<br>overconfidence"]
    J --> K["Disrupted by<br>strategy from below"]
...
```mermaid
flowchart TD
    A["New Strategic Challenge"] --> B["New Framework Developed"]
    B --> C["Framework claims<br>scientific certainty"]
    C --> D["Applied broadly"]
    D --> E["Reality exposes<br>limitations"]
    E --> F["Framework revised<br>or abandoned"]
    F --> A
    style A fill:#e74c3c,color:#fff
...
```mermaid
flowchart TD
    A["Ancient Foundations"] --> B["Sun Tzu<br>Guile tradition"]
    A --> C["Thucydides<br>Realpolitik"]
    A --> D["Bible<br>Weaker-defeats-stronger"]
    B --> E["Liddell Hart<br>Indirect approach"]
    C --> F["Machiavelli<br>Fox and Lion"]
    D --> G["Gandhi & King<br>Nonviolent strategy"]
    F --> H["Clausewitz<br>Friction, Trinity"]
    H --> I["Nuclear Deterrence<br>Schelling, Kahn"]
    H --> J["Boyd<br>OODA Loop"]
    E --> K["Mao<br>Protracted War"]
    G --> L["Alinsky<br>Community Organising"]
    I --> M["Game Theory<br>Rational Choice"]
    M --> N["Business Strategy<br>Porter, Christensen"]
    J --> N
    style A fill:#2980b9,color:#fff
