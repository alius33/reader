...
date: 2026-03-25
type: book-summary
tags: [book-summary, strategy, competitive-advantage, choice-cascade, where-to-play, how-to-win]
...
```mermaid
flowchart TD
    A[Winning Aspiration] --> B[Where to Play]
    B --> C[How to Win]
    C --> D[Core Capabilities]
    D --> E[Management Systems]
    E -->|Feedback| A
    B <-->|Reinforce| C
    D <-->|Reinforce| C
...
```mermaid
flowchart LR
    A[Cost Leadership] -->|Lower cost| C[Wider Margin Gap]
    B[Differentiation] -->|Higher revenue| C
    C --> D[Sustainable Advantage]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart LR
    A[Consumer Understanding] --> B[Innovation]
    B --> C[Brand Building]
    C --> D[Go-to-Market]
    D --> E[Global Scale]
    E -->|Funds| A
...
```mermaid
flowchart TD
    A[Frame the Choice] --> B[Generate Possibilities]
    B --> C[Specify What Must Be True]
    C --> D[Identify Barriers]
    D --> E[Design Tests]
    E --> F[Conduct Tests]
    F --> G{Barrier Cleared?}
    G -->|Yes| H[Next Barrier]
    G -->|No| I[Eliminate Option]
    H --> G
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart LR
    A[Distinctive Activity System] --> B[Customer Loyalty]
    B --> C[Margin Advantage]
    C --> D[Resource Superiority]
