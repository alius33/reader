...
date: 2026-03-28
type: book-summary
tags: [book-summary, parenting, baby-sleep, sleep-training, newborn, toddler, independent-sleep, naps, bedtime, practical-guide]
...
```mermaid
flowchart TB
    subgraph problem ["Why Your Baby Isn't Sleeping"]
        A["Baby falls asleep<br/>with YOU<br/>(nursing, rocking, holding)"]
        B["Baby wakes every<br/>50-min sleep cycle"]
        C["Sleep association<br/>is GONE"]
        D["Baby cries until<br/>you recreate it"]
    end
    A --> B --> C --> D --> A

    subgraph solution ["The Fix"]
        E["Teach baby to fall<br/>asleep INDEPENDENTLY"]
        F["Baby wakes at<br/>sleep cycle"]
        G["Everything is the<br/>same as bedtime"]
        H["Baby falls back<br/>to sleep alone"]
    end
    E --> F --> G --> H
```
...
```mermaid
flowchart TD
    A["Baby isn't sleeping"] --> B{"Is baby falling asleep<br/>INDEPENDENTLY?"}
    B -->|No| C["THIS IS YOUR PROBLEM.<br/>Go to SWAP/SLIP chapters."]
    B -->|Yes| D{"Is bedtime consistent<br/>and age-appropriate?"}
    D -->|No| E["Fix bedtime timing<br/>and routine"]
    D -->|Yes| F{"Medical issue?<br/>(Reflux, allergy, apnea)"}
    F -->|Maybe| G["See pediatrician"]
    F -->|No| H{"Sleep regression?<br/>(Temporary, 1-2 weeks)"}
    H -->|Yes| I["Weather it. Don't create<br/>new bad habits."]
    H -->|No| J{"Too much/too little<br/>total sleep?"}
    J -->|Too much| K["Cap naps or<br/>shorten night"]
    J -->|Too little| L["Earlier bedtime,<br/>protect naps"]
    J -->|Just right| M["Environmental audit:<br/>light, noise, temp,<br/>diaper, hunger"]
```
...
```mermaid
flowchart TD
    A["NEWBORN<br/>(0-3 months)"] --> B["Use ALL Power Tools<br/>White noise + swaddle +<br/>pacifier + swing"]
    B --> C["Don't sweat<br/>independent sleep yet"]
    C --> D["Manage wake times<br/>Don't let baby stay<br/>awake too long"]
    D --> E["ESTABLISH BEDTIME<br/>(3-4 months)"]
    E --> F["Consistent time, routine,<br/>location every night"]
    F --> G["TEACH INDEPENDENT<br/>SLEEP (4-6 months)"]
    G --> H{"Try SWAP<br/>methods first"}
    H -->|Works| I["Maintain consistency"]
    H -->|Doesn't work| J["Move to SLIP<br/>(full commitment)"]
    J --> I
    I --> K["TACKLE NAPS<br/>(after bedtime is solid)"]
    K --> L["Four Conventions<br/>of Nap Nirvana"]
    L --> M["WEAN POWER TOOLS<br/>(as age-appropriate)"]
```
