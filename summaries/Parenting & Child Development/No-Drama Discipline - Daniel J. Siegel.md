...
date: 2026-03-26
type: book-summary
tags: [book-summary, parenting, discipline, neuroscience, child-development, brain-integration, emotional-intelligence, toddler]
...
```mermaid
flowchart LR
    Misbehaviour["Child Misbehaves"] --> ThreeQ{"Three Questions"}
    ThreeQ -->|"Why?"| Understand["Understand the cause<br/>(tired? hungry? overwhelmed?<br/>testing limits? lacking skills?)"]
    ThreeQ -->|"What?"| Lesson["Identify the lesson<br/>(impulse control? empathy?<br/>communication? safety?)"]
    ThreeQ -->|"How?"| Method["Choose the method<br/>(age-appropriate, context-aware,<br/>relationship-preserving)"]
    Method --> Connect["Step 1: CONNECT<br/>Emotional attunement<br/>Move from reactive to receptive"]
    Connect --> Redirect["Step 2: REDIRECT<br/>Teach the lesson<br/>Problem-solve together"]
...
```mermaid
flowchart LR
    Misbehaviour2["Child Misbehaves"] --> ParentReacts{"Parent's Response"}
    ParentReacts -->|"Punish / Yell / Lecture"| Reactive["Child Goes REACTIVE<br/>Amygdala fires<br/>Upstairs brain offline<br/>Cannot learn"]
    ParentReacts -->|"Connect First"| Receptive["Child Becomes RECEPTIVE<br/>Upstairs brain re-engages<br/>Can hear, process, learn"]
    Reactive -->|"Escalation cycle"| MoreDrama["More Drama<br/>Less Learning<br/>Relationship damaged"]
    Receptive -->|"Now redirect"| Teaching["Effective Teaching<br/>Skills built<br/>Relationship strengthened"]
```
...
```mermaid
flowchart TB
    Redirect["REDIRECT<br/>(After Connection)"] --> Insight["Insight<br/>What was happening inside me?<br/>Why did I act that way?"]
    Redirect --> Empathy["Empathy<br/>How did the other person feel?<br/>What was their perspective?"]
    Redirect --> Repair["Repair<br/>How can I make this right?<br/>What can I do to reconnect?"]
    Insight --> InternalCompass["Internal Compass<br/>(Self-regulation, self-awareness)"]
    Empathy --> InternalCompass
    Repair --> InternalCompass
    InternalCompass --> LifeSkills["Life Skills That Last<br/>Beyond this moment"]
```
...
```mermaid
flowchart TB
    Trigger2["Trigger: TV turned off"] --> Outburst["Outburst: 'I HATE YOU!'<br/>Door slam"]
    Outburst --> Pause2["Parent Pauses<br/>(3 Questions: Why? What? How?)"]
    Pause2 --> ConnectStep["CONNECT<br/>Knock gently. Validate.<br/>'I can tell you're upset.'"]
    ConnectStep --> GreenZone2["Child Returns to Green Zone<br/>(Calm enough to talk)"]
    GreenZone2 --> RedirectStep["REDIRECT<br/>'I hate you' hurt. What could<br/>you say instead when angry?"]
    RedirectStep --> RepairStep["REPAIR<br/>Child apologises in own way"]
    RepairStep --> InsightStep["BUILD INSIGHT (later)<br/>'What was happening inside you?'<br/>'What do you think I was feeling?'"]
    InsightStep --> BrainGrowth["Brain Growth:<br/>Insight + Empathy + Repair"]
```
