...
date: 2026-03-25
type: book-summary
tags: [business, innovation, leadership, amazon, long-term-thinking, customer-obsession, decision-making]
author: Jeff Bezos
...
```mermaid
flowchart LR
    A[Curiosity] --> B[Connect Arts & Science]
    B --> C[Think Different]
    C --> D[Reality Distortion]
    D --> E[Childlike Wonder]
    E -->|feeds back| A
...
```mermaid
flowchart TD
    A[Decision Required] --> B{Reversible?}
    B -->|Yes: Two-Way Door| C[Move Fast<br>Lightweight Process]
    B -->|No: One-Way Door| D{Data Available?}
    D -->|Yes| E[Math-Based<br>Let Data Decide]
    D -->|No| F[Judgment-Based<br>Debate + Commit]
...
```mermaid
flowchart TD
    A[Amazon's Three Pillars] --> B[Marketplace]
    A --> C[Prime]
    A --> D[AWS]
    B --> B1[58% of units by 2018]
    B1 --> B2[Third-party sellers<br>kicking first-party butt]
    C --> C1[100M+ paid members]
    C1 --> C2[Two-day shipping<br>Video, Music, Reading]
    D --> D1[$20B+ revenue]
    D1 --> D2[Reinvented how<br>companies buy computing]
    style A fill:#2980b9,color:#fff
```
...
```mermaid
flowchart TD
    A[Star Trek<br>Childhood Fantasy] --> B[Machine Learning<br>Decades of Research]
    A --> C[Cloud Computing<br>AWS Infrastructure]
    B --> D[Echo / Alexa]
    C --> D
    E[Fire Phone<br>Failure] -->|Developers + Learnings| D
    D --> F[100M+ Devices Sold]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart LR
    A[Energy at Work] -->|Makes you better| B[Energy at Home]
    B -->|Makes you better| A
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A[Customer Obsession] --> B[Long-Term Thinking]
    B --> C[Willingness to Fail]
    C --> D[Invention Through Wandering]
    D --> E[Self-Service Platforms]
    E --> F[Empowering Others to Create]
    F -->|Increases| A
    B --> G[High Standards Culture]
    G --> H[Six-Page Memos]
    G --> I[Type 1 / Type 2 Decisions]
    I --> J[High-Velocity Execution]
    J -->|Delivers for| A
    style A fill:#27ae60,color:#fff
