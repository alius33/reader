...
date: 2026-03-25
type: book-summary
tags: [book-summary, business, decision-making, thinking, strategy, financial-literacy, entrepreneurship, critical-thinking, risk-management, leadership, mental-models]
...
```mermaid
flowchart TD
    A["Most Business Owners"] --> B["Act on Impulse<br>Gut + Optimism"]
    B --> C["Unforced Errors<br>(Dumb Tax)"]
    C --> D["Firefighting<br>Crisis Management"]
    D --> B
    E["Cunningham's Approach"] --> F["Scheduled<br>Thinking Time"]
    F --> G["Powerful Questions<br>Structured Analysis"]
    G --> H["Better Decisions<br>Fewer Landmines"]
    H --> I["Compounding<br>Returns"]
...
```mermaid
timeline
    title Anatomy of a Thinking Time Session
    0-5 min : Question Selection
            : Write one specific question
            : No vague topics allowed
    5-15 min : Surface Thinking
             : Obvious answers emerge
             : Resist the urge to stop here
    15-30 min : Deep Wrestling
              : Discomfort peaks
              : Second-order effects surface
    30-45 min : Breakthrough Zone
              : Non-obvious insights appear
              : Connections between ideas form
    45-60 min : Actionable Conclusions
              : Distill 2-3 next steps
              : Review and commit to action
```
...
```mermaid
flowchart LR
    A["Inputs<br>Capital, Labour<br>Materials, Time"] --> B["Business Machine<br>Operations<br>Sales, Delivery"]
    B --> C["Outputs<br>Revenue, Profit<br>Cash, Satisfaction"]
    D["Drivers"] -.-> B
    E["Levers"] -.-> B
    F["Constraints"] -.-> B
...
```mermaid
flowchart TD
    A["Revenue<br>(Vanity)"] --> B["- Cost of Goods Sold"]
    B --> C["Gross Profit"]
    C --> D["- Operating Expenses"]
    D --> E["Net Profit<br>(Sanity)"]
    E --> F["± Working Capital<br>Changes"]
    F --> G["± Capital<br>Expenditures"]
    G --> H["Free Cash Flow<br>(King)"]
    style A fill:#e74c3c,color:#fff
...
```mermaid
flowchart TD
    A["Entrepreneur"] --> B{"Working IN<br>or ON the<br>business?"}
    B -->|"IN (Operator)"| C["Does the work<br>Solves problems<br>Manages daily tasks"]
    C --> D["Business depends<br>on founder"]
    D --> E["No leverage<br>No scalability<br>No freedom"]
    B -->|"ON (Owner)"| F["Thinks strategically<br>Builds systems<br>Hires and delegates"]
    F --> G["Business runs<br>independently"]
    G --> H["Leverage<br>Scalability<br>Freedom"]
...
```mermaid
flowchart LR
    A["Growth<br>Opportunity"] --> B{"Do I have the<br>infrastructure?"}
    B -->|"Yes"| C["Pursue Growth<br>Sustainably"]
    C --> D["Build More<br>Infrastructure"]
    D --> A
    B -->|"No"| E{"Can I build it<br>in time?"}
    E -->|"Yes"| F["Build First<br>Then Grow"]
    F --> C
    E -->|"No"| G["Pass or<br>Partner"]
    style C fill:#27ae60,color:#fff
...
```mermaid
flowchart TD
    A["Week 1-4<br>Feels unproductive<br>Mind wanders"] --> B["Week 5-8<br>Sessions get focused<br>First real insights"]
    B --> C["Month 3-6<br>Pattern recognition<br>Better questions"]
    C --> D["Month 6-12<br>Major course corrections<br>Avoid expensive mistakes"]
    D --> E["Year 2+<br>Compound effect<br>Transformed business"]
    style A fill:#e74c3c,color:#fff
