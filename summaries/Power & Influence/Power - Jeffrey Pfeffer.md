...
date: 2026-03-25
type: book-summary
tags: [book-summary, power, politics, influence, organisational-behaviour]
...
```mermaid
flowchart LR
    A[Will + Skill] --> B[Strategic Positioning]
    B --> C[Resources & Networks]
    C --> D[Reputation & Influence]
    D --> E[Power]
    E -->|Self-reinforcing| C
...
```mermaid
flowchart TD
    A[Do Unglamorous Work] --> B[Control Information/Events]
    B --> C[People Come to You]
    C --> D[Build Relationships]
    D --> E[Accumulate Influence]
    E --> F[Gain Formal Resources]
    F -->|Cycle repeats| B
    style A fill:#2980b9,color:#fff
    style F fill:#27ae60,color:#fff
```
...
```mermaid
flowchart TD
    A[First Impression Forms] --> B[Attention Decrement]
    A --> C[Cognitive Discounting]
    A --> D[Behavioural Confirmation]
    B --> E[Contradictory evidence ignored]
    C --> E
    D --> F[Self-fulfilling prophecy]
    E --> G[Original impression persists]
    F --> G
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A[Acquire Power] --> B[Approach behaviour increases]
    A --> C[Inhibition decreases]
    B --> D[Boldness & risk-taking]
    C --> E[Empathy & vigilance erode]
    D --> F[Overconfidence]
    E --> F
    F --> G[Power Lost]
