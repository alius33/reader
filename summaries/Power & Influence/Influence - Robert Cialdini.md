...
date: 2026-03-25
type: book-summary
tags: [book-summary, persuasion, influence, compliance, psychology, social-psychology]
...
```mermaid
flowchart LR
    A["Gift or Favour Given"] --> B["Obligation Triggered"]
    B --> C{"Target's Response"}
    C -->|"Repays"| D["Social Equilibrium Restored"]
    C -->|"Refuses to Repay"| E["Labelled Ingrate"]
    E --> F["Social Punishment"]
...
sequenceDiagram
    ...
    R->>T: Large request ($5 tickets)
    T->>R: Refusal
    Note over R: Appears to concede
    R->>T: Smaller request ($1 chocolate bars)
    Note over T: Feels obligated to reciprocate the concession
    T->>R: Compliance
    Note over T: Also perceives $1 as trivial after $5 (contrast)
```
...
```mermaid
flowchart TD
    A["Small Commitment"] --> B["Written?"]
    B -->|"Yes"| C["Stronger"]
    B -->|"No"| D["Weaker"]
    C --> E["Public?"]
    D --> E
    E -->|"Yes"| F["Even Stronger"]
    E -->|"No"| G["Moderate"]
    F --> H["Effortful?"]
    G --> H
    H -->|"Yes"| I["Very Strong"]
    H -->|"No"| J["Moderate-Strong"]
    I --> K["Internally Motivated?"]
    J --> K
    K -->|"Yes"| L["UNSHAKEABLE"]
    K -->|"No"| M["Strong but Fragile"]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A["Ambiguous event occurs"] --> B{"Each bystander looks to others"}
    B --> C["Others appear calm"]
    C --> D["Conclude: not an emergency"]
    D --> E["Remain passive"]
    E --> F["Your passivity confirms others' conclusion"]
    F --> B
...
```mermaid
flowchart TD
    A["Item Freely Available"] --> B{"Supply Changes?"}
    B -->|"Always scarce"| C["Desirable"]
    B -->|"Newly scarce"| D["More Desirable"]
    D --> E{"Why scarce?"}
    E -->|"Accident/error"| F["High Desire"]
    E -->|"Social demand"| G["Highest Desire"]
    G --> H{"Add competition?"}
    H -->|"Yes"| I["FRENZY"]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A["Tupperware Party"] --> B["Reciprocity<br>(free gifts, party games)"]
    A --> C["Commitment<br>(public testimonials)"]
    A --> D["Social Proof<br>(watching others buy)"]
    A --> E["Liking<br>(hostess is a friend)"]
    A --> F["Scarcity<br>(limited-time offers)"]
    B --> G["PURCHASE"]
    C --> G
    D --> G
    E --> G
...
```mermaid
flowchart LR
    A["Trigger Detected"] --> B{"Identify Principle"}
    B --> C["Reciprocity"]
    B --> D["Commitment"]
    B --> E["Social Proof"]
    B --> F["Liking"]
    B --> G["Authority"]
    B --> H["Scarcity"]
    C --> I["Ask: Gift or trick?"]
    D --> J["Ask: Would I choose this again?"]
    E --> K["Ask: Is the crowd informed?"]
    F --> L["Ask: Seller or product?"]
    G --> M["Ask: Real expertise?"]
    H --> N["Ask: Own it or use it?"]
    I --> O["DECIDE CONSCIOUSLY"]
    J --> O
    K --> O
    L --> O
    M --> O
