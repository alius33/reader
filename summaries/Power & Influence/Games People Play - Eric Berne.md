...
date: 2026-03-25
type: book-summary
tags: [book-summary, transactional-analysis, psychology, communication, games, relationships, ego-states, manipulation]
author: Eric Berne
...
```mermaid
flowchart TD
    subgraph person1 ["Person A"]
        P1["Parent"]
        A1["Adult"]
        C1["Child"]
    end
    subgraph person2 ["Person B"]
        P2["Parent"]
        A2["Adult"]
        C2["Child"]
    end
    A1 <-->|"Complementary<br>(smooth)"| A2
    P1 -->|"Crossed<br>(conflict)"| C2
    C1 -.->|"Ulterior<br>(hidden game)"| P2
...
```mermaid
flowchart TD
    A["Stimulus: Social Level<br>(Adult → Adult)"] --> B["Response at Social Level<br>(Expected: Adult → Adult)"]
    C["Stimulus: Psychological Level<br>(Adult → Child)"] -.-> D["Response at Psychological Level<br>(Child reacts)"]
    D --> E["Actual Outcome:<br>Determined by hidden level"]
...
```mermaid
flowchart LR
    A["Withdrawal<br>(Zero risk)"] --> B["Rituals<br>('How are you?')"]
    B --> C["Pastimes<br>(Small talk)"]
    C --> D["Activities<br>(Working together)"]
    D --> E["Games<br>(Hidden agendas)"]
    E --> F["Intimacy<br>(Game-free)"]
...
```mermaid
flowchart LR
    A["Con<br>(The hook)"] --> B["Gimmick<br>(Weakness<br>targeted)"]
    B --> C["Response<br>(Mark engages)"]
    C --> D["Switch<br>(Reveal)"]
    D --> E["Crossup<br>(Confusion)"]
    E --> F["Payoff<br>(Bad feeling<br>collected)"]
...
```mermaid
flowchart TD
    A["Player presents problem"] --> B{"Others offer solutions"}
    B --> C["'Yes, but...'"]
    C --> B
    B --> D["Advisors fall silent"]
    D --> E["Payoff: 'See?<br>Nobody can help me'"]
    F["ANTITHESIS:<br>'What do you think<br>you should do?'"] --> G["Game collapses:<br>Child forced into Adult"]
    style E fill:#e74c3c,color:#fff
    style F fill:#27ae60,color:#fff
...
```mermaid
flowchart TD
    A["Family Game<br>Repertoire"] --> B["Child learns<br>games by age 6"]
    B --> C["Adult selects<br>game-compatible<br>partners"]
    C --> D["New family forms<br>with merged<br>game repertoire"]
    D --> E["Children learn<br>new combined<br>repertoire"]
    E --> C
    F["Work Groups"] --> G["Select members<br>for game<br>compatibility"]
    G --> H["Eject members<br>who refuse<br>to play"]
...
```mermaid
flowchart TD
    A["Parental Messages<br>(Ages 2-6)"] --> B["Injunctions<br>('Don't succeed')"]
    A --> C["Attributions<br>('You're the weak one')"]
    A --> D["Permissions<br>('It's okay to try')"]
    B --> E["Script Decision<br>('I am a loser')"]
    C --> E
    D --> F["Counter-Script<br>('Maybe I can win')"]
    E --> G["Game Selection<br>(Games that confirm script)"]
    F --> G
    G --> H["Life Course<br>(Script fulfilled)"]
    style A fill:#2980b9,color:#fff
    style E fill:#e74c3c,color:#fff
...
```mermaid
flowchart TD
    P["Persecutor<br>'It's your fault'"] --> V["Victim<br>'Poor me'"]
    V --> R["Rescuer<br>'Let me help'"]
    R --> P
    P -.->|"Collects enough<br>stamps"| V
    V -.->|"Explodes"| P
    R -.->|"Help rejected"| V
...
```mermaid
flowchart TD
    A["Parental Messages<br>(Childhood)"] --> B["Life Script<br>(Winner / Loser / Non-Winner)"]
    B --> C["Game Selection<br>(Games that confirm script)"]
    C --> D["Stroke Collection<br>(Familiar bad feelings)"]
    D --> E["Stamp Book Full"]
    E --> F["Cash In<br>(Blowup / Divorce / Breakdown)"]
    F --> G["Script Confirmed<br>('See? I was right all along')"]
    G --> C
    H["AUTONOMY<br>(Awareness + Spontaneity<br>+ Intimacy)"] --> I["Break the Cycle"]
    I --> J["Game-Free Living"]
    style A fill:#2980b9,color:#fff
