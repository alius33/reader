...
date: 2026-03-25
type: book-summary
tags: [book-summary, persuasion, pre-suasion, psychology, attention, priming, influence, cialdini, behavioural-science, social-psychology]
...
```mermaid
flowchart LR
    A["Pre-Suasive Opener"] --> B["Channels Attention"]
    B --> C["Activates Associations"]
    C --> D["Creates Privileged Moment"]
    D --> E["Message Delivered"]
    E --> F["Higher Compliance"]
...
```mermaid
flowchart TD
    A["Part 1: Attention<br>(Chapters 1-6)"] --> D["How to direct<br>and hold focus"]
    B["Part 2: Association<br>(Chapters 7-9)"] --> E["What to activate<br>in the mind"]
    C["Part 3: Optimisation<br>(Chapters 10-14)"] --> F["How to deploy<br>and anchor changes"]
    D --> G["The Complete<br>Pre-Suasion System"]
    E --> G
    F --> G
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A["Traditional Persuasion"] --> B["Changes what<br>people THINK"]
    C["Pre-Suasion"] --> D["Changes what people<br>THINK ABOUT"]
    B --> E["Requires strong arguments<br>Triggers counter-arguing"]
    D --> F["Requires only attention direction<br>Bypasses defences"]
...
```mermaid
flowchart TD
    A["Single-Chute Question Asked"] --> B["One-Directional Memory Search"]
    B --> C["Confirmatory Evidence Found"]
    C --> D["Temporary Self-Assessment Shifts"]
    D --> E["Privileged Moment Created"]
    E --> F["Request Delivered → Higher Compliance"]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart LR
    A["Attractors<br>(The Sexual, Threatening, Different)"] -->|"Seize attention"| B["Initial Focus"]
    B --> C["Magnetizers<br>(Self-Relevant, Unfinished, Mysterious)"]
    C -->|"Hold attention"| D["Sustained Engagement"]
    D --> E["Message Absorption"]
    style A fill:#e74c3c,color:#fff
...
```mermaid
flowchart TD
    A["Physical Environment"] --> B["Visual Cues Activated"]
    A --> C["Auditory Cues Activated"]
    A --> D["Tactile Cues Activated"]
    B --> E["Associative Networks Primed"]
    C --> E
    D --> E
    E --> F["Thinking Shaped<br>Without Awareness"]
    style A fill:#2980b9,color:#fff
    style F fill:#27ae60,color:#fff
```
...
```mermaid
flowchart TD
    A["Set a Goal"] --> B["Form If/When-Then Plan"]
    B --> C["Heightened Vigilance<br>for the CUE"]
    C --> D["Cue Detected<br>in Environment"]
    D --> E["Automatic Launch<br>of Planned ACTION"]
    E --> F["Goal Progress Without<br>Willpower Drain"]
    style A fill:#2980b9,color:#fff
    style D fill:#f9d77e,stroke:#d4a017
    style F fill:#27ae60,color:#fff
```
...
```mermaid
flowchart TD
    A["Unity"] --> B["Being Together<br>(Shared Categories)"]
    A --> C["Acting Together<br>(Shared Experiences)"]
    B --> D["Genetic Relatedness"]
    B --> E["Geographic Commonality"]
    B --> F["Shared Group Membership"]
    C --> G["Synchrony<br>(Moving Together)"]
    C --> H["Collaboration<br>(Working Together)"]
    C --> I["Co-Creation<br>(Making Together)"]
    style A fill:#2980b9,color:#fff
    style B fill:#f9d77e,stroke:#d4a017
...
```mermaid
flowchart LR
    A["Pre-Suasive Opener"] --> B["Privileged Moment<br>(Temporary)"]
    B --> C["Commitment Obtained<br>(Active, Public, Effortful)"]
    C --> D["Self-Image Shift<br>(Durable)"]
    D --> E["Lasting Behaviour Change"]
    style A fill:#2980b9,color:#fff
    style B fill:#f9d77e,stroke:#d4a017
...
```mermaid
flowchart TD
    A["1. Choose Your Association<br>(What should be prominent?)"] --> B["2. Select Pre-Suasive Opener<br>(Attractor + Magnetizer)"]
    B --> C["3. Channel Attention<br>(Direct focus to desired concept)"]
    C --> D["4. Deliver Message<br>(Inside the Privileged Moment)"]
    D --> E["5. Obtain Commitment<br>(Active, Public, Effortful)"]
    E --> F["6. Bridge to Action<br>(If/When-Then Plan)"]
    F --> G["7. Durable Change Achieved"]
    style A fill:#2980b9,color:#fff
    style D fill:#f9d77e,stroke:#d4a017
