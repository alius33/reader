...
date: 2026-03-25
type: book-summary
tags: [book-summary, corporate-politics, HR, promotion, career-protection, perception-management, layoffs]
author: Cynthia Shapiro
title: "Corporate Confidential: 50 Secrets Your Company Doesn't Want You to Know"
year: 2005
---
...
```mermaid
flowchart TD
    A[Employee Behaviour] --> B{Perceived Alignment?}
    B -->|Yes| C[Indispensable List]
    B -->|No| D[Invisible List]
    C --> E[Protected in Layoffs]
    C --> F[Fast-tracked for Promotion]
    C --> G[Premium Assignments]
    D --> H[First to Be Cut]
    D --> I[Passed Over]
    D --> J[Managed Out]
...
```mermaid
flowchart LR
    A[Visible Behaviours] --> B[Appearance & Dress]
    A --> C[Email & Communication]
    A --> D[Meeting Conduct]
    A --> E[Workspace Organisation]
    B --> F[Perception by Decision-Makers]
    C --> F
    D --> F
    E --> F
    F --> G{Aligned or Risky?}
    G -->|Aligned| H[Indispensable]
    G -->|Risky| I[Invisible / Managed Out]
...
```mermaid
flowchart TD
    A[Opportunity Offered] --> B{Visible to Decision-Makers?}
    B -->|Yes| C[Say Yes]
    B -->|No| D{Builds Network or Skills?}
    D -->|Yes| C
    D -->|No| E[Deprioritise Quietly]
    C --> F[Increased Visibility]
    C --> G[Expanded Network]
    F --> H[Perceived as Ready for More]
    G --> H
    E --> I[Protected Time for Core Work]
    style C fill:#27ae60,color:#fff
...
```mermaid
flowchart LR
    A[Negative Perception] --> B[Deliberate Behavioural Change]
    B --> C[3 Months Consistent Positivity]
    C --> D[Manager Attributes to Own Leadership]
    C --> E[Recency Bias Overwrites History]
    D --> F[Perception Reset]
    E --> F
    F --> G[Indispensable Status]
    style A fill:#e74c3c,color:#fff
