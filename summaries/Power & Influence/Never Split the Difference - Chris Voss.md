...
date: 2026-03-25
type: book-summary
tags: [book-summary, negotiation, influence, empathy, communication, behavioural-economics]
author: Chris Voss
...
```mermaid
flowchart LR
    S1["System 1<br/>(Fast, emotional)"] -->|"where decisions<br/>are actually made"| D["Decision"]
    D -->|"rationalised by"| S2["System 2<br/>(Slow, logical)"]
    V["Voss's toolkit"] -->|"targets"| S1
    GTY["Getting to Yes"] -->|"targets"| S2
    style S1 fill:#e8d44d,stroke:#333
    style V fill:#5b9bd5,stroke:#333
```
...
```mermaid
flowchart LR
    A["Active<br/>Listening"] --> B["Empathy"]
    B --> C["Rapport"]
    C --> D["Influence"]
    D --> E["Behavioural<br/>Change"]
    style A fill:#5b9bd5,stroke:#333
    style E fill:#70ad47,stroke:#333
```
...
```mermaid
quadrantChart
    title Voss's Negotiation Tools — Effort vs Impact
    x-axis Low Effort --> High Effort
    y-axis Low Impact --> High Impact
    quadrant-1 High-Value Investments
    quadrant-2 Quick Wins
    quadrant-3 Low Priority
    quadrant-4 Grind Work
    Mirroring: [0.2, 0.75]
    Labelling: [0.3, 0.85]
    Calibrated Questions: [0.55, 0.9]
    Accusation Audit: [0.6, 0.8]
    Ackerman System: [0.8, 0.7]
    Late-Night DJ Voice: [0.15, 0.65]
    Black Swan Hunt: [0.85, 0.95]
```
...
```mermaid
flowchart TD
    V["Voice Tone<br/>(38% of impact)"] --> DJ["Late-Night DJ<br/>Calm, safe, authoritative"]
    V --> PP["Positive/Playful<br/>Warm, open, default"]
    V --> AS["Assertive<br/>Direct, forceful"]
    DJ -->|"Use for"| DU["De-escalation,<br/>boundaries,<br/>difficult news"]
    PP -->|"Use for"| PU["Most conversations,<br/>rapport building"]
    AS -->|"Use for"| AU["Last resort only"]
    style DJ fill:#70ad47,stroke:#333
    style AS fill:#c0504d,stroke:#333
```
...
```mermaid
flowchart TD
    Q["Question Design"] --> YO["'Yes'-Oriented"]
    Q --> NO["'No'-Oriented"]
    YO --> PR["Pressure, reactance,<br/>counterfeit agreement"]
    NO --> SF["Safety, autonomy,<br/>genuine engagement"]
    SF --> RO["Real openness<br/>to your proposal"]
    PR --> RS["Resistance or<br/>empty compliance"]
    style NO fill:#70ad47,stroke:#333
    style YO fill:#c0504d,stroke:#333
```
...
```mermaid
flowchart TD
    AG["Agreement Reached"] --> Q1["'How will we<br/>make sure this<br/>gets done?'"]
    Q1 --> Q2["'How will we<br/>know we're<br/>on track?'"]
    Q2 --> Q3["'How do we handle<br/>it if we're<br/>off track?'"]
    Q3 --> R3["Rule of Three<br/>(test commitment)"]
    R3 --> BTP["Surface behind-<br/>the-table players"]
    BTP --> EX["Genuine<br/>Execution"]
    style AG fill:#e8d44d,stroke:#333
    style EX fill:#70ad47,stroke:#333
```
...
```mermaid
flowchart LR
    T["Target: $5,000"] --> O1["Offer 1: $3,250<br/>(65%)"]
    O1 -->|"empathy +<br/>calibrated Qs"| O2["Offer 2: $4,250<br/>(85%)"]
    O2 -->|"empathy +<br/>calibrated Qs"| O3["Offer 3: $4,751<br/>(95%, precise)"]
    O3 -->|"+ non-monetary<br/>item"| DEAL["Deal: $4,751<br/>+ stereo"]
    style T fill:#5b9bd5,stroke:#333
    style DEAL fill:#70ad47,stroke:#333
```
...
```mermaid
flowchart TD
    TE["Tactical Empathy<br/>(emotional foundation)"] --> M["Mirroring"]
    TE --> L["Labelling"]
    TE --> AA["Accusation Audit"]
    M --> CQ["Calibrated Questions<br/>('How' and 'What')"]
    L --> CQ
    AA --> CQ
    CQ --> TR["'That's Right'<br/>(genuine buy-in)"]
    TR --> AK["Ackerman System<br/>(numbers)"]
    AK --> BS["Black Swan Discovery<br/>(unknown unknowns)"]
    style TE fill:#5b9bd5,stroke:#333
    style AK fill:#e8d44d,stroke:#333
    style BS fill:#70ad47,stroke:#333
```
...
```mermaid
flowchart TD
    BS["Black Swan<br/>Discovery"] --> PL["Positive Leverage<br/>(you have what<br/>they want)"]
    BS --> NL["Negative Leverage<br/>(you can cause<br/>them pain)"]
    BS --> NR["Normative Leverage<br/>(their own standards<br/>against them)"]
    PL --> OUT["Transformed<br/>Negotiation"]
    NL --> OUT
    NR --> OUT
    style BS fill:#5b9bd5,stroke:#333
    style OUT fill:#70ad47,stroke:#333
```
