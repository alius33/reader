...
date: 2026-03-25
type: book-summary
tags: [book-summary, persuasion, influence, science, social-psychology, practical, compliance, taxonomy]
...
```mermaid
flowchart TD
    A["Persuasion Challenge"] --> B{"Approach?"}
    B -->|"Intuition-based"| C["Misses hidden drivers<br>of behaviour"]
    B -->|"Science-based"| D["Identifies precise<br>levers of influence"]
    C --> E["Mediocre results"]
    D --> F["Dramatically better<br>results at no extra cost"]
...
```mermaid
quadrantChart
    title Yes! Strategies — Ease of Implementation vs Impact
    ...
    Post-it Notes: [0.9, 0.9]
    Hotel Towel Signs: [0.85, 0.7]
    Labelling: [0.7, 0.75]
    Fear Plus Action: [0.4, 0.8]
    Benjamin Franklin: [0.5, 0.65]
    Precise Numbers: [0.8, 0.6]
    Genuine Dissent: [0.3, 0.85]
    Goal Gradient: [0.6, 0.7]
```
...
```mermaid
flowchart TD
    subgraph SP["Social Proof<br>(Chapters 1-4)"]
        A1["Implied demand"]
        A2["Matched reference group"]
        A3["Avoid negative norms"]
        A4["Approval signals"]
    end
    subgraph RC["Reciprocity<br>(Chapters 6, 9-12)"]
        B1["Give first"]
        B2["Personalise"]
        B3["Make it unexpected"]
        B4["Cash in early"]
    end
    subgraph CM["Commitment<br>(Chapters 13-18, 20)"]
        C1["Start small"]
        C2["Get it in writing"]
        C3["Label the identity"]
        C4["Pre-question trap"]
    end
    subgraph AU["Authority<br>(Chapters 22, 26-28)"]
        D1["Third-party intro"]
        D2["Lead with weakness"]
        D3["Internal attribution"]
    end
    subgraph LK["Liking<br>(Chapters 29-33)"]
        E1["Name similarity"]
        E2["Mirroring"]
        E3["Genuine warmth"]
    end
    subgraph FL["Fluency<br>(Chapters 35-38)"]
        F1["Because"]
        F2["Rhyme"]
        F3["Easy names"]
        F4["Simple language"]
...
```mermaid
flowchart LR
    A["Descriptive Norm<br>(what people do)"] --> B{"Is the person<br>above or below?"}
    B -->|Below average| C["Pulls UP<br>toward average<br>(good)"]
    B -->|Above average| D["Pulls DOWN<br>toward average<br>(bad)"]
    D --> E["Add injunctive<br>norm signal"]
    E --> F["Maintains good<br>behaviour"]
...
```mermaid
flowchart LR
    subgraph receiver ["Favour RECEIVER"]
        R1["Day 1: High value"] --> R2["Month 1: Moderate"] --> R3["Year 1: 'Not that big'"]
    end
    subgraph giver ["Favour GIVER"]
        G1["Day 1: Low value"] --> G2["Month 1: Moderate"] --> G3["Year 1: 'I really helped'"]
    end
...
```mermaid
flowchart LR
    subgraph before ["Before: 1 Product"]
        A["Bread Maker<br>$275"] --> B{"Buy or don't buy?"}
        B --> C["Difficult decision<br>Moderate sales"]
    end
    subgraph after ["After: 2 Products"]
        D["Original<br>$275"] --> F{"Which one?"}
        E["Premium<br>$429"] --> F
        F --> G["$275 is the smart choice<br>Sales nearly double"]
    end
```
...
```mermaid
flowchart LR
    A["Same information"] --> B{"How is it framed?"}
    B -->|"Gain frame"| C["Moderate motivation<br>'You could save $500'"]
    B -->|"Loss frame"| D["Strong motivation<br>'You could lose $500'"]
...
```mermaid
flowchart TD
    A["Information<br>Encountered"] --> B{"How easy<br>to process?"}
    B -->|"Easy<br>(fluent)"| C["Positive feeling"]
    B -->|"Hard<br>(disfluent)"| D["Negative feeling"]
    C --> E["Attributed to content:<br>True, valuable,<br>competent"]
    D --> F["Attributed to content:<br>Suspicious, low quality,<br>incompetent"]
    style C fill:#27ae60,color:#fff
    style D fill:#e74c3c,color:#fff
...
```mermaid
flowchart TD
    A["Encounter a claim"] --> B["Stage 1: Comprehension<br>(Automatic belief)"]
    B --> C{"Cognitive<br>resources<br>available?"}
    C -->|"Yes"| D["Stage 2: Assessment<br>(Evaluate truth)"]
    C -->|"No (tired,<br>distracted)"| E["Claim stays believed<br>(no filter)"]
    D --> F["Accept or reject<br>based on evidence"]
...
```mermaid
flowchart TD
    A["Same Persuasion<br>Principles"] --> B["Individualist<br>Culture"]
    A --> C["Collectivist<br>Culture"]
    A --> D["Relationship<br>Culture"]
    B --> E["Lead with<br>Consistency"]
    C --> F["Lead with<br>Social Proof"]
    D --> G["Lead with<br>Liking"]
...
```mermaid
flowchart TD
    A["What is your<br>persuasion challenge?"] --> B{"Is your audience<br>uncertain?"}
    B -->|Yes| C["Use SOCIAL PROOF<br>(Show what others do)"]
    B -->|No| D{"Do you need<br>long-term compliance?"}
    D -->|Yes| E["Use COMMITMENT<br>(Start small, write it down)"]
    D -->|No| F{"Is urgency needed?"}
    F -->|Yes| G["Use SCARCITY<br>(Frame as loss)"]
    F -->|No| H{"Do you need<br>credibility?"}
    H -->|Yes| I["Use AUTHORITY<br>(Third-party intro)"]
    H -->|No| J["Use RECIPROCITY<br>(Give something first)"]
    style C fill:#2980b9,color:#fff
    style E fill:#27ae60,color:#fff
    style G fill:#e74c3c,color:#fff
    style I fill:#f9d77e,stroke:#d4a017
