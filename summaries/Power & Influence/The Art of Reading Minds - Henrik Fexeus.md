...
date: 2026-03-25
type: book-summary
tags: [book-summary, mind-reading, body-language, rapport, NLP, influence, nonverbal-communication, emotions, micro-expressions, anchoring, suggestion, sensory-acuity]
...
```mermaid
flowchart TD
    A["Person A thinks/feels something"] --> B["Physical change occurs<br>(face, body, voice, breathing)"]
    B --> C["Person B's unconscious<br>detects the signal"]
    C --> D["Person B's unconscious<br>sends a response signal"]
    D --> E["Person A's unconscious<br>detects the response"]
    E --> F["The conversation continues<br>on two levels simultaneously"]
    G["CONSCIOUS LEVEL:<br>Words, chosen gestures"] -.-> F
    H["UNCONSCIOUS LEVEL:<br>Micro-expressions, breathing,<br>pupils, posture, tone"] -.-> F
...
```mermaid
flowchart LR
    A["OBSERVE<br>(Notice their state)"] --> B["PACE<br>(Match their body language,<br>breathing, rhythm)"]
    B --> C["RAPPORT ESTABLISHED<br>(Unconscious connection)"]
    C --> D["LEAD<br>(Change YOUR state;<br>they follow)"]
    D --> E["Influence achieved<br>without control"]
...
flowchart TD
    subgraph Visual
    A["Up-Left<br>(Visual Recall)"]
    B["Up-Right<br>(Visual Construct)"]
    end
    subgraph Auditory
    C["Sideways-Left<br>(Auditory Recall)"]
    D["Sideways-Right<br>(Auditory Construct)"]
    end
    subgraph Internal
    E["Down-Left<br>(Internal Dialogue)"]
    F["Down-Right<br>(Kinesthetic / Feelings)"]
    end
...
```mermaid
flowchart LR
    A["Brief Eye<br>Contact"] --> B["Repeated Eye<br>Contact + Smile"]
    B --> C["Preening<br>Behaviours"]
    C --> D["Proximity<br>Reduction"]
    D --> E["Brief Touch"]
    E --> F["Synchronisation"]
    style A fill:#e74c3c,color:#fff
    style F fill:#27ae60,color:#fff
...
```mermaid
flowchart TD
    A["Emotional state<br>builds naturally"] --> B["Peak intensity<br>reached"]
    B --> C["Deliver stimulus<br>(touch, word, gesture)"]
    C --> D["Association formed:<br>stimulus = emotion"]
    D --> E["Later: repeat<br>the exact stimulus"]
    E --> F["Emotional state<br>returns automatically"]
