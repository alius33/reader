...
date: 2026-03-25
type: book-summary
tags: [book-summary, rapport, FBI, social-skills, influence, body-language, friendship, nonverbal-communication, persuasion]
author: Jack Schafer
...
```mermaid
flowchart LR
    A["Proximity<br>(Be near them)"] --> B["Frequency<br>(Be seen often)"]
    B --> C["Duration<br>(Spend more time)"]
    C --> D["Intensity<br>(Deepen connection)"]
    D --> E["FRIENDSHIP"]
...
```mermaid
flowchart TD
    A["Person Detected"] --> B{"Brain Assessment<br>(< 3 seconds)"}
    B -->|"Friend signals"| C["Dopamine release<br>Approach response"]
    B -->|"Foe signals"| D["Cortisol release<br>Fight-or-flight"]
    B -->|"No signals"| E["Ignored<br>Neutral processing"]
    C --> F["Guard drops<br>Open to interaction"]
    D --> G["Guard up<br>Resistant to interaction"]
    E --> H["No engagement<br>Background noise"]
...
```mermaid
flowchart TD
    A["Laws of Attraction"] --> B["Similarity"]
    A --> C["Reciprocity"]
    A --> D["Misattribution"]
    A --> E["Curiosity"]
    A --> F["Scarcity"]
    B --> G["We like people<br>who are like us"]
    C --> H["We like people<br>who like us"]
    D --> I["We credit arousal<br>to nearby people"]
    E --> J["We're drawn to<br>what intrigues us"]
    F --> K["We value<br>what's rare"]
    style A fill:#2980b9,color:#fff
```
...
```mermaid
flowchart TD
    A["Similarity<br>'Like me'"] --- B["Reciprocity<br>'Likes me'"]
    B --- C["Misattribution<br>'Exciting to be around'"]
    C --- D["Curiosity<br>'Want to know more'"]
    D --- E["Scarcity<br>'Hard to get'"]
    E --- A
    F["Combined Effect:<br>Strong Attraction"]
    A --> F
    B --> F
    C --> F
    D --> F
...
```mermaid
flowchart TD
    A["Surface level<br>(Weather, sports, news)"] --> B["Opinion level<br>(Views on topics)"]
    B --> C["Personal level<br>(Experiences, feelings)"]
    C --> D["Intimate level<br>(Fears, dreams, vulnerabilities)"]
    A -.->|"Each level requires<br>reciprocal matching"| A
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart LR
    A["Establish Baseline<br>(Easy, truthful questions)"] --> B["Observe Normal Behaviour<br>(Posture, eyes, speech)"]
    B --> C["Ask Critical Questions"]
    C --> D{"Behaviour Changes?"}
    D -->|"Yes — cluster of<br>foe signals"| E["Potential Deception<br>or Discomfort"]
    D -->|"No — consistent<br>with baseline"| F["Likely Truthful<br>or Comfortable"]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A["Send Friend Signals<br>(Eyebrow flash, head tilt, smile)"] --> B["Establish Proximity<br>(Be in their space, non-threateningly)"]
    B --> C["Increase Frequency<br>(Be seen regularly)"]
    C --> D["Extend Duration<br>(Spend more time)"]
    D --> E["Deepen Intensity<br>(Empathic statements, self-disclosure)"]
    E --> F["Apply Golden Rule<br>(Make them feel good about themselves)"]
    F --> G["Activate Laws of Attraction<br>(Similarity, reciprocity)"]
    G --> H["Build Closeness<br>(Matched self-disclosure)"]
    H --> I["Maintain the Formula<br>(Continuous investment)"]
    style A fill:#2980b9,color:#fff
    style F fill:#27ae60,color:#fff
