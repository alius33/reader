...
date: 2026-03-25
type: book-summary
tags: [book-summary, executive-presence, leadership, gravitas, communication, appearance]
...
```mermaid
flowchart LR
    A[Appearance] -->|"Pass the filter"| B[Communication]
    B -->|"Deliver the signal"| C[Gravitas]
    C -->|"Earn trust"| D[Leadership Perception]
...
```mermaid
flowchart TD
    A[Grace Under Fire] --> B[Decisiveness]
    B --> C[Speaking Truth to Power]
    C --> D[Emotional Intelligence]
    D --> E[Reputation Management]
    E --> F[Vision & Charisma]
...
```mermaid
flowchart TD
    A[Superior Speaking Skills] --> B[Ditching the Props]
    B --> C[Silence as a Power Tool]
    C --> D[Reading the Room]
    D --> E[Assertiveness]
    E --> F[Body Language]
    F --> G[Humour & Banter]
    style A fill:#27ae60,stroke:#333,color:#fff
...
```mermaid
flowchart LR
    A[Conform to norms] --> B[Deliver results]
    B --> C[Build trust & credibility]
    C --> D[Earn latitude]
    D --> E[Assert authenticity]
...
```mermaid
flowchart TD
    A[Professional accommodation required] --> B{Does it touch core identity?}
    B -->|No| C[Strategic accommodation]
    C --> D[Adapt style & delivery]
    B -->|Yes| E[Identity erasure]
    E --> F{Can you change the environment?}
    F -->|Yes| G[Spend credibility to push back]
    F -->|No| H[Walk away]
