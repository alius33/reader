...
date: 2026-03-25
type: book-summary
tags: [book-summary, networking, relationships, introversion, referrals, communication]
author: Charlie Lawson
title: "The Unnatural Networker"
year: 2014
---
...
- He describes the feeling with visceral honesty, establishing early that his audience is the professional who knows networking matters, suspects they are bad at it, and assumes the problem is their personality
...
```mermaid
flowchart LR
    A[Start Networking] --> B[Long Flat Phase]
    B --> C[Trust Threshold]
    C --> D[Sharp Acceleration]
    D --> E[Referrals Flow]
...
> **The lesson:** The Creator does what the contact cannot do for themselves — and the way to move people up the taxonomy is to create for them first.
...
- Lawson cites BNI's data: the average BNI member generates £48,000 in referral business per year through their group
...
> - Everyone in the room wondered: "If I become his client, will he point at me next?"
...
```mermaid
flowchart TD
    A[Best Current Client] --> B[Their Industry]
    B --> C[Similar Company]
    C --> D[Relevant Role]
    D --> E[Named Person]
    style A fill:#3498db,color:#fff
...
```mermaid
flowchart LR
    A[Online Networking] -->|Provides| B[Width / Visibility]
    C[Face-to-Face Networking] -->|Provides| D[Depth / Credibility]
    B --> E[Combine Both]
    D --> E
...
```mermaid
flowchart TD
    A[Everyone You Meet] --> B{Triage}
    B -->|High value| C[Big Fish]
    B -->|Useful but not transformative| D[Little Fish]
    B -->|Time-wasters| E[Wellington Boots]
    C --> F[One-to-ones, Creator effort, priority follow-up]
    D --> G[Maintain, stay in touch, help when opportunities arise]
    E --> H[Exit quickly and gracefully]
    style C fill:#27ae60,color:#fff
