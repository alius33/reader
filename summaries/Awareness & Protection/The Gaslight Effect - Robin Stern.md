...
date: 2026-03-25
type: book-summary
tags: [book-summary, gaslighting, manipulation, emotional-abuse, relationships, psychology, recovery, boundaries, self-trust]
author: Robin Stern
...
```mermaid
flowchart TD
    A["Stage 1: DISBELIEF<br>'Did that really happen?'<br>'Am I overreacting?'"] --> B["Stage 2: DEFENCE<br>'I need to prove I'm right'<br>'If I just explain clearly enough...'"]
    B --> C["Stage 3: DEPRESSION<br>'Maybe I really am the problem'<br>'I can't trust my own mind'"]
    C --> D["THE GASLIGHT TANGO<br>Gaslighter initiates<br>Victim engages<br>Cycle deepens"]
    D -->|"Each cycle<br>erodes more<br>self-trust"| A
...
```mermaid
flowchart LR
    A["Gaslightee notices<br>distortion"] --> B["Gathers evidence<br>to prove it"]
    B --> C["Presents evidence<br>to gaslighter"]
    C --> D{"Gaslighter responds"}
    D --> E["'You're obsessive'"]
    D --> F["'That's not what happened'"]
    D --> G["'Everyone agrees with me'"]
    E --> H["Gaslightee's evidence<br>becomes 'proof of instability'"]
    F --> H
    G --> H
    H --> I["Gaslightee gathers<br>MORE evidence"]
    I --> C
    style A fill:#f39c12,color:#fff
...
```mermaid
flowchart LR
    subgraph PO["POWER-OVER"]
        A1["Gaslighter defines reality"] --> B1["Gaslightee must agree"]
        B1 --> C1["Disagreement = threat"]
        C1 --> D1["Escalation: 'You're wrong/crazy/sensitive'"]
    end
    subgraph PW["POWER-WITH"]
        A2["Both define reality together"] --> B2["Disagreement is normal"]
        B2 --> C2["Both perspectives honoured"]
        C2 --> D2["Resolution through dialogue"]
    end
...
```mermaid
flowchart TD
    A["Boss gives verbal instruction"] --> B["Employee follows instruction"]
    B --> C["Boss denies giving instruction"]
    C --> D{"Employee's response"}
    D --> E["Accepts boss's version<br>→ Self-doubt grows"]
    D --> F["Documents conversations<br>→ 'Why are you so adversarial?'"]
    D --> G["Pushes back verbally<br>→ 'You're confused/emotional'"]
    E --> H["Pattern continues<br>Employee loses confidence"]
    F --> H
    G --> H
    style C fill:#e74c3c,color:#fff
...
```mermaid
flowchart TD
    A["Notice your<br>own feelings"] --> B["Validate them<br>internally"]
    B --> C["Make a small<br>decision based<br>on YOUR preference"]
    C --> D["Notice: the world<br>did not end"]
    D --> E["Self-trust<br>grows slightly"]
    E --> F["Make a slightly<br>larger decision"]
    F --> D
...
```mermaid
flowchart TD
    A["THE GASLIGHT EFFECT"] --> B["Three Stages"]
    A --> C["The Gaslight Tango"]
    A --> D["Three Types of Gaslighters"]
    A --> E["Recovery Framework"]
    ...
    C --> C1["Urge to Merge"]
    C --> C2["Power-Over vs Power-With"]
    C --> C3["Explanation Trap"]

    D --> D1["Glamour Gaslighter"]
    D --> D2["Good-Guy Gaslighter"]
    D --> D3["Intimidator"]

    E --> E1["Opt Out of the Argument"]
    E --> E2["Build Reality Checks"]
    E --> E3["Flight Attendant Self"]
    E --> E4["Rebuild Self-Trust"]
