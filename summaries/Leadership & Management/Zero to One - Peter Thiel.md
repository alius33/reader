...
date: 2026-03-25
type: book-summary
tags: [book-summary, startups, innovation, contrarian-thinking, monopoly, strategy, technology, venture-capital]
author: Peter Thiel
title: "Zero to One"
year: 2014
---
...
```mermaid
flowchart LR
    A["0 to 1<br>(Create something new)<br>Technology<br>Monopoly"] ---|"vs"| B["1 to n<br>(Copy what works)<br>Globalisation<br>Competition"]
...
```mermaid
flowchart TD
    subgraph MONO ["Monopolists"]
        M1["Describe huge market<br>'We're a tech company'"]
        M2["Hide dominance<br>'So much competition'"]
    end
    subgraph COMP ["Competitors"]
        C1["Describe tiny niche<br>'Only one of its kind'"]
        C2["Exaggerate uniqueness<br>'Nobody else does this'"]
    end
    M1 --> R1["Reality: 90%+ share<br>of a defined market"]
    C1 --> R2["Reality: undifferentiated<br>in a huge market"]
...
```mermaid
flowchart TD
    subgraph MONO ["Monopoly Characteristics"]
        A["Proprietary Technology<br>(10x improvement)"]
        B["Network Effects<br>(value grows with users)"]
        C["Economies of Scale<br>(near-zero marginal cost)"]
        D["Branding<br>(earned, not manufactured)"]
    end
    A --> E["Durable Competitive<br>Advantage"]
    B --> E
    C --> E
    D --> E
    E --> F["Monopoly Profits<br>+ Resources to Innovate"]
...
```mermaid
flowchart LR
    A["Viral<br>< $100<br>(Free/cheap)"] --> B["DEAD ZONE<br>$100 - $1K<br>(Too cheap for sales,<br>too expensive for viral)"]
    B --> C["Sales Team<br>$1K+<br>(Justified by CLV)"]
    style A fill:#27ae60,color:#fff
    style B fill:#e74c3c,color:#fff
...
```mermaid
flowchart LR
    A["Machine Processing<br>(speed, scale, patterns)"] --> C["Human-Machine<br>Collaboration"]
    B["Human Judgment<br>(context, meaning, ethics)"] --> C
    C --> D["Outcomes neither<br>could achieve alone"]
...
```mermaid
flowchart TD
    Q["Ask: What truth do few<br>people agree with me on?"] --> S["Find a SECRET"]
    S --> SMALL["Start with a SMALL MARKET<br>you can dominate"]
    SMALL --> TECH["Build 10x TECHNOLOGY<br>(not 10% better)"]
    TECH --> MONO["Create a MONOPOLY<br>(no real competition)"]
    MONO --> DIST["Build DISTRIBUTION<br>(sales matters as much as product)"]
    DIST --> SCALE["SCALE to adjacent markets"]
    SCALE --> POWER["Capture POWER LAW returns<br>(one great bet outweighs all others)"]

    style Q fill:#8e44ad,color:#fff
    style S fill:#e74c3c,color:#fff
    style SMALL fill:#e67e22,color:#fff
    style TECH fill:#f39c12,color:#fff
    style MONO fill:#2980b9,color:#fff
    style DIST fill:#27ae60,color:#fff
    style SCALE fill:#1abc9c,color:#fff
    style POWER fill:#2c3e50,color:#fff
```
