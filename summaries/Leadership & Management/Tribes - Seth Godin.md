...
date: 2026-03-25
type: book-summary
tags: [book-summary, leadership, tribes, movement-building, change, heresy, initiative]
author: Seth Godin
...
```mermaid
flowchart LR
    L[Leader] -->|broadcasts| T[Tribe Members]
    T -->|feedback| L
    T -->|strongest bond| T2[Other Members]
    T -->|evangelism| O[Outsiders]
...
```mermaid
flowchart TD
    A[Desire to Lead] --> B{Fear of Criticism?}
    B -->|Yes| C[Imagined Worst Case]
    C --> D[Compliance Mode]
    D --> E[Skills Atrophy]
    E --> F[Gap Feels Unbridgeable]
    B -->|Act Anyway| G[Lead]
    G --> H[Real Criticism Usually Mild]
    H --> I[Confidence Grows]
    I --> J[Lead Again]
...
```mermaid
flowchart TD
    N[Narrative] --> M((Movement))
    C[Connection] --> M
    T[Something to Do] --> M
    N -.->|without connection| P[Propaganda]
    C -.->|without tasks| SC[Social Club]
    T -.->|without narrative| W[Just Work]
...
```mermaid
flowchart TB
    A[Crowd — vaguely aware, uninvolved] --> B[Seekers — interested, sympathetic, uncommitted]
    B --> C[Members — joined and participating]
    C --> D[True Believers — would continue without you]
