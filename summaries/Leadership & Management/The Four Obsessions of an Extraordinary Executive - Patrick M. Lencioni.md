...
date: 2026-03-25
type: book-summary
tags: [book-summary, executive-leadership, organizational-health, team-building, meetings, hiring, communication]
...
```mermaid
flowchart LR
    A[Cohesive Team] --> B[Organisational Clarity]
    B --> C[Over-Communicate]
    C --> D[Reinforce via Systems]
    D -->|feeds back| A
...
```mermaid
flowchart TD
    A[Thematic Goal] --> B[Major Strategic Goal 1]
    A --> C[Major Strategic Goal 2]
    A --> D[Major Strategic Goal 3]
    B --> E[Metrics]
    C --> E
    D --> E
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A[Organisational Clarity] --> B[Hiring]
    A --> C[Performance Reviews]
    A --> D[Rewards & Compensation]
    A --> E[Dismissal]
    B --> F[Culture Reinforced]
    C --> F
    D --> F
    E --> F
    F -->|sustains| A
...
```mermaid
flowchart TD
    A[Executives avoid conflict] --> B[Unresolved disagreements]
    B --> C[Inconsistent messaging]
    C --> D[Departments hear different versions]
    D --> E[Employees take sides]
    E --> F[Politics takes root]
    F --> G[Best people leave]
...
```mermaid
flowchart LR
    A[Rich loses confidence] --> B[Meetings less decisive]
    B --> C[Unclear decisions]
    C --> D[Inconsistent cascading]
    D --> E[Employee confusion]
    E --> F[Politics emerges]
    style A fill:#e74c3c,color:#fff
