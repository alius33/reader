...
date: 2026-03-28
type: book-summary
tags: [book-summary, management, leadership, production, leverage, meetings, decision-making, motivation, performance-reviews, training, intel, silicon-valley, organizational-design, delegation, task-relevant-maturity]
...
```mermaid
flowchart TD
    A["MANAGER'S OUTPUT"] --> B["Output of<br>Own Organization"]
    A --> C["Output of<br>Neighboring Orgs<br>Under Influence"]
    B --> D["Supervision"]
    B --> E["Training"]
    B --> F["Motivation"]
    C --> G["Know-how<br>Sharing"]
    C --> H["Nudging<br>Decisions"]
    C --> I["Role<br>Modeling"]
...
```mermaid
flowchart LR
    A["FREE<br>DISCUSSION"] --> B["CLEAR<br>DECISION"]
    B --> C["FULL<br>SUPPORT"]
    A -.- D["All points of view<br>openly welcomed"]
    B -.- E["Terms framed<br>with utter clarity"]
    C -.- F["Commitment to<br>back the decision<br>(not necessarily agreement)"]
...
```mermaid
flowchart TB
    subgraph HYBRID["HYBRID ORGANIZATION (Grove's Law)"]
        direction LR
        M["MISSION-ORIENTED<br>Business Divisions"] --- F["FUNCTIONAL<br>Service Groups"]
    end
    M --> R["Responsive to<br>local market needs"]
    F --> S["Economies of scale<br>Expertise leverage<br>Resource flexibility"]
...
```mermaid
flowchart LR
    A["Job 1:<br>MEETS"] --> B["Job 1:<br>EXCEEDS"]
    B --> C["PROMOTED<br>to Job 2"]
    C --> D["Job 2:<br>MEETS"]
    D --> E["Job 2:<br>EXCEEDS"]
    E --> F["PROMOTED<br>to Job 3"]
    F --> G["Job 3:<br>MEETS<br>(settles here)"]
    style B fill:#27ae60,color:#fff
...
```mermaid
flowchart TB
    subgraph PROCESS["PROCESS-ORIENTED (Scheduled)"]
        O2O["One-on-Ones<br>Weekly/Biweekly<br>Subordinate's agenda"]
        STAFF["Staff Meetings<br>Weekly<br>Peer interaction"]
        OPREV["Operation Reviews<br>Monthly/Quarterly<br>Cross-org learning"]
    end
    subgraph MISSION["MISSION-ORIENTED (Ad Hoc)"]
        DEC["Decision Meetings<br>As needed<br>Max 8 people"]
    end
    PROCESS --> |"Should handle<br>~80% of issues"| OUTPUT["HIGH OUTPUT"]
    MISSION --> |"Remaining<br>~20%"| OUTPUT
    style O2O fill:#27ae60,color:#fff
    style STAFF fill:#2980b9,color:#fff
    style OPREV fill:#8e44ad,color:#fff
