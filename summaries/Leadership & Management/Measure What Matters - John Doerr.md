...
date: 2026-03-25
type: book-summary
tags: [book-summary, OKRs, metrics, goal-setting, accountability, management, performance]
author: John Doerr
...
```mermaid
flowchart LR
    A["Drucker's MBOs (1954)"] --> B["Grove's iMBOs (1970s)"]
    B --> C["Doerr learns at Intel"]
    C --> D["Doerr introduces to Google (1999)"]
    D --> E["OKRs spread globally"]
...
flowchart TB
    ...
    O --> T[Tracking]
    O --> S[Stretching]
    F --> F1[Ruthless subtraction]
    F --> F2[3-5 Objectives max]
    A --> A1[Public goals]
    A --> A2[50% bottom-up]
    T --> T1[Weekly check-ins]
    T --> T2[Colour-coded progress]
    S --> S1[Aspirational targets]
    S --> S2[10x thinking]
...
```mermaid
flowchart TB
    OKR[OKRs — Mechanical System] --- CFR[CFRs — Human System]
    CFR --> C[Conversations]
    CFR --> F[Feedback]
    CFR --> R[Recognition]
    C --> C1[Manager as resource provider]
    F --> F1[Bidirectional and real-time]
    R --> R1[Peer-driven and specific]
    style OKR fill:#2980b9,color:#fff
    style CFR fill:#27ae60,color:#fff
```
