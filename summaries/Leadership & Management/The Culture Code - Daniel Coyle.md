...
date: 2026-03-25
type: book-summary
tags: [book-summary, culture, teams, leadership, belonging, trust, vulnerability, purpose, group-dynamics, psychological-safety]
author: Daniel Coyle
...
```mermaid
flowchart LR
    A["1. BUILD SAFETY<br>(Belonging cues)"] --> B["2. SHARE VULNERABILITY<br>(Trust through risk)"]
    B --> C["3. ESTABLISH PURPOSE<br>(Shared narratives)"]
    C --> D["Extraordinary<br>Cooperation"]
...
```mermaid
flowchart TD
    A["Psychological Safety Present"] --> B["People speak up"]
    A --> C["Mistakes are admitted"]
    A --> D["Ideas are challenged"]
    B --> E["Better decisions"]
    C --> F["Faster learning"]
    D --> G["More innovation"]
...
```mermaid
flowchart TD
    A["Person A admits weakness<br>or asks for help"] --> B["Person B sees A's<br>vulnerability as signal<br>of trust"]
    B --> C["Person B reciprocates<br>with their own vulnerability"]
    C --> D["TRUST IS ESTABLISHED<br>(Neither person punished<br>for openness)"]
    D --> E["Future vulnerability<br>becomes easier"]
    E --> A
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A["HIGH-PURPOSE<br>ENVIRONMENTS"] --> B["PROFICIENCY<br>Purpose"]
    A --> C["CREATIVITY<br>Purpose"]
    B --> D["Clear, simple<br>priorities"]
    B --> E["Repeated<br>heuristics"]
    B --> F["Consistent<br>behaviours"]
    C --> G["Support for<br>risk-taking"]
    C --> H["Permission<br>to fail"]
    C --> I["Candid<br>feedback loops"]
    style A fill:#2980b9,color:#fff
    style B fill:#e74c3c,color:#fff
    style C fill:#27ae60,color:#fff
```
...
flowchart LR
    ...
    C["ARTEFACTS"] --> D
    E["RITUALS"] --> D
    D --> F["Automatic<br>Decision-Making"]
    D --> G["Aligned<br>Behaviour"]
    D --> H["Resilience<br>Under Stress"]
    style D fill:#27ae60,color:#fff
...
flowchart TD
    subgraph foundation ["FOUNDATION"]
        S["SAFETY<br>(Belonging cues:<br>proximity, eye contact,<br>turn-taking, energy)"]
    end
    subgraph engine ["ENGINE"]
        V["VULNERABILITY<br>(Vulnerability loops:<br>admit weakness, ask for help,<br>leader goes first)"]
    end
    subgraph direction ["DIRECTION"]
        P["PURPOSE<br>(Shared narratives:<br>stories, mottos, artefacts,<br>overcommunicated priorities)"]
    end
    S --> V --> P
    P --> R["EXTRAORDINARY<br>GROUP PERFORMANCE"]
