...
date: 2026-03-25
type: book-summary
tags: [book-summary, power, strategy, mastery, daily-practice, compilation]
...
```mermaid
flowchart LR
    subgraph Inner["Inner Foundation"]
        A[Jan: Purpose] --> B[Feb: Apprenticeship]
        B --> C[Mar: Mastery]
    end
    subgraph Social["Social Battlefield"]
        D[Apr: Courtier] --> E[May: Character]
        E --> F[Jun: Indirection]
    end
    subgraph Offensive["Offensive Capability"]
        G[Jul: Seduction] --> H[Aug: Persuasion]
        H --> I[Sep: Strategy]
    end
    subgraph Inward["Inward Return"]
        J[Oct: Emotions] --> K[Nov: Rationality]
        K --> L[Dec: Sublime]
    end
    C --> D
    F --> G
...
```mermaid
flowchart TD
    A[Read the Target's Psychology] --> B[Mirror Their Values & Style]
    B --> C[Rapport & Trust Form]
    C --> D[Defences Lower]
    D --> E[Calibrated Influence]
    E --> F[Target Feels They Chose Freely]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A[Audit Actual Resources] --> B[Position for Maximum Potential]
    B --> C{Events Unfold}
    C -->|Path A| D[Advantage]
    C -->|Path B| E[Advantage]
    C -->|Path C| F[Advantage]
...
```mermaid
flowchart TD
    A[Power Game] --> B{Is Power Everything?}
    B -->|Machiavellian: Yes| C[Wins but feels hollow]
    B -->|Radical Realist: No| D[Plays skilfully AND sees beyond]
    D --> E[The Sublime]
    E --> F[Awe, Flow, Mortality Awareness]
    F --> G[Game matters AND is not ultimate reality]
