...
date: 2026-03-25
type: book-summary
tags: [book-summary, power, politics, strategy, influence, manipulation, reputation]
...
```mermaid
flowchart TD
    A[Master Your Emotions] --> B[Control Information]
    A --> C[Manage Perception]
    A --> D[Navigate Relationships]
    B --> E[Conceal Intentions]
    B --> F[Say Less]
    C --> G[Guard Reputation]
    C --> H[Create Spectacle]
    D --> I[Build Dependency]
    D --> J[Stay Uncommitted]
    E --> K[Power]
    F --> K
    G --> K
    H --> K
...
```mermaid
flowchart TD
    A[The Relationship Paradox] --> B[Need Allies]
    A --> C[Avoid Dependency]
    B --> D["Law 18: Don't Isolate"]
    B --> E["Law 24: Play the Courtier"]
    C --> F["Law 20: Don't Commit"]
    C --> G["Law 11: Create Dependency"]
    D --> H[Ideal Position]
    E --> H
    F --> H
    G --> H
...
```mermaid
flowchart LR
    A[Assess Situation] --> B{Which time?}
    B -->|Patient cultivation| C[Long Time]
    B -->|Create urgency| D[Forced Time]
    B -->|Moment is ripe| E[End Time]
    C --> F[Build advantage quietly]
    D --> G[Rush opponent's decisions]
    E --> H[Strike decisively]
    style A fill:#2980b9,color:#fff
    style H fill:#27ae60,color:#fff
```
...
```mermaid
flowchart LR
    A[Establish Power] --> B[Preserve Old Forms]
    B --> C[Gradual Changes]
    C --> D[Consolidate Control]
    D --> E[Deeper Reform]
    style A fill:#2980b9,color:#fff
