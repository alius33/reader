...
date: 2026-03-25
type: book-summary
tags: [book-summary, promotion, politics, career-advancement, perception, influence, corporate-politics]
author: Brendan Reid
title: "Stealing the Corner Office"
year: 2014
---
...
```mermaid
flowchart LR
    A[Genuine Talent] --> B{Blind Spot?}
    B -->|Indispensability| C[Go-to Guy]
    B -->|Ownership of Outcomes| D[Passion Player]
    B -->|Enforcing Standards| E[Task Master]
    C --> F[Stuck at Current Level]
    D --> F
    E --> F
```
...
```mermaid
flowchart TD
    A[Incompetent Executive Tactics] --> B[Jack of All Trades]
    A --> C[Mr. Big Picture]
    A --> D[Precision Passive Aggressive]
    B --> E[Breadth signals versatility]
    C --> F[Frameworks signal leadership]
    D --> G[Mentoring signals seniority]
    E --> H[Perceived as promotion-ready]
    F --> H
    G --> H
```
...
```mermaid
flowchart LR
    A[Field Goals] --> B[Career Security]
    A --> C[Invisible to Decision-Makers]
    D[Touchdowns] --> E[Cross-Functional Visibility]
    E --> F[Promotion Decisions]
...
```mermaid
flowchart TD
    A[Results] --> B[Tied to One Manager]
    A --> C[Tied to One Strategy]
    A --> D[Tied to One Era]
    B --> E[Expire When Context Changes]
    C --> E
    D --> E
    F[Skills] --> G[Transfer Across Managers]
    F --> H[Transfer Across Strategies]
    F --> I[Transfer Across Companies]
    G --> J[Compound Over Time]
    H --> J
    I --> J
...
```mermaid
flowchart LR
    A[Identify Target Roles] --> B[Map Influencers]
    B --> C[Declare Intentions]
    C --> D[Close the Gap]
    D --> E[Promotion]
    C -->|Feedback Loop| D
