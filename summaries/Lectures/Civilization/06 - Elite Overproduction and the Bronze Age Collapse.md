...
tags: [lecture-summary, civilization, bronze-age, collapse, elite-overproduction, rent-seeking, mycenaean-greece, hittites, sea-peoples, peter-turchin, piketty, social-contract, debt-slavery, palace-economy]
...
```mermaid
flowchart LR
    B["Britain<br/>Tin"] --> TR["Troy<br/>The Toll Gate"]
    IB["Iberia<br/>Tin"] --> TR
    AF["Afghanistan<br/>Tin"] --> TR
    AN["Anatolia / Hittites<br/>Tin + Copper"] --> TR
    CY["Cyprus & Crete<br/>Copper"] --> TR
    TR --> MG["Mycenaean Greece"]
    MG --> CA["Canaan"]
    CA --> EG["Egypt<br/>Breadbasket"]
    EG --> ME["Mesopotamia / Iraq"]
    ME --> PE["Persia / Iran"]
    PE --> IN["India"]
    style TR fill:#e74c3c,color:#fff
    style MG fill:#2980b9,color:#fff
    style EG fill:#27ae60,color:#fff
```
...
```mermaid
flowchart LR
    A["~1200 BCE<br/>Bronze Age peak"] --> SP["Sea Peoples<br/>attack from west"]
    SP --> MG["Mycenaean Greece<br/>burned, -25% population"]
    SP --> HI["Hittite Empire<br/>destroyed entirely"]
    SP --> CA["Canaan<br/>ceases to exist"]
    SP --> EG["Egypt<br/>survives but weakened"]
    MG --> GC["Greek Civilisation<br/>emerges"]
    CA --> IS["Israelites<br/>emerge"]
    GC --> WC["Two Pillars of<br/>Western Civilisation"]
    IS --> WC
...
```mermaid
flowchart TB
    M["Marx: Bottom-Up Collapse"] --> MW["Workers exploited"]
    MW --> MR["Workers rebel"]
    MR --> MC["Revolution → Collapse"]
    T["Turchin: Top-Down Collapse"] --> TE["Elite multiply over generations"]
    TE --> TF["Too many elites,<br/>not enough resources to charge rent on"]
    TF --> TC["Elite civil war → Collapse"]
    MC --> SAME["Society collapses"]
    TC --> SAME
    style M fill:#e74c3c,color:#fff
    style T fill:#2980b9,color:#fff
    style SAME fill:#7f8c8d,color:#fff
```
...
```mermaid
flowchart TB
    RS["Elite charge rent<br/>(share of crops)"] --> BY["Bad harvest:<br/>farmer can't pay"]
    BY --> D["Farmer borrows<br/>→ goes into debt"]
    D --> CI["Compound interest:<br/>debt grows faster<br/>than crops"]
    CI --> DS["Debt slavery:<br/>farmer becomes property"]
    DS --> POP["Slave population grows<br/>→ anger, instability"]
    RS --> EG2["Elite population<br/>grows too"]
    EG2 --> COMP["More elites compete<br/>for finite land"]
    COMP --> FRAG["Society becomes fragile:<br/>no shock can be absorbed"]
    POP --> FRAG
    FRAG --> SHOCK["Any shock hits:<br/>earthquake, famine,<br/>invasion, riot"]
    SHOCK --> COLLAPSE["Collapse"]
    style RS fill:#e74c3c,color:#fff
    style DS fill:#e74c3c,color:#fff
    style FRAG fill:#e74c3c,color:#fff
    style COLLAPSE fill:#7f8c8d,color:#fff
```
...
```mermaid
flowchart TB
    EO["Too many elites<br/>competing for rent"] --> S1["Land redistribution<br/>A king emerges,<br/>cancels debt, redistributes"]
    EO --> S2["Civil war<br/>Elites fight;<br/>winner takes all"]
    EO --> S3["Empire building<br/>Conquer new territory<br/>to give elites new rent sources"]
    S1 --> I1["= Revolution"]
    S2 --> I2["= War"]
    S3 --> I3["= War + new elite overproduction"]
    I1 --> R["More instability<br/>Cycle restarts"]
    I2 --> R
    I3 --> R
    style EO fill:#e74c3c,color:#fff
...
```mermaid
flowchart TB
    PE["Palace Economy"] --> RED["Farmers bring ALL produce<br/>to the palace"]
    RED --> KING["King redistributes<br/>— keeping a cut"]
    KING --> GOOD["Good years:<br/>system works"]
    KING --> BAD["Bad years:<br/>elite still demand full cut"]
    BAD --> DEBT["Farmers go into debt"]
    DEBT --> SLAVE["Debt slavery"]
    SLAVE --> REVOLT["Internal revolt:<br/>burn the palace"]
    REVOLT --> SEA["Weakened society<br/>falls to Sea Peoples"]
    style PE fill:#2980b9,color:#fff
    style BAD fill:#e74c3c,color:#fff
    style REVOLT fill:#e74c3c,color:#fff
    style SEA fill:#7f8c8d,color:#fff
```
...
```mermaid
flowchart TB
    BAC["Bronze Age Collapse<br/>~1200 BCE"] --> MGB["Mycenaean Greece<br/>burns"] --> GC["New Greek Civilisation<br/>Emerges"]
    BAC --> CAB["Canaan<br/>collapses"] --> IS["Israelites emerge<br/>→ the Bible"]
    GC --> WC["Foundations of<br/>Western Civilisation"]
    IS --> WC
    FF["Forest Fire<br/>Analogy"] --> CL["Clears dead wood"]
    CL --> RG["New growth:<br/>stronger ecosystem"]
    WC -.->|"Same logic"| RG
    style BAC fill:#e74c3c,color:#fff
    style GC fill:#27ae60,color:#fff
    style IS fill:#27ae60,color:#fff
    style WC fill:#27ae60,color:#fff
...
```mermaid
flowchart LR
    subgraph EG["Egalitarian Society"]
        E1["No private property"]
        E2["Women control population"]
        E3["Max ~10,000 people"]
        E4["Stable and peaceful"]
    end
    subgraph EL["Elite Society"]
        L1["Private property + rent-seeking"]
        L2["Permanent hereditary elite"]
        L3["Up to ~1,000,000 people"]
        L4["Powerful but unstable"]
    end
    EL -->|"Must expand to<br/>satisfy surplus elites"| CON["Conquers egalitarian societies"]
    CON -->|"Absorbs population<br/>and resources"| EL
    style EG fill:#27ae60,color:#fff
    style EL fill:#e74c3c,color:#fff
    style CON fill:#7f8c8d,color:#fff
```
