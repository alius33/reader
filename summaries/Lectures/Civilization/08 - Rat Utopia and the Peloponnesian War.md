...
tags: [lecture-summary, civilization, greece, sparta, athens, peloponnesian-war, rat-utopia, eudaimonia, polis, geography, persian-wars, democracy, oligarchy, elite-conflict, calhoun, status-lock-in]
...
```mermaid
flowchart LR
    G["Greek Geography"] --> SP["Sparta<br/>(Peloponnese — Plains)"]
    G --> AT["Athens<br/>(Attica — Coast)"]
    SP --> SL["Agriculture<br/>→ Need Labour"]
    SL --> SH["Helots<br/>(10:1 ratio)"]
    SH --> SM["Militarism<br/>(control slaves)"]
    SM --> SC["Conservative<br/>Isolationist"]
    AT --> AC["Harbour<br/>→ Trade"]
    AC --> AT2["Olive oil, pottery<br/>+ colonisation"]
    AT2 --> AE["Eudaimonia<br/>Competition + Expansion"]
    AE --> AD["Democracy<br/>Expansionist"]
...
```mermaid
flowchart TB
    EU["Eudaimonia<br/>'Be the best you can be'"]
    EU --> C1["Every Athenian<br/>competes for glory"]
    C1 --> OS["Too competitive<br/>→ ostracism invented"]
    OS --> D["10-year banishment<br/>= social death"]
    EU --> NB["Only nobility<br/>can hold real power"]
    NB --> UN["Upper Nobility<br/>(established elite)"]
    NB --> LN["Lower Nobility<br/>(aspiring elite)"]
    UN --> CONF["Constant conflict<br/>between factions"]
    LN --> CONF
    style EU fill:#2980b9,color:#fff
...
```mermaid
flowchart LR
    IR["Ionian Greeks revolt<br/>against Persian taxes"] --> AH["Athens joins — eudaimonia"]
    AH --> MA["Battle of Marathon<br/>490 BCE<br/>10k Athenians vs 25k Persians"]
    MA --> MW["Athenians win<br/>192 vs 5,000 dead"]
    MW --> XE["Xerxes plans revenge<br/>~500,000 soldiers"]
    XE --> TH["Thermopylae 480 BCE<br/>300 Spartans + 5k Greeks fall"]
    TH --> AB["Athens burned"]
    AB --> SA["Battle of Salamis 480 BCE<br/>Themistocles's trap"]
    SA --> PN["Persian navy destroyed<br/>(ego overrides generals' advice)"]
    PN --> PL["Battle of Plataea 479 BCE<br/>100k vs 100k"]
    PL --> GV["Greeks win — Persia<br/>retreats permanently"]
    GV --> WL["Treasure flows into Greece<br/>→ extreme wealth"]
    style MA fill:#27ae60,color:#fff
    style SA fill:#27ae60,color:#fff
    style PL fill:#27ae60,color:#fff
    style AB fill:#e74c3c,color:#fff
    style WL fill:#2980b9,color:#fff
```
...
```mermaid
flowchart TB
    PL["Pericles<br/>(lower nobility)"] --> DM["Extends democracy<br/>to all citizens"]
    DM --> VM["Lower classes<br/>vote for Pericles"]
    PL --> ST["Moves Delian League<br/>treasury to Athens"]
    ST --> PT["Builds Parthenon +<br/>public works<br/>(billions spent)"]
    PT --> JB["Jobs + patronage<br/>for supporters"]
    JB --> VM
    VM --> OX["Opponents try<br/>to ostracise him"]
    OX --> PE["People vote to<br/>exile his opponents instead"]
    PE --> RU["Pericles rules<br/>unchallenged<br/>461–429 BCE"]
    RU --> EM["Delian League<br/>→ Athenian Empire<br/>20% revenue from allies"]
...
```mermaid
flowchart TB
    subgraph ATH_COULD["Athens Could Have..."]
        A1["Land navy on<br/>Spartan coast"] --> A2["Arm and support<br/>helot rebellion"]
        A2 --> A3["10:1 ratio —<br/>Sparta collapses in weeks"]
    end
    subgraph SPA_COULD["Sparta Could Have..."]
        S1["Free the helots"] --> S2["Offer freedom<br/>for military service"]
        S2 --> S3["Army grows 10x —<br/>Athens overwhelmed"]
    end
    A3 --> NX["Neither side did this"]
    S3 --> NX
    NX --> WHY["WHY?<br/>Both ruling classes feared<br/>social change more than defeat"]
    WHY --> RE["Real war: Upper Nobility<br/>vs Lower Nobility<br/>in both cities simultaneously"]
    style NX fill:#e74c3c,color:#fff
    style WHY fill:#2980b9,color:#fff
...
```mermaid
flowchart TB
    W431["Greek World 431 BCE<br/>Upper nobility in power<br/>Lower nobility frustrated"] --> WAR["27-year<br/>Peloponnesian War"]
    WAR --> DEAD["Generation of<br/>lower nobility killed"]
    WAR --> W404["Greek World 404 BCE<br/>Upper nobility in power<br/>Lower nobility depleted"]
    W431 -.->|"Same families<br/>Same hierarchy<br/>Same power structure"| W404
    DEAD --> PURP["War's real purpose:<br/>eliminate internal dissent<br/>while preserving the status quo"]
    style WAR fill:#e74c3c,color:#fff
    style PURP fill:#2980b9,color:#fff
