...
tags: [lecture-summary, civilization, yamnaya, steppe, indo-european, patriarchy, social-evolution, pastoralism, conquest, horse-domestication, plague, primogeniture, sky-father, mother-goddess]
...
```mermaid
flowchart LR
    NE["Near East Farmers<br/>~8,000 BC"] --> OE["Old Europe<br/>Mother Goddess<br/>Egalitarian, Peaceful"]
    NE --> ST["The Steppe<br/>Grassland, no crops<br/>Survival crisis"]
    OE --> QU["QUESTION:<br/>Who were the Yamnaya<br/>and how did they destroy this?"]
    ST --> QU
    style NE fill:#2980b9,color:#fff
    style OE fill:#27ae60,color:#fff
    style ST fill:#e74c3c,color:#fff
    style QU fill:#2980b9,color:#fff
```
...
```mermaid
flowchart TB
    S["Scarce Steppe Resources<br/>(can't eat grass)"] --> OCC["Open Cooperative Competition<br/>Many groups, no hegemon,<br/>still communicating"]
    OCC --> G1["Group A: Strategy 1"]
    OCC --> G2["Group B: Strategy 2"]
    OCC --> G3["Group C: Strategy 3"]
    OCC --> G4["Group D: Strategy 4"]
    G1 & G2 & G3 & G4 --> T["Trade, observe, learn<br/>across groups"]
    T --> RA["THE RUTHLESS ADOPTER<br/>Adopts ALL innovations<br/>to destroy others"]
    RA --> CON["CONQUEST"]
    style S fill:#e74c3c,color:#fff
    style OCC fill:#2980b9,color:#fff
...
```mermaid
flowchart TB
    I1["1. Pastoral Economy<br/>Cattle eat grass,<br/>humans eat cattle"] --> I2["2. Dairy Revolution<br/>Lactose tolerance —<br/>drink milk"]
    I2 --> I3["3. Horse Domestication<br/>3,000 years of effort —<br/>horses flee humans"]
    I3 --> I4["4. The Wheel<br/>Horse + wheel =<br/>wagon"]
    I4 --> I5["5. Nomadic Pastoral Life<br/>Move herds across<br/>the steppe"]
    I2 --> HT["20cm taller<br/>than farmers"]
    I5 --> GR["Conflict over<br/>grazing rights"]
    I5 --> CR["Cattle raiding<br/>= rational wealth strategy"]
    GR & CR --> WC["WAR CULTURE"]
    style I1 fill:#2980b9,color:#fff
    style WC fill:#e74c3c,color:#fff
...
```mermaid
flowchart TB
    E["ECONOMY<br/>Cattle = private wealth"] --> S["SOCIETY<br/>Men protect wealth<br/>→ patriarchy"]
    S --> PR["PRIMOGENITURE<br/>Eldest son inherits all"]
    PR --> YM["YOUNGER SONS<br/>Must raid to build wealth"]
    YM --> WC["WAR CULTURE<br/>Permanent production<br/>of aggressive men"]
    WC --> R["RELIGION<br/>Sky Father replaces<br/>Mother Goddess"]
    R -->|"justifies"| E
    R -->|"commands"| WC
...
```mermaid
flowchart TB
    EU["Europe's Farming<br/>Population<br/>(numerical advantage)"]
    PL["1. PLAGUE — MOST IMPORTANT<br/>Bubonic plague devastates<br/>dense farming settlements"]
    CC["2. CLIMATE CHANGE<br/>Mini ice age destroys<br/>crops (~5,000–6,000 yrs ago)"]
    YA["3. YAMNAYA ASSAULT<br/>Taller, mounted, culturally<br/>programmed for conquest"]
    EU --> PL
    PL --> D["Population decimated<br/>— most killed"]
    D --> CC
    CC --> W["Weakened + starving<br/>survivors"]
    W --> YA
    YA --> C["CONQUEST OF EUROPE"]
    style PL fill:#e74c3c,color:#fff
    style CC fill:#e74c3c,color:#fff
    style YA fill:#e74c3c,color:#fff
...
```mermaid
flowchart LR
    Y["YAMNAYA<br/>(origin: Ukraine)"] --> EU["Europe<br/>(inc. England)"]
    Y --> IR["Iran"]
    Y --> IN["India<br/>(Vedic civilisation)"]
    Y --> MO["Mongolia"]
    Y -.->|"BLOCKED"| CH["China"]
    HI["Himalayas<br/>(horses can't<br/>cross mountains)"] -.->|"geographic barrier"| CH
    POP["Large Chinese<br/>population"] -.->|"could repel<br/>incursions"| CH
