...
tags: [lecture-summary, civilization, old-europe, gimbutas, proto-indo-european, yamnaya, linguistics, matriarchy, egalitarianism, kurgan-hypothesis, sexual-agency, patriarchy]
...
```mermaid
flowchart LR
    PIE["<b>Proto-Indo-European</b><br/>The lost mother tongue<br/>(Ukrainian steppes)"] --> EN["English<br/><i>father, two</i>"]
    PIE --> LAT["Latin<br/><i>pater, duo</i>"]
    PIE --> GRK["Greek<br/><i>patēr, dúo</i>"]
    PIE --> PER["Persian<br/><i>padar</i>"]
    PIE --> HIN["Hindi<br/><i>pitā</i>"]
    PIE --> GER["Germanic<br/><i>Vater, zwei</i>"]
    style PIE fill:#2980b9,color:#fff
```
...
```mermaid
flowchart TB
    V1["Five words for <b>wheel</b>"] --> D1["Invented the wheel<br/>(~3500 BCE, Ukrainian steppes)"]
    V2["Dairy vocabulary<br/><i>milk, cheese, curds,<br/>sour milk, buttermilk</i>"] --> D2["First lactose-tolerant people<br/>(genetic mutation ~4500–3500 BCE)"]
    V3["Few agricultural terms<br/><i>relative to other cultures</i>"] --> D3["Not primarily farmers<br/>(pastoral lifestyle)"]
    V4["Words for <b>horse</b><br/><i>+ yoke, wagon</i>"] --> D4["First to domesticate the horse<br/>(3,000 years: 5500–2200 BCE)"]
    D1 --> PROFILE["<b>The Yamnaya</b><br/>Nomadic pastoralists<br/>from the Eurasian steppes<br/>— 20cm taller than farmers"]
    D2 --> PROFILE
    D3 --> PROFILE
    D4 --> PROFILE
    style PROFILE fill:#e74c3c,color:#fff
    style V1 fill:#2980b9,color:#fff
    style V2 fill:#2980b9,color:#fff
    style V3 fill:#2980b9,color:#fff
    style V4 fill:#2980b9,color:#fff
```
...
```mermaid
flowchart TB
    Q["How did PIE spread<br/>across Europe?"] --> C["<b>Conquest model</b><br/>Genghis Khan /<br/>European colonisation"]
    Q --> D["<b>Diffusion model</b><br/>English spreading<br/>to China today"]
    C --> TC["Original genes <b>erased</b><br/>(people killed)"]
    D --> TD["Original genes <b>persist</b><br/>(people survive)"]
    TC --> DNA["DNA evidence:<br/>Old European genes<br/><b>collapse after 3000 BCE</b>"]
    TD --> X["❌ Rejected by evidence"]
    DNA --> V["✅ Conquest confirmed<br/>— it was genocide"]
    style V fill:#e74c3c,color:#fff
    style X fill:#999,color:#fff
    style DNA fill:#2980b9,color:#fff
```
...
```mermaid
flowchart LR
    A["<b>9000 BCE</b><br/>Agriculture begins<br/>in Near East"] --> B["<b>6500 BCE</b><br/>Climate cools /<br/>soil exhausted in Anatolia"]
    B --> C["<b>Farming families migrate</b><br/>Anatolia → Europe<br/>Bring mother goddess religion"]
    C --> D["<b>6500–2500 BCE</b><br/>OLD EUROPE<br/>4,000 years of peace"]
    E["<b>~3500 BCE</b><br/>Yamnaya develop<br/>on Ukrainian steppes"] --> F["<b>~3000 BCE</b><br/>Yamnaya conquest<br/>of Europe begins"]
    F --> G["<b>~2500 BCE</b><br/>Old Europe<br/>destroyed"]
...
```mermaid
flowchart TB
    G["<b>Gimbutas ridiculed</b><br/>for decades"] --> R1["<b>Male academic bias</b><br/>Male professors couldn't accept<br/>women had ever been in charge"]
    G --> R2["<b>Violence assumption</b><br/>Scholars believed humans<br/>are inherently warlike —<br/>4,000 years of peace<br/>'can't be true'"]
    G --> R3["<b>Wealth assumption</b><br/>Economists believed money<br/>always drives behaviour —<br/>no wealth means no civilisation"]
    R1 --> DNA["<b>Ancient DNA evidence</b><br/>(2010s onward)<br/>confirms Kurgan hypothesis<br/>on every point"]
    R2 --> DNA
    R3 --> DNA
    DNA --> V["<b>Gimbutas vindicated</b><br/>She was right.<br/>The men who laughed were wrong."]
    style G fill:#e74c3c,color:#fff
...
```mermaid
flowchart TB
    MG["<b>Mother Goddess Religion</b><br/>All life shares one<br/>divine source"] --> U["Unity of all beings<br/>Humans = animals =<br/>plants = sacred"]
    U --> P["No weapons produced<br/>(despite knowing metallurgy)"]
    U --> A["Superb art<br/>Pottery, sculpture,<br/>shrines, temples"]
    U --> W["Invented writing<br/>for religious purposes<br/>(before the Sumerians<br/>invented it for economics)"]
    U --> E["Egalitarian communities<br/>No hierarchy, no forts,<br/>communal living"]
    P --> S["<b>4,000 years of</b><br/><b>peace and stability</b><br/>6500–2500 BCE"]
    A --> S
    W --> S
    E --> S
...
```mermaid
flowchart TB
    PRE["<b>Pre-plough societies</b><br/>Men and women do<br/>the same agricultural work"] --> EQ["Equal political power<br/>Equal labour participation"]
    PLOUGH["<b>Plough agriculture</b><br/>Requires upper-body<br/>strength (favours men)"] --> SHIFT["Men contribute more<br/>to food production<br/>for the first time"]
    SHIFT --> POWER["Men gain political power<br/>Lower female participation<br/>(Harvard/UCLA study)"]
    POWER --> PP["<b>Private property</b> emerges<br/>Accumulated surplus<br/>becomes ownable"]
    PP --> INH["Need to control<br/>inheritance — who<br/>gets the goats?"]
    INH --> PAT["Need to control<br/>paternity — whose<br/>children are mine?"]
    PAT --> BODY["Control women's bodies —<br/>she can only sleep<br/>with her husband"]
    BODY --> SHAME["Sexual shame enforced<br/>as 'morality'"]
    SHAME --> RESULT["<b>PATRIARCHY</b>"]
    style PRE fill:#27ae60,color:#fff
    style RESULT fill:#e74c3c,color:#fff
    style PLOUGH fill:#2980b9,color:#fff
```
...
```mermaid
flowchart TB
    W["<b>Women as</b><br/><b>political class</b>"] --> M1["<b>Cooperation</b><br/>More willing to compromise<br/>and share power —<br/>no status cost for backing down"]
    W --> M2["<b>Emotional intelligence</b><br/>Govern through relationships,<br/>not abstract rules —<br/>responsive to individuals"]
    W --> M3["<b>Non-violent social control</b>"]
    M3 --> G["<b>Gossip</b><br/>Social pressure to conform<br/>through praise and criticism —<br/>reputation is everything"]
    M3 --> S["<b>Sex distribution</b><br/>Reward: inclusion<br/>Punishment: exclusion<br/>— leverages need to belong"]
    M1 --> RESULT["<b>4,000 years of</b><br/><b>peace and stability</b>"]
    M2 --> RESULT
    G --> RESULT
    S --> RESULT
