...
tags: [lecture-summary, civilization, greece, macedon, philip-ii, military-innovation, phalanx, diplomacy, leadership, meritocracy, sacred-band-thebes, assassination, parmenion, alexander, father-son-archetype]
...
```mermaid
flowchart LR
    A["Greek Civilisation"] -->|"Standard account"| B["Alexander the Great<br/>(spreads it by conquest)"]
    A -->|"Prof. Jiang's account"| C["Philip II of Macedon<br/>(builds the army<br/>that makes it possible)"]
    C --> D["World's most disciplined<br/>and innovative army"]
    D --> B
    B --> E["Persian Empire conquered<br/>Greek culture spread worldwide"]
...
```mermaid
flowchart LR
    F["THE FATHER<br/>(Builder — Philip II)"] --> F1["Innovative /<br/>Wise judgement"]
    F --> F2["Visionary — attracts<br/>talent with a dream"]
    F --> F3["Selfless / Disciplined —<br/>works harder than everyone"]
    S["THE SON<br/>(Expander — Alexander)"] --> S1["Aggressive —<br/>takes risks no one else will"]
    S --> S2["Promotes loyalty<br/>over talent"]
    S --> S3["Driven by insecurity:<br/>must prove he is<br/>greater than the father"]
...
```mermaid
flowchart TB
    R["RICH NATION<br/>(South Korea / Athens / Thebes)"] --> I["Inequality"]
    I --> D["Division — factions<br/>hate each other"]
    D --> LF["Low fertility /<br/>Cultural decline"]
    LF --> W["Military weakness<br/>and complacency"]
    P["POOR NATION<br/>(North Korea / Macedon)"] --> E["Equality<br/>(everyone is poor together)"]
    E --> U["Unity and obedience"]
    U --> H["Hunger — will work<br/>hard for very little"]
    H --> MS["Military strength<br/>and hunger to fight"]
    W --> C["Poor nation CONQUERS<br/>rich nation"]
    MS --> C
...
```mermaid
flowchart TB
    M["MACEDON<br/>Poor, Weak, Divided"] --> G["Geography Problem<br/>Farmland south /<br/>Mountains north"]
    M --> E["Enemy Problem<br/>Surrounded on<br/>every side"]
    M --> S["Succession Problem<br/>King's many wives<br/>→ rival sons"]
    G --> G1["Mountain tribes raid<br/>the farmland; can't<br/>be pursued into hills"]
    E --> E1["Thrace (east)"]
    E --> E2["Illyria (west) —<br/>warlike mountain fortress people"]
    E --> E3["Thessaly (south) —<br/>famous cavalry"]
    E --> E4["Thebes / Athens / Sparta"]
    E --> E5["Persia — once<br/>conquered Macedon outright"]
    S --> S1["Each rival son backed<br/>by a foreign power<br/>wanting Macedon weak"]
    S1 --> S2["Perpetual civil<br/>conflict"]
...
```mermaid
flowchart TB
    H["Philip — hostage in Thebes<br/>369–365 BCE<br/>(age ~14–18)"]
    H --> L1["Lesson 1: Anyone can be<br/>a great soldier with training —<br/>not just the rich or noble"]
    H --> L2["Lesson 2: Elite soldiers must<br/>be deployed effectively —<br/>oblique order (Sacred Band first)"]
    H --> L3["Lesson 3: Discipline unlocks<br/>three capabilities:<br/>Mobility, Coordination, Flexibility"]
    L3 --> M["Mobility — march faster<br/>than any opponent"]
    L3 --> C["Coordination — phalanx<br/>and cavalry work together<br/>(anvil and hammer)"]
    L3 --> F["Flexibility — strategy<br/>adapts to each specific enemy"]
    M --> R["No army in Greece<br/>had all three"]
    C --> R
    F --> R
    R --> P["Philip resolves to<br/>build one that does"]
...
```mermaid
flowchart TB
    P["PHILIP BECOMES REGENT<br/>359 BCE<br/>'The Macedonian army was a complete joke'"]
    P --> M["1. MERITOCRACY<br/>Promote by talent,<br/>not birth"]
    P --> L["2. LOYALTY<br/>Fight alongside soldiers,<br/>eat and drink with them,<br/>communicate vision"]
    P --> PH["3. PHALANX REINVENTION<br/>Lighter armour + sarissa<br/>+ shield bearers"]
    P --> D["4. DIPLOMACY<br/>Alliances, marriage,<br/>bribery — buy time<br/>while army trains"]
    P --> G["5. GOLD (347 BCE)<br/>Conquers Amphipolis<br/>→ funds everything"]
    M --> PA["Parmenion: lower<br/>nobility → supreme general"]
    PH --> AH["Anvil and hammer<br/>now executable"]
    G --> PS["Pays professional<br/>soldiers, buys nobility,<br/>builds infrastructure,<br/>bribes foreign elites"]
...
```mermaid
flowchart LR
    SB["Sacred Band of Thebes<br/>teaches Philip<br/>369–365 BCE"] --> PL["Philip builds<br/>Macedonian army<br/>on Theban principles"]
    PL --> CH["Battle of Chaeronea<br/>338 BCE"]
    CH --> A["Athens: army<br/>destroyed entirely"]
    CH --> T["Thebes: nearly<br/>destroyed"]
    T --> LS["Sacred Band makes<br/>last stand — sacrifices<br/>itself for Theban retreat"]
    LS --> END["Sacred Band<br/>destroyed forever"]
    END -.->|"Student kills teacher"| SB
    CH --> U["Philip unites<br/>all of Greece"]
    U --> PP["Parmenion sent to<br/>Anatolia with 10,000<br/>vanguard for Persia"]
...
```mermaid
flowchart TB
    AS["ASSASSINATION OF PHILIP II<br/>~337 BCE<br/>Daughter's wedding<br/>One bodyguard — Pausanias"]
    AS --> P1["Suspect 1: PERSIA"]
    AS --> P2["Suspect 2: PERSONAL GRUDGE<br/>(Pausanias)"]
    AS --> P3["Suspect 3: OLYMPIAS<br/>& ALEXANDER"]
    P1 --> M1["Motive: ✅<br/>(Philip about to invade)"]
    P1 --> O1["Opportunity: ❌<br/>(Philip was perceptive<br/>about who to trust;<br/>easier to fund Sparta/Athens)"]
    M1 --> V1["UNLIKELY"]
    O1 --> V1
    P2 --> M2["Motive: ⚠️<br/>(jealous ex-lover — possible)"]
    P2 --> O2["Opportunity: ⚠️<br/>(bodyguard had access,<br/>but Philip was good<br/>at reading people)"]
    M2 --> V2["POSSIBLE<br/>BUT UNLIKELY"]
    O2 --> V2
    P3 --> M3["Motive: ✅<br/>(Philip dies → Alexander<br/>becomes king at height<br/>of Macedon's power)"]
    P3 --> O3["Opportunity: ✅<br/>(family; constant access<br/>to the bodyguard corps;<br/>Olympias built monument<br/>to Pausanias after his death)"]
    M3 --> V3["MOST LIKELY"]
    O3 --> V3
...
```mermaid
flowchart LR
    A["383 BCE<br/>Philip born"] --> B["369–365 BCE<br/>Hostage in Thebes<br/>Learns from Sacred Band generals"]
    B --> C["359 BCE<br/>Becomes regent<br/>(brother dies)"]
    C --> D["359–350s<br/>Army reforms:<br/>meritocracy, phalanx,<br/>loyalty, diplomacy"]
    D --> E["347 BCE<br/>Conquers Amphipolis<br/>(gold mines)"]
    E --> F["356 BCE<br/>Alexander born<br/>(son by Olympias)"]
    F --> G["338 BCE<br/>Battle of Chaeronea<br/>All Greece united"]
    G --> H["~337 BCE<br/>Assassinated at<br/>daughter's wedding"]
