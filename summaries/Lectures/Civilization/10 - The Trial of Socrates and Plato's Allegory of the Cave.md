...
tags: [lecture-summary, civilization, greece, philosophy, socrates, plato, democracy, trial, allegory-of-the-cave, republic, aristophanes, peloponnesian-war, christianity, forms, hemlock, academy, philosopher-kings]
...
```mermaid
flowchart LR
    S["Person makes<br/>a truth claim"] --> Q1["What do you<br/>mean by that?"]
    Q1 --> Q2["How do you<br/>know it's true?"]
    Q2 --> Q3["Can you define<br/>your terms?"]
    Q3 --> Q4["Does your definition<br/>hold under scrutiny?"]
    Q4 --> R["Claim collapses:<br/>exposed ignorance"]
    R --> T["Socrates's thesis proven:<br/>you cannot reason"]
...
```mermaid
flowchart TB
    PW["Athens loses<br/>Peloponnesian War<br/>(404 BCE)"] --> SP["Sparta installs<br/>the 30 Tyrants"]
    SP --> TY["Tyrants = wealthy<br/>Athenian aristocrats,<br/>many Socrates's students"]
    TY --> K["Kill 5% of<br/>Athenian population"]
    TY --> ST["Steal wealth,<br/>terrorise citizens"]
    K --> REV["Democratic revolt<br/>overthrows tyrants"]
    REV --> AM["General amnesty —<br/>collaborators forgiven"]
    AM --> TR["Socrates put on<br/>trial (399 BCE):<br/>five years later"]
...
```mermaid
flowchart TB
    C["Charges: impiety +<br/>corrupting youth<br/>(from The Clouds)"] --> E["Athens expects<br/>apology + forgiveness"]
    E --> S["Socrates insults jury:<br/>'If you're stupid,<br/>you'll convict me'"]
    S --> G["Guilty: 280–220<br/>(close despite provocation)"]
    G --> P["Proposes punishment:<br/>a pension, or a small fine"]
    P --> D["Death by hemlock"]
    D --> B["Athens tries<br/>to back down"]
    B --> I["Socrates demands<br/>his own execution"]
    I --> M["Martyrdom at 70:<br/>'Democracy cannot<br/>reason to truth'"]
...
```mermaid
flowchart LR
    SD["Socrates dies<br/>(399 BCE)"] --> PT["Plato travels<br/>12 years<br/>(399–387 BCE)"]
    PT --> INF["Absorbs Egyptian,<br/>Mesopotamian,<br/>Greek philosophy"]
    INF --> AC["Founds Academy<br/>(~387 BCE)"]
    AC --> AR["Trains Aristotle<br/>+ global elite"]
    AR --> REP["Writes the Republic<br/>(~375 BCE)"]
    REP --> ALG["Allegory of the Cave<br/>redeems Socrates forever"]
    style SD fill:#e74c3c,color:#fff
    style AC fill:#2980b9,color:#fff
    style REP fill:#27ae60,color:#fff
```
...
```mermaid
flowchart TB
    P["Prisoners chained<br/>in cave — can only<br/>see the wall"] --> W["See only shadows<br/>of puppets cast<br/>by a fire"]
    W --> L["Name the shadows,<br/>create language,<br/>play games"]
    L --> A["Award the best<br/>describers — poets,<br/>playwrights, artists"]
    A --> LIE["All of it is<br/>illusion"]
    LIE --> FREE["One prisoner's<br/>chains loosen"]
    FREE --> CLIMB["Climbs toward<br/>the light above"]
    CLIMB --> PAIN["Sunlight burns —<br/>eyes unadjusted"]
    PAIN --> POOL["First sees<br/>reflections in pools"]
    POOL --> WORLD["Gradually sees<br/>the real world"]
    WORLD --> SUN["Finally sees<br/>the sun itself —<br/>source of all life"]
    SUN --> TRUTH["Fully understands<br/>truth for the<br/>first time"]
    TRUTH --> RETURN["Returns to cave<br/>out of pity<br/>for friends"]
    RETURN --> FAIL["Cannot describe<br/>truth — no language<br/>for sunlight"]
    FAIL --> KILL["Prisoners think<br/>he's an idiot —<br/>they kill him"]
    style P fill:#e74c3c,color:#fff
    style SUN fill:#f39c12,color:#fff
    style TRUTH fill:#27ae60,color:#fff
...
```mermaid
flowchart TB
    ALG["Allegory of the Cave"]
    ALG --> P1["Power 1:<br/>Redeems Socrates"]
    ALG --> P2["Power 2:<br/>Creates Christ archetype"]
    ALG --> P3["Power 3:<br/>Provides Christian<br/>cosmology"]
    P1 --> E1["Clown → Greatest<br/>philosopher who<br/>ever lived"]
    P2 --> E2["Socrates = Jesus:<br/>truth-bearer descended<br/>from above, killed<br/>by those he served"]
    P3 --> E3["Form of Good = God<br/>Perfect Forms = Heaven<br/>Our world = fallen Earth"]
    style ALG fill:#2980b9,color:#fff
...
```mermaid
flowchart TB
    subgraph PLATO["Plato's Cosmology"]
        FG["Form of the Good<br/>(the sun)"]
        FG --> IDEALS["Reason — Beauty<br/>Truth — Justice"]
        IDEALS --> FORMS["Perfect Forms<br/>(perfect horse,<br/>perfect person)"]
        FORMS --> WORLD["Our world:<br/>imperfect copies /<br/>shadows"]
    end
    subgraph CHRISTIAN["Christian Framework"]
        GOD["God"]
        GOD --> HEAVEN["Heaven<br/>(angels, saints)"]
        HEAVEN --> EARTH["Earth<br/>(mortal, fallen)"]
    end
    FG -.->|"="| GOD
    FORMS -.->|"="| HEAVEN
    WORLD -.->|"="| EARTH
    style FG fill:#f39c12,color:#fff
    style GOD fill:#f39c12,color:#fff
    style WORLD fill:#e74c3c,color:#fff
    style EARTH fill:#e74c3c,color:#fff
```
...
```mermaid
flowchart LR
    Q["What is a<br/>good society?"] --> ND["Not democracy —<br/>it killed Socrates"]
    ND --> J["A society based<br/>on justice"]
    J --> T["Justice requires truth"]
    T --> FG["Truth = the<br/>Form of the Good"]
    FG --> PH["Only philosophers<br/>can access it<br/>through reason"]
    PH --> PK["Therefore:<br/>philosopher kings<br/>should rule"]
    style Q fill:#2980b9,color:#fff
    style ND fill:#e74c3c,color:#fff
...
```mermaid
flowchart TB
    PL["Plato's enduring<br/>influence"]
    PL --> R["Readability:<br/>playwright training<br/>— dialogue on the page"]
    PL --> AD["Anti-democratic politics:<br/>kings promoted him<br/>for 2,000 years"]
    PL --> AC["The Academy:<br/>trained the world's<br/>elite students"]
    R --> RES["Only writer read<br/>continuously for<br/>2,000 years"]
    AD --> RES
    AC --> RES
    style PL fill:#2980b9,color:#fff
