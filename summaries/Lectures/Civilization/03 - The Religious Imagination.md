...
tags: [lecture-summary, civilization, religion, imagination, animism, barasana, pygmies, creation-myths, shamanism, collective-belief]
...
```mermaid
flowchart LR
    A["40,000 years ago<br/>Figurines + Flutes<br/>(Germany)"] --> B["30,000 years ago<br/>Cave Paintings<br/>(Europe-wide)"]
    B --> C["9,500 BC<br/>Göbekli Tepe<br/>(Turkey)"]
    C --> D["Today<br/>Barasana<br/>(Amazon)"]
    C --> E["Today<br/>Pygmies<br/>(Central Africa)"]
...
```mermaid
flowchart TB
    R["A Powerful<br/>Religion"] --> G["Grandness<br/>Vast in scope"]
    R --> C["Completeness<br/>Explains everything<br/>you observe"]
    R --> U["Unity<br/>Coherent beginning<br/>and end"]
    G --> A["Authority<br/>equal to science"]
    C --> A
    U --> A
...
```mermaid
flowchart TB
    subgraph ANIM["The Animist Worldview"]
        MG["Mother Goddess<br/>(one source of life)"]
        MG --> SOUL["Permanent Soul<br/>(body dies, soul continues)"]
        MG --> EQUAL["All Beings Equal<br/>(trees, animals, humans)"]
        MG --> SPIRIT["Spirit World<br/>(more real than<br/>physical world)"]
        SPIRIT --> SHADOW["Shadow Dimension<br/>(invisible to most,<br/>visible to shamans)"]
        EQUAL --> RECIP["Reciprocity<br/>(give-and-take<br/>with nature)"]
    end
    style MG fill:#2980b9,color:#fff
    style SPIRIT fill:#2980b9,color:#fff
    style SHADOW fill:#27ae60,color:#fff
    style RECIP fill:#27ae60,color:#fff
```
...
```mermaid
flowchart TB
    NEED["Community<br/>needs food"] --> PERM["Shaman negotiates<br/>with spirit guardians"]
    PERM --> CONTRACT["Mystical contract<br/>(reciprocity)"]
    CONTRACT --> HUNT["Hunters go<br/>to the forest"]
    HUNT --> KILL["Animal killed"]
    KILL --> TRIBUTE["Pay tribute /<br/>Ask forgiveness"]
    TRIBUTE --> RETURN["Animal's soul returns<br/>to spirit world"]
    RETURN --> BALANCE["Cosmic balance<br/>maintained"]
    BALANCE -.-> NEED
    style NEED fill:#e74c3c,color:#fff
    style PERM fill:#2980b9,color:#fff
...
```mermaid
flowchart LR
    subgraph MOD["Modern Mind"]
        M1["Man vs. Nature"]
        M2["Nature = dangerous"]
        M3["Obsession with<br/>CONTROL"]
        M4["Fear of the<br/>unknown"]
    end
    subgraph PRE["Pre-Modern Mind"]
        P1["Man as part<br/>of Nature"]
        P2["Nature = home"]
        P3["Foundation of<br/>TRUST"]
        P4["Fear only of what<br/>is outside nature"]
    end
    MOD <--> |"fundamental<br/>divide"| PRE
...
```mermaid
flowchart TB
    SLEEP["Someone sleeps<br/>during the molimo"]
    SLEEP --> MSG["Message: 'This<br/>religion is not real'"]
    MSG --> T1["Rejects the<br/>community"]
    MSG --> T2["Insults the<br/>forest / God"]
    MSG --> T3["Endangers<br/>collective belief"]
    T3 --> CORE["Religion is only real<br/>if EVERYONE believes"]
    CORE --> THREAT["One non-believer can<br/>cause the whole<br/>reality to collapse"]
    THREAT --> DEATH["The sleeper must<br/>be eliminated"]
...
```mermaid
flowchart TB
    TURN["Turnbull observes<br/>the molimo ceremony"]
    TURN --> SEES["Sees women showing<br/>fear, men singing,<br/>trumpet sounds"]
    SEES --> VOCAB["His vocabulary:<br/>'pretending'<br/>'make-believe'"]
    SEES --> FEELS["His instinct:<br/>'something very real<br/>and very great'"]
    VOCAB --> MAT["Materialism:<br/>if you can't see it,<br/>it's not real"]
    FEELS --> TRUTH["The ceremony IS<br/>real — more real<br/>than physical reality"]
    MAT <--> |"chasm"| TRUTH
    style MAT fill:#e74c3c,color:#fff
    style TRUTH fill:#27ae60,color:#fff
    style TURN fill:#2980b9,color:#fff
```
...
```mermaid
flowchart TB
    REL["The Religious<br/>Imagination"] --> P["Peaceful<br/>(war disturbs<br/>spirit world)"]
    REL --> E["Egalitarian<br/>(all beings from<br/>same source)"]
    REL --> A["Artistic<br/>(worship requires<br/>celebration)"]
    P --> S["200,000 years of<br/>stable society"]
    E --> S
    A --> S
    S --> Q["What destroyed<br/>this worldview?"]
    Q --> Y["The Yamnaya<br/>(Lecture 4)"]
...
```mermaid
flowchart TB
    subgraph DIV1["Divide 1: Relationship to Nature"]
        MA["Modern: Man vs. Nature<br/>(control, fear)"]
        PA["Pre-Modern: Man as Nature<br/>(trust, belonging)"]
        MA <--> |"revealed by<br/>forest walk"| PA
    end
    subgraph DIV2["Divide 2: Nature of Reality"]
        MB["Modern: Materialism<br/>(only measurable is real)"]
        PB["Pre-Modern: Spiritualism<br/>(invisible is more real)"]
        MB <--> |"revealed by<br/>'pretending'"| PB
    end
    DIV1 --> SYN["Both divides produce<br/>the same blindness:<br/>we cannot see what<br/>they see"]
    DIV2 --> SYN
    style MA fill:#e74c3c,color:#fff
    style PA fill:#27ae60,color:#fff
    style MB fill:#e74c3c,color:#fff
    style PB fill:#27ae60,color:#fff
    style SYN fill:#2980b9,color:#fff
```
