...
tags: [lecture-summary, civilization, greece, theatre, tragedy, democracy, aeschylus, sophocles, euripides, oresteia, oedipus, antigone, bacchae, hubris, empire, peloponnesian-war, pericles]
...
```mermaid
flowchart LR
    M1["Schools<br/>(shared knowledge)"] --> ID["Collective<br/>Identity"]
    M2["Mass Media<br/>(shared worldview)"] --> ID
    M3["Entertainment<br/>(shared culture)"] --> ID
    ID --> MOD["Modern<br/>Societies"]
    TH["Athenian<br/>Theatre"] --> ALL["All three<br/>functions at once"]
    ALL --> DEM["Democratic<br/>Identity"]
    style TH fill:#2980b9,color:#fff
    style DEM fill:#27ae60,color:#fff
    style MOD fill:#95a5a6,color:#fff
```
...
```mermaid
flowchart TB
    A["Atreus feeds brother<br/>his own children"] --> CU["Curse on the<br/>House of Atreus"]
    CU --> AG["Agamemnon sacrifices<br/>daughter Iphigenia"]
    AG --> CL["Clytemnestra<br/>kills Agamemnon"]
    CL --> OR["Orestes kills<br/>his mother"]
    OR --> FU["Furies haunt<br/>Orestes"]
    FU --> AT["Athena convenes<br/>jury of 500"]
    AT --> HJ["Hung jury:<br/>250 vs. 250"]
    HJ --> AV["Athena casts<br/>deciding vote"]
    AV --> FREE["Orestes freed.<br/>Furies become<br/>agents of justice"]
...
```mermaid
flowchart TB
    CR["Creon becomes king"] --> BAN["Bans burial<br/>of Polynices"]
    BAN --> AN["Antigone defies<br/>the order"]
    AN --> IG1["Creon ignores<br/>Antigone's argument"]
    IG1 --> IG2["Creon ignores<br/>Haemon's warning"]
    IG2 --> IG3["Creon ignores<br/>the seer's prophecy"]
    IG3 --> D1["Antigone<br/>dies"]
    D1 --> D2["Haemon<br/>dies"]
    D2 --> D3["Eurydice<br/>dies"]
    D3 --> ALONE["Creon alone —<br/>hubris destroyed<br/>everything"]
    style CR fill:#2980b9,color:#fff
    style ALONE fill:#e74c3c,color:#fff
    style IG1 fill:#e67e22,color:#fff
    style IG2 fill:#e67e22,color:#fff
    style IG3 fill:#e67e22,color:#fff
```
...
```mermaid
flowchart LR
    subgraph Oresteia["The Oresteia"]
        O1["Old gods<br/>(Furies)"] -->|"willingly yield"| O2["New order<br/>(Athena/Justice)"]
        O2 --> O3["✅ Justice<br/>and peace"]
    end
    subgraph Antigone["Antigone"]
        A1["Old king<br/>(Creon)"] -->|"refuses to yield"| A2["Young generation<br/>(Haemon/Antigone)"]
        A2 --> A3["❌ Total<br/>catastrophe"]
    end
...
```mermaid
flowchart TB
    MELOS["416 BCE:<br/>Athens massacres Melos"] --> TW["415 BCE:<br/>Trojan Women staged"]
    TW --> MIRROR["Trojan women =<br/>Women of Melos"]
    TW --> GREEKS["Greek atrocities =<br/>Athenian atrocities"]
    MIRROR --> REJECT["Athens rejects<br/>the play"]
    GREEKS --> REJECT
    REJECT --> EXILE["Euripides exiles<br/>himself to Macedonia"]
    EXILE --> BACCH["Writes The Bacchae<br/>(his masterpiece)"]
    BACCH --> DEATH["Dies in exile"]
    DEATH --> WIN["The Bacchae wins<br/>first place posthumously"]
    style MELOS fill:#e74c3c,color:#fff
...
```mermaid
flowchart TB
    B["The Bacchae:<br/>Three Interpretations"]
    B --> R["Religious<br/>Fanaticism"]
    B --> S["Satire on Theatre<br/>& Democracy"]
    B --> E["Critique of<br/>Empire"]
    R --> R1["Faith overwhelms<br/>reason → violence"]
    S --> S1["Festival of Dionysus =<br/>'wild sex party'"]
    E --> E1["Old send young<br/>to die for glory"]
    E1 --> PREF["✅ Prof. Jiang's<br/>preferred reading"]
    style B fill:#2980b9,color:#fff
    style PREF fill:#27ae60,color:#fff
```
...
```mermaid
flowchart TB
    POW["Power"] --> HUB["Hubris<br/>(arrogance)"]
    HUB --> BAD["Bad<br/>judgement"]
    BAD --> VIO["Violence<br/>& suffering"]
    VIO --> REV["Revenge<br/>(endless cycle)"]
    REV --> VIO
    BAD -.-> DEM["Democracy<br/>distributes power"]
    DEM -.-> BREAK["No single person's<br/>hubris destroys all"]
...
```mermaid
flowchart LR
    A["Aeschylus<br/>CELEBRATION"] -->|"Democracy is<br/>a divine gift"| B["Sophocles<br/>DEFENCE"]
    B -->|"Monarchy breeds<br/>hubris & ruin"| C["Euripides<br/>CRITIQUE"]
    C -->|"Democracy must<br/>examine itself"| D["Complete<br/>Democratic<br/>Education"]
