...
tags: [lecture-summary, civilization, greece, homer, iliad, alphabet, polis, democracy, oral-culture, literary-culture, empathy, metaphor, dark-ages, troy, china-divergence, bronze-age-collapse]
...
```mermaid
flowchart LR
    H["Homer's Iliad<br/>~800–600 BCE"] --> R1["Still favourite<br/>book in China today"]
    P["Plato's Republic<br/>~380 BCE"] --> R2["Still transforms<br/>readers' worldviews"]
    T["Thucydides<br/>~400 BCE"] --> R3["Still read by<br/>generals to win wars"]
    R1 --> Q["How did one civilisation<br/>produce all this in<br/>~200 years?"]
    R2 --> Q
    R3 --> Q
...
```mermaid
flowchart TB
    subgraph Polis["The Three Engines of the Polis"]
        COMP["Competition<br/>Thousands of poleis<br/>constantly at war"]
        DIV["Diversity<br/>Varied geography →<br/>varied economies & cultures"]
        DEM["Democracy<br/>Poverty → everyone fights →<br/>everyone earns the right to speak"]
    end
    COMP --> INN["Rapid Innovation<br/>(Adapt or die)"]
    DIV --> EXP["Simultaneous Social<br/>Experiments"]
    DEM --> CIT["Every Citizen Thinks<br/>About Politics"]
    INN --> GC["Greek Creative Explosion"]
    EXP --> GC
    CIT --> GC
...
```mermaid
flowchart TB
    P["Pictograms<br/>(Pictures as contracts)<br/>📜 = 'I owe you 2 bushels'"] --> S["Symbols<br/>(Simplified pictures<br/>everyone recognises)"]
    S --> I["Ideograms<br/>(Symbols = ideas)<br/>☀ + 🌙 = 'day'<br/>Chinese today"]
    I --> SY["Syllabaries<br/>(Symbols = sounds)<br/>'moon' + 'sun' = 'monsoon'"]
    SY --> A["Alphabet<br/>(Symbols = consonants)<br/>M, S, T... = any word"]
    P -.->|"requires"| SC1["Professional Scribes<br/>(years of training)"]
    S -.->|"requires"| SC1
    I -.->|"requires"| SC1
    SY -.->|"requires"| SC1
    A -.->|"requires"| SC2["Anyone<br/>(weeks of learning)"]
...
```mermaid
flowchart LR
    subgraph Court["Court Poet — Serves King"]
        K1["Legitimise Authority<br/>(Prove the king<br/>was chosen by gods)"]
        K2["Create Cultural Identity<br/>(Unify people under<br/>a common story)"]
        K3["Cultural Differentiation<br/>(Define 'us' vs 'them')"]
    end
    subgraph Free["Homer — Serves Ordinary People"]
        H1["Entertainment<br/>(Only source of fun<br/>in a world without media)"]
        H2["Education<br/>(Learn to speak in<br/>polis by imitating Homer)"]
        H3["Edification<br/>(Transform who you are —<br/>how you see, feel, imagine)"]
    end
    H3 --> NT["New Theory of<br/>What It Means<br/>to Be Human"]
    style Court fill:#e74c3c,color:#fff
...
```mermaid
flowchart TB
    TW["The Trojan War<br/>(Popular Greek victory story)"]
    TW --> I1["Innovation 1:<br/>Perspective-Switching<br/>(Trojans are more heroic)"]
    TW --> I2["Innovation 2:<br/>Psychological Depth<br/>(Why characters really act)"]
    TW --> I3["Innovation 3:<br/>Metaphor<br/>(New tools for thought)"]
    I1 --> E["Empathy<br/>Seeing as others see"]
    I2 --> IM["Imagination<br/>Feeling as others feel"]
    I3 --> TH["Thought<br/>Connecting the unconnected"]
    E --> NT["New Theory of Humanity:<br/>To be human is to<br/>See, Feel, Think"]
    IM --> NT
    TH --> NT
    style NT fill:#2980b9,color:#fff
...
```mermaid
flowchart TB
    A1["Pride<br/>(Quarrels with Agamemnon)"] --> A2["Stubbornness<br/>(Refuses to fight)"]
    A2 --> A3["Guilt<br/>(Patroclus dies because of him)"]
    A3 --> A4["Rage<br/>(Kills Hector)"]
    A4 --> A5["Displaced Guilt<br/>(Tortures Hector's body)"]
    A5 --> A6["Shame<br/>(Priam kneels, kisses his hand)"]
    A6 --> A7["Grace<br/>(Returns the body — forgives himself)"]
    style A1 fill:#e74c3c,color:#fff
    style A3 fill:#e74c3c,color:#fff
    style A5 fill:#e74c3c,color:#fff
    style A6 fill:#2980b9,color:#fff
...
```mermaid
flowchart LR
    subgraph Greece["Greece's Path"]
        GD["Dark Ages<br/>(Elite class destroyed)"] --> GA["Alphabet adopted<br/>(Literacy democratised)"]
        GA --> GP["Poets at top<br/>(Homer as founder)"]
        GP --> GI["Creative Explosion<br/>(200 years)"]
    end
    subgraph China["China's Path"]
        CS["Stability<br/>(Elite class preserved)"] --> CM["Scholar officials<br/>(Literacy monopoly)"]
        CM --> CC["Classical Chinese invented<br/>(Literacy made harder)"]
        CC --> CE["Censorship<br/>(Control of thought)"]
        CE --> CD["Endurance<br/>(Not innovation)"]
    end
    style GI fill:#27ae60,color:#fff
