...
tags: [lecture-summary, geo-strategy, russia, putin, putinism, ukraine, warrior-culture, consumerism, liberal-democracy, hegel, fukuyama, marx, capitalism, communism, civilizational-identity, total-war, multipolar-world, neoliberalism]
...
```mermaid
flowchart TB
    Q["Why is Putin calling<br/>for total war?"]
    Q --> E1["Manpower shortage<br/>Russia needs soldiers"]
    Q --> E2["NATO escalation<br/>Macron, conscription"]
    Q --> E3["Territorial expansion<br/>Moldova, Baltics"]
    Q --> E4["Civilisational renewal<br/>War as gym for society"]
    E1 --> R1["Conventional analysis"]
    E2 --> R2["Conventional analysis"]
    E3 --> R3["Conventional analysis"]
    E4 --> R4["Prof. Jiang's argument"]
...
```mermaid
flowchart TB
    R["Russia's Three Crises"]
    R --> C["Corruption<br/>Most corrupt in Europe"]
    R --> A["Alcoholism<br/>1 in 6 males"]
    R --> F["Fertility Crisis<br/>Rate: 1.5"]
    C --> C1["Rich steal & flee<br/>to Dubai / Europe / US"]
    C --> C2["GDP lower than<br/>South Korea or Texas"]
    A --> A1["1 in 3 deaths<br/>alcohol-related"]
    A --> A2["Rate of young men<br/>dying is rising"]
    F --> F1["Below 2.1<br/>replacement level"]
    F --> F2["Population declining<br/>since 2000"]
    C2 --> D["Russia ceases<br/>to exist as a nation"]
    A2 --> D
    F2 --> D
...
```mermaid
flowchart LR
    T["Capitalism<br/>(Thesis)"]
    T -->|"Too extreme —<br/>alienates workers"| AT["Communism<br/>(Antithesis)"]
    AT -->|"Too extreme —<br/>no individual rights"| S["Liberal Democracy<br/>(Synthesis)"]
    S --> E["'The End of History'<br/>— Fukuyama"]
...
```mermaid
flowchart TB
    C["Consumerism begins"]
    C --> B["Buy things<br/>(house, furniture, goods)"]
    B --> SM["Post on social media<br/>for status comparison"]
    SM --> COMP["Escalating competition<br/>for prestige"]
    COMP --> DEBT["Go into debt<br/>to buy more"]
    DEBT --> ISO["Atomisation and isolation<br/>you hate each other"]
    ISO --> EL["Economic logic<br/>replaces human logic"]
    EL --> SLAVE["Perfection of slavery:<br/>enslaved by choice"]
    SLAVE --> NOREB["Never rebel —<br/>don't know they're enslaved"]
    style SLAVE fill:#e74c3c,color:#fff
    style NOREB fill:#e74c3c,color:#fff
...
```mermaid
flowchart LR
    subgraph Consumer["Consumer — Liberal Democracy"]
        direction TB
        A1["Acts alone"]
        A2["Preserves only their life"]
        A3["Economic logic only"]
        A4["Coward — will not sacrifice"]
        A5["Lacks imagination"]
        A6["Atomised and alienated"]
    end
    subgraph Warrior["Warrior — Putinism"]
        direction TB
        B1["Acts with others"]
        B2["Shapes history through courage"]
        B3["Moral and civilisational logic"]
        B4["Willing to sacrifice — even die"]
        B5["Imagination and agency"]
        B6["Unified and purposeful"]
    end
    Consumer -->|"Putinism replaces"| Warrior
    style Consumer fill:#e74c3c,color:#fff
    style Warrior fill:#27ae60,color:#fff
```
...
```mermaid
flowchart TB
    P["Putinism:<br/>Continuous Contained War"]
    P --> STR["Strengths"]
    P --> WEAK["Fatal Contradictions"]
    STR --> S1["Disciplines society"]
    STR --> S2["Unifies population"]
    STR --> S3["Beats consumer<br/>culture easily"]
    STR --> S4["Creates structure,<br/>meaning, purpose"]
    WEAK --> W1["The King Problem"]
    WEAK --> W2["The Pyramid Scheme"]
    W1 --> W1A["Warrior culture needs<br/>a great leader"]
    W1 --> W1B["When Putin dies:<br/>civil war between generals"]
    W2 --> W2A["Each war's costs force<br/>the next conquest"]
    W2 --> W2B["Nazi Germany model:<br/>invaded Russia for resources"]
    W2 --> W2C["Short-term success,<br/>long-term disaster"]
