...
tags: [lecture-summary, geo-strategy, iran, irgc, raisi, game-theory, succession-politics, intelligence]
...
```mermaid
flowchart TB
    Q["How did Raisi die?<br/>May 19, 2024"]
    Q --> P1["Possibility 1:<br/>Accident"]
    Q --> P2["Possibility 2:<br/>Foreign Adversary"]
    Q --> P3["Possibility 3:<br/>Internal Enemy"]
    P1 --> V1["Most probable —<br/>will go down in history"]
    P2 --> V2["Weakest —<br/>fails both criteria"]
    P3 --> V3["Analytically strongest —<br/>generates testable predictions"]
...
```mermaid
flowchart TB
    K["Ayatollah Khamenei dies<br/>(expected within 5 years)"]
    K --> PA["Path A: Raisi<br/>becomes Supreme Leader"]
    K --> PB["Path B: Mojtaba Khamenei<br/>becomes Supreme Leader"]
    PA --> RA["Builds own power base"]
    RA --> RB["Reduces IRGC power<br/>to establish authority"]
    RB --> RC["IRGC THREATENED"]
    PB --> MA["Unpopular — no<br/>independent support"]
    MA --> MB["Only in power because<br/>IRGC installed him"]
    MB --> MC["IRGC POWER PRESERVED"]
    RD["Raisi dies<br/>May 19, 2024"] -.->|"Path A<br/>eliminated"| PA
...
```mermaid
flowchart TB
    R["1979 Revolution<br/>Shah overthrown"] --> P["Problem: Army loyal<br/>to the Shah"]
    P --> S["Solution: Create a<br/>parallel armed force"]
    S --> IRGC["IRGC Founded"]
    IRGC --> D1["Mandate 1:<br/>Protect the revolution<br/>domestically"]
    IRGC --> D2["Mandate 2:<br/>Export the revolution<br/>across the Middle East"]
    D2 --> AOR["Axis of Resistance:<br/>Hamas, Hezbollah,<br/>Houthis, Syria, Iraq"]
    style IRGC fill:#2980b9,color:#fff
...
```mermaid
flowchart LR
    E["US Embassy<br/>Hostage Crisis<br/>November 1979"] --> S["Students piece<br/>together shredded<br/>documents"]
    S --> D["Discover: Embassy<br/>was the real centre<br/>of power in Iran"]
    D --> R["Radicalisation:<br/>America is the<br/>Great Satan"]
    R --> L1["Hossein Dehghan<br/>Minister of Defence<br/>2013-2017"]
    R --> L2["Mohammed Ali Jafari<br/>Head of IRGC<br/>2007-2019"]
    R --> L3["Mohammed Bagheri<br/>Chief of Staff<br/>(current)"]
...
```mermaid
flowchart TB
    W["Iran-Iraq War<br/>1980-1988"] --> B["Basij: teenage<br/>volunteers with<br/>rifles and keys"]
    W --> O["Outcome: IRGC<br/>defends revolution<br/>against superpowers"]
    O --> P1["Unlimited government<br/>funding"]
    O --> P2["Dominant military<br/>institution"]
    O --> P3["Strategic lesson:<br/>fanaticism defeats<br/>superior firepower"]
    P3 --> C["Conclusion: take<br/>the fight to enemies<br/>BEFORE they come"]
    C --> AOR["Fund the Axis<br/>of Resistance"]
    style W fill:#e74c3c,color:#fff
    style P3 fill:#27ae60,color:#fff
    style AOR fill:#2980b9,color:#fff
```
...
```mermaid
flowchart TB
    A["IRGC monopolises<br/>the economy"] --> B["Economy stagnates,<br/>people become poor"]
    B --> C["Public protests<br/>erupt"]
    C --> D["Regime depends on<br/>IRGC to crush protests"]
    D --> E["IRGC becomes<br/>more powerful"]
    E --> A
...
```mermaid
flowchart TB
    D["Raisi eliminated<br/>(May 2024)"] --> MC["Military class<br/>controls policy"]
    MC --> P1["Accelerate nuclear<br/>programme"]
    MC --> P2["Escalate proxy<br/>wars"]
    MC --> P3["Disrupt shipping<br/>lanes"]
    MC --> P4["Terror campaigns<br/>against US allies"]
    P1 --> G["Goal: Provoke US<br/>into invading Iran"]
    P2 --> G
    P3 --> G
    P4 --> G
    G --> W["IRGC fights on<br/>home terrain —<br/>the only war<br/>they can win"]
...
```mermaid
flowchart TB
    US["United States"] --> USM["Pentagon:<br/>Hubris drives<br/>willingness to fight"]
    US --> USP["Politicians:<br/>Christian Zionism +<br/>donor pressure"]
    US --> USC["Civilians:<br/>Don't want war"]
    IR["Iran"] --> IRM["IRGC:<br/>Fanaticism drives<br/>provocation strategy"]
    IR --> IRP["Politicians:<br/>Counsel restraint<br/>(Raisi eliminated)"]
    IR --> IRC["Civilians:<br/>Don't want war"]
    USM --> WAR["War"]
    USP --> WAR
    IRM --> WAR
    USC -.->|"No institutional<br/>power to stop it"| WAR
    IRP -.->|"Sidelined or<br/>eliminated"| WAR
    IRC -.->|"Protests crushed<br/>by IRGC"| WAR
    style WAR fill:#e74c3c,color:#fff
    style USC fill:#27ae60,color:#fff
    style IRC fill:#27ae60,color:#fff
    style IRP fill:#27ae60,color:#fff
```
