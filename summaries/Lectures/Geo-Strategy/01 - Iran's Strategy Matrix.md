...
tags: [lecture-summary, geo-strategy, iran, asymmetrical-warfare, middle-east, military-strategy]
...
```mermaid
flowchart LR
    A["US-Israel Military Dominance<br/>Technology + HUMINT + SIGINT"] --> B["Can Strike Anywhere<br/>(Damascus embassy, Soleimani)"]
    B --> C["Does dominance = victory?"]
    C --> D["Conventional thinking: YES"]
    C --> E["Prof. Jiang: NO"]
    E --> F["Asymmetrical warfare<br/>reverses the equation"]
...
```mermaid
flowchart LR
    subgraph OF["Open Field"]
        J1["Jack<br/>(armour + machine gun)"] -->|"always wins"| P1["Prof. Jiang<br/>(nothing)"]
    end
    subgraph DF["Dark Forest"]
        P2["Prof. Jiang<br/>(traps, terrain knowledge)"] -->|"controls engagement"| J2["Jack<br/>(advantages neutralised)"]
    end
    OF -.->|"Jack enters anyway<br/>— HUBRIS"| DF
...
```mermaid
flowchart TB
    MC["2002 Millennium Challenge<br/>US War Simulation"]
    MC --> S1["Simulation 1<br/>Asymmetrical tactics ALLOWED"]
    MC --> S2["Simulation 2<br/>Asymmetrical tactics BANNED"]
    S1 --> R1["IRAN WINS"]
    S2 --> R2["US WINS"]
    R1 --> C["US response:<br/>'This is CHEATING'"]
    C --> H["HUBRIS<br/>Change the rules,<br/>not the doctrine"]
    H --> V["Vietnam<br/>Same inflexibility<br/>Same outcome"]
    style R1 fill:#27ae60,color:#fff
    style R2 fill:#e74c3c,color:#fff
    style H fill:#e74c3c,color:#fff
    style V fill:#e74c3c,color:#fff
    style MC fill:#2980b9,color:#fff
```
...
```mermaid
flowchart TB
    M["Iran Strategy Matrix<br/>Every action must serve<br/>ALL four goals simultaneously"]
    M --> G1["1. Unite the Population<br/>1953 coup — grievance<br/>transcends internal divisions"]
    M --> G2["2. Build Alliances<br/>Axis of Resistance (Layer 1)<br/>Russia + China (Layer 2)"]
    M --> G3["3. Win Global Opinion<br/>Keep Gaza in spotlight<br/>Demonstrate restraint"]
    M --> G4["4. Weaken the Enemy<br/>US-Israel friction<br/>NATO reluctance<br/>American civil unrest"]
    G1 --> OTP["Operation True Promise<br/>Tests all four simultaneously"]
    G2 --> OTP
    G3 --> OTP
    G4 --> OTP
    ...
    style OTP fill:#2980b9,color:#fff
```
...
```mermaid
flowchart TB
    subgraph UKR["Ukraine War — Rules in Practice"]
        U1["NATO provides weapons<br/>+ financing"] --> U2["Russia tolerates<br/>material support"]
        U3["NATO does NOT<br/>send troops"] --> U4["Russia does NOT<br/>strike NATO territory"]
    end
    subgraph IRAN["Projected Iran War — Rules"]
        I1["No tactical nuclear<br/>weapons used"] --> I2["Russia enforces via<br/>nuclear deterrence"]
        I3["Russia/China provide<br/>limited assistance"] --> I4["Within limits<br/>US can accept"]
        I5["Strategic ambiguity<br/>maintained"] --> I6["No public declarations<br/>of alliance"]
    end
    UKR -->|"precedent"| IRAN
...
flowchart LR
    US["United States"]
    IS["Israel"]
    US -->|"constrains:<br/>prevents regional war,<br/>stops disproportionate response"| IS
    IS -->|"leverages:<br/>Israel lobby,<br/>shapes US foreign policy"| US
    US -->|"provides:<br/>military aid,<br/>diplomatic cover,<br/>intelligence"| IS
    IS -->|"serves as:<br/>regional enforcer,<br/>strategic ally"| US
...
```mermaid
flowchart TB
    E["American Empire<br/>Under Stress"]
    E --> C1["Overextension<br/>Too many wars at once<br/>(Ukraine, Iran, global commitments)"]
    E --> C2["Debt<br/>Wars drain resources<br/>faster than economy grows"]
    E --> C3["Civil Unrest<br/>Public turns against<br/>foreign wars (Vietnam repeat)"]
    C1 --> C2
    C2 --> C3
    C3 --> D["Unipolar → Multipolar"]
    D --> R1["Germany dominates Europe"]
    D --> R2["Israel dominates Middle East"]
    D --> R3["Japan dominates East Asia"]
