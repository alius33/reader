...
tags: [lecture-summary, secret-history, evil, transgression, ritual-sacrifice, game-theory, kant, hegel, plato, dante, gnosticism, sparta, thebes, macedonia, synchronicity, cohesion, monad, geist]
...
```mermaid
flowchart TB
    RS["Ritual Sacrifice<br/>(public atrocity)"] --> T["Crosses ultimate taboo"]
    T --> NR["No retreat possible<br/>(river behind your back)"]
    NR --> U["Group unity forced"]
    U --> I["Outsiders' hatred<br/>reinforces isolation"]
    I --> C["Only safe place<br/>is within the group"]
    C --> P["Absolute commitment<br/>= Power"]
    style RS fill:#e74c3c,color:#fff
    style NR fill:#e74c3c,color:#fff
...
```mermaid
flowchart LR
    A["Strangers<br/>(no common language)"] --> B["Common<br/>Language"]
    B --> C["Shared<br/>Stories"]
    C --> D["Founding<br/>Myth"]
    D --> E["Religion<br/>& Rituals"]
    E --> F["Leadership<br/>by Sacrifice"]
    F --> G["Cohesion"]
    G --> H["Synchronicity<br/>(Hive Mind)"]
...
```mermaid
flowchart LR
    S["Sparta<br/>(coercive, elite-only)"] --> T["Thebes<br/>(voluntary, open)"]
    T --> M["Macedonia<br/>(improved, global conquest)"]
    S -.->|"System copied"| T
    T -.->|"System copied"| M
...
```mermaid
flowchart TB
    A["Age 5-6<br/>Taken from home"] --> B["Hazing Phase<br/>Beaten by older boys"]
    B --> C["Solidarity<br/>through shared suffering"]
    C --> D["Age 11-13<br/>Mentored by soldiers"]
    D --> E["Hive Mind<br/>through intimate bonding"]
    E --> F["Age 18<br/>Graduation Mission"]
    F --> G["Kill a helot<br/>at night"]
    G --> H["Ultimate Cohesion<br/>through shared transgression"]
...
```mermaid
flowchart TB
    A["Million individuals<br/>competing"] --> B{"Best strategy?"}
    B --> C["Visible coordination<br/>(family, religion, ethnicity)"]
    B --> D["Secret coordination<br/>(transgression)"]
    C --> E["Others see it →<br/>Arms race of coordination"]
    E --> F["Advantage<br/>neutralised"]
    D --> G["Shared guilt<br/>forces secrecy"]
    G --> H["Secrecy forces trust"]
    H --> I["Unbeatable<br/>advantage"]
...
```mermaid
flowchart TB
    A["Pranks<br/>(low risk, small rush)"] --> B["Petty Theft<br/>(fear → liberation)"]
    B --> C["Greater Rule-Breaking<br/>(addictive escalation)"]
    C --> D["Serious Transgression<br/>(deep bonding through guilt)"]
    D --> E["Ultimate Taboos<br/>(irreversible commitment)"]
    A -.->|"Cohesion"| A2["Low"]
    B -.->|"Cohesion"| B2["Moderate"]
    C -.->|"Cohesion"| C2["High"]
    D -.->|"Cohesion"| D2["Very High"]
    E -.->|"Cohesion"| E2["Absolute"]
...
```mermaid
flowchart TB
    K["Kant's 3 Problems"] --> Q1["What is the<br/>noumena?"]
    K --> Q2["Who gave us<br/>our filters?"]
    K --> Q3["How do we see<br/>the same world?"]
    Q1 --> H1["Noumena =<br/>the Geist"]
    Q2 --> H2["Geist gave us<br/>our filters"]
    Q3 --> H3["We all access<br/>the same Geist"]
    H1 --> G["Hegel's Geist<br/>(Ghost + Geyser + Gist)"]
    H2 --> G
    H3 --> G
    G --> N["But where does<br/>the Geist come from?"]
    N --> P["Plato & Gnosticism:<br/>The Monad"]
...
```mermaid
flowchart TB
    M["The Monad<br/>(The One / Nous)"] -->|"Emanation<br/>(breathing/vibrating)"| D1["Dyads<br/>(pairs of forces)"]
    D1 --> D2["Further<br/>Realities"]
    D2 --> D3["More<br/>Layers"]
    D3 --> E["Earth<br/>(outermost shell)"]
    P["Plato's Path<br/>Knowledge / Geometry"] -.->|"Return"| M
    DA["Dante's Path<br/>Love"] -.->|"Return"| M
    style M fill:#27ae60,color:#fff
    style E fill:#e74c3c,color:#fff
    style P fill:#2980b9,color:#fff
