...
tags: [lecture-summary, secret-history, power, money, individualism, nation-state, monotheism, alchemy, kant, central-banking, education, eudaimonia, polytheism, brainwashing]
...
```mermaid
flowchart LR
    A["Present<br/>Study geopolitics"] --> B["Build ethical/<br/>analytical model"]
    B --> C["Future<br/>Make predictions"]
    C --> D{"Predictions<br/>correct?"}
    D -->|Yes| E["Past<br/>Reveal secret history"]
    D -->|No| B
    E --> F["Liberation<br/>See through power"]
...
```mermaid
flowchart LR
    A["Gold deposited<br/>in bank"] --> B["Receipt issued<br/>to depositor"]
    A --> C["Receipt issued<br/>to borrower"]
    B --> D["Used as money<br/>in trade"]
    C --> E["Used as money<br/>in trade"]
    D --> F["Same gold backs<br/>TWO receipts"]
    E --> F
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TB
    B["Bank with<br/>$5M gold"] --> R["Issues $10M+<br/>in receipts"]
    R --> RISK1["Risk 1: Bank Run<br/>Everyone demands<br/>gold at once"]
    R --> RISK2["Risk 2: Defaulting Kings<br/>Refuse to repay<br/>war loans"]
    RISK1 --> SOL["Solution: Form<br/>Banking CARTELS"]
    RISK2 --> SOL
    SOL --> IM["Intermarriage across<br/>Europe's banking families"]
    IM --> P1["Pool gold to<br/>survive bank runs"]
    IM --> P2["Fund rivals to<br/>overthrow defaulting kings"]
    P1 --> CB["CENTRAL BANKING<br/>Controls the world"]
    P2 --> CB
    style B fill:#2980b9,color:#fff
    style RISK1 fill:#e74c3c,color:#fff
...
```mermaid
flowchart TB
    A["Banks create<br/>money from nothing"] --> B["Too much money<br/>in the system"]
    B --> C["People stop<br/>working hard"]
    C --> D["Crisis or war<br/>destroys money"]
    D --> E["Scarcity<br/>restored"]
    E --> F["People work<br/>hard again"]
    F --> A
...
```mermaid
flowchart TB
    subgraph P["Polytheism (Worldview 1)"]
        P1["Gods control fate"] --> P2["No individual agency"]
        P2 --> P3["Eudaimonia:<br/>live your best today"]
        P3 --> P4["Collective action<br/>possible"]
        P4 --> P5["Flourishing"]
    end
    subgraph M["Modern Science (Worldview 2)"]
        M1["Synapses control fate"] --> M2["Full individual agency"]
        M2 --> M3["Individual responsibility<br/>for all problems"]
        M3 --> M4["No collective action<br/>possible"]
        M4 --> M5["Isolation & slavery"]
    end
...
```mermaid
flowchart TB
    A["Polytheistic Gods"] --> B["Openly capricious,<br/>vengeful, flawed"]
    B --> C["Everyone KNOWS<br/>power is arbitrary"]
    C --> D["No illusion of<br/>benevolent authority"]
    D --> E["Response:<br/>EUDAIMONIA<br/>Live fully today"]

    F["Modern Authority"] --> G["Claims to be<br/>benevolent & rational"]
    G --> H["People TRUST<br/>that power serves them"]
    H --> I["Illusion of<br/>legitimate authority"]
    I --> J["Response:<br/>OBEDIENCE<br/>Serve the system"]
...
```mermaid
flowchart LR
    A["Sparta<br/>~800 BC"] --> D["All three were<br/>WAR SOCIETIES"]
    B["Aztecs<br/>~1400s AD"] --> D
    C["Prussia<br/>~1700s AD"] --> D
    D --> E["Modern world<br/>copies Prussia"]
...
```mermaid
flowchart TB
    A["Child with parent<br/>(secure, questioning)"] --> B["Separated from parent<br/>(school begins)"]
    B --> C["Child feels insecure,<br/>anxious, afraid"]
    C --> D["Teacher becomes<br/>authority figure"]
    D --> E["Whatever teacher says<br/>is accepted as true"]
    E --> F["Nation state identity<br/>implanted"]
    F --> G["Obedient citizen<br/>who serves the state"]
...
```mermaid
flowchart TB
    M["Monotheism<br/>(one true God)"] --> A["Money<br/>(infinite → appears scarce)"]
    M --> B["The Individual<br/>(didn't exist → appears natural)"]
    M --> C["The Nation State<br/>(fiction → appears real)"]
    A --> P["POWER = ALCHEMY<br/>Turning nothing<br/>into everything"]
    B --> P
    C --> P
...
```mermaid
flowchart TB
    subgraph Ancient["Ancient World (Polytheism)"]
        A1["Collective identity"] --> A2["Eudaimonia<br/>(flourishing)"]
        A2 --> A3["Cultural greatness<br/>(Homer, Plato)"]
    end
    subgraph Modern["Modern World (Monotheism)"]
        B1["Individual identity"] --> B2["Pleasure-seeking<br/>(consumption)"]
        B2 --> B3["Misery & isolation<br/>(video games, porn)"]
    end
    R["Monotheism"] -->|"revolution in<br/>human thought"| Modern
