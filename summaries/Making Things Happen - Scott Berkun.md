...
date: 2026-03-25
type: book-summary
tags: [book-summary, project-management, execution, leadership, decision-making, politics]
author: Scott Berkun
...
```mermaid
flowchart LR
    A[Explore] --> B[Expand Options]
    B --> C[Checkpoint: Evaluate]
    C --> D[Narrow Options]
    D --> E[Checkpoint: Commit]
    E --> F[Specify & Build]
...
```mermaid
flowchart TD
    GP[Granted Power] --> Compliance
    EP[Earned Power] --> Commitment
    GP --> Room[Gets You in the Room]
    EP --> Listen[Ensures People Listen]
    Compliance -.-> |Alone, not enough| Fragile[Fragile Authority]
    Commitment -.-> |Combined| Durable[Durable Leadership]
    Room -.-> |Combined| Durable
    Listen -.-> |Combined| Durable
    style GP fill:#e74c3c,color:#fff
    style EP fill:#27ae60,color:#fff
    style Durable fill:#2980b9,color:#fff
```
...
```mermaid
flowchart LR
    A[Position A] --> C[Impasse]
    B[Position B] --> C
    C --> D[Reframe: What Are the Interests?]
    D --> E[Interest A]
    D --> F[Interest B]
    E --> G[Shared Solution]
    F --> G
...
```mermaid
flowchart TD
    J[PM Judgement] --> |Shapes| P[Process]
    P --> |Supports| D[Decisions]
    D --> |Drive| O[Outcomes]
    J --> |Overrides when needed| D
    P -.-> |Without judgement| B[Bureaucracy]
    J -.-> |Without process| C[Chaos]
