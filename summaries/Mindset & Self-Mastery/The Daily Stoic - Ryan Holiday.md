...
date: 2026-03-25
type: book-summary
tags: [book-summary, stoicism, daily-practice, philosophy, marcus-aurelius, seneca, epictetus]
author: Ryan Holiday
...
```mermaid
flowchart LR
    J["January<br>Clarity"] --> F["February<br>Desires"] --> MA["March<br>Awareness"] --> AP["April<br>Bias"]
    AP --> MY["May<br>Right Action"] --> JU["June<br>Obstacles"] --> JL["July<br>Duty"] --> AU["August<br>Pragmatism"]
    AU --> S["September<br>Fortitude"] --> O["October<br>Virtue"] --> N["November<br>Acceptance"] --> D["December<br>Mortality"]

    subgraph PERC ["Discipline of Perception"]
        J
        F
        MA
        AP
    end
    subgraph ACT ["Discipline of Action"]
        MY
        JU
        JL
        AU
    end
    subgraph WILL ["Discipline of Will"]
        S
        O
        N
        D
    end
    style PERC fill:#3498db,color:#fff
    style ACT fill:#27ae60,color:#fff
    style WILL fill:#8e44ad,color:#fff
```
...
```mermaid
flowchart TD
    subgraph THREE ["Three Voices, One Philosophy"]
        M["Marcus Aurelius<br>(Emperor)<br>Duty, impermanence,<br>self-discipline"]
        S["Seneca<br>(Senator)<br>Time, wealth,<br>practical wisdom"]
        E["Epictetus<br>(Former Slave)<br>Control, choice,<br>resilience"]
    end
    M --> C["Common Core:<br>Focus on what you control.<br>Act with virtue.<br>Accept what you cannot change."]
    S --> C
    E --> C
...
```mermaid
flowchart TD
    S["Stoic Practice"] --> P["Discipline of Perception"]
    S --> A["Discipline of Action"]
    S --> W["Discipline of Will"]
    P --> P1["See things as they are<br>not as you fear or wish"]
    A --> A1["Act with virtue<br>for the common good"]
    W --> W1["Accept what you<br>cannot change"]
...
```mermaid
flowchart TD
    E["Event occurs"] --> Q1{"Can I change it?"}
    Q1 -->|Yes| A["Discipline of Action<br>Act with virtue"]
    Q1 -->|No| Q2{"Can I change how<br>I see it?"}
    Q2 -->|Yes| P["Discipline of Perception<br>Reframe and see clearly"]
    Q2 -->|No| W["Discipline of Will<br>Accept and endure"]
    A --> R["Peace through effort"]
    P --> R2["Peace through clarity"]
    W --> R3["Peace through acceptance"]
    style A fill:#27ae60,color:#fff
    style P fill:#3498db,color:#fff
    style W fill:#8e44ad,color:#fff
```
...
```mermaid
flowchart LR
    I["Impression<br>(Raw event)"] --> J["Judgment<br>(Your interpretation)"]
    J --> EM["Emotion<br>(Your feeling)"]
    EM --> AC["Action<br>(Your response)"]
    J -.->|"INTERVENE HERE<br>(Prosoche)"| ALT["Pause & examine:<br>Is this judgment accurate?"]
    ALT --> NEW["Revised response<br>(Chosen, not reactive)"]
...
```mermaid
flowchart TD
    subgraph VIRTUES ["The Four Stoic Virtues"]
        W["Wisdom<br>(See clearly)"]
        C["Courage<br>(Act bravely)"]
        J["Justice<br>(Treat fairly)"]
        T["Temperance<br>(Practise moderation)"]
    end
    W --> L["The Good Life:<br>Virtue is the only<br>true good"]
    C --> L
    J --> L
    T --> L
...
```mermaid
flowchart LR
    W["Wisdom"] -->|"Informs"| C["Courage"]
    C -->|"Enables"| J["Justice"]
    J -->|"Moderated by"| T["Temperance"]
    T -->|"Guides"| W
    style W fill:#3498db,color:#fff
    style C fill:#e74c3c,color:#fff
    style J fill:#27ae60,color:#fff
...
```mermaid
flowchart LR
    MR["Morning<br>Read entry<br>Set intention"] --> DAY["Throughout Day<br>Practise at<br>trigger points"]
    DAY --> ER["Evening<br>Review day<br>Calibrate"]
    ER --> MR
    style MR fill:#3498db,color:#fff
    style DAY fill:#27ae60,color:#fff
    style ER fill:#8e44ad,color:#fff
```
...
```mermaid
flowchart TD
    CR["CRISIS HITS"] --> P["Step 1: PERCEPTION<br>Strip away panic.<br>What is ACTUALLY happening?"]
    P --> A["Step 2: ACTION<br>What can I DO?<br>Only what is in your control"]
    A --> W["Step 3: WILL<br>What must I ACCEPT?<br>What is beyond your power"]
    W --> R["RESPONSE:<br>Clear perception<br>+ Purposeful action<br>+ Acceptance of fate<br>= Equanimity under fire"]
