...
date: 2026-03-25
type: book-summary
tags: [book-summary, generalism, breadth, adaptability, analogical-thinking, learning, innovation, forecasting]
...
```mermaid
flowchart LR
    A[Broad Sampling] --> B[Multiple Skills]
    B --> C[Cross-Domain Patterns]
    C --> D[Natural Specialisation]
    D --> E[Flexible Mastery]
...
```mermaid
flowchart TD
    A[Premodern Cognition] --> B[Concrete Reasoning]
    B --> C[Each Problem Is Unique]
    D[Modern Cognition] --> E[Abstract Classification]
    E --> F[Structural Patterns Across Domains]
    F --> G[Far Transfer Possible]
    G --> H[Range Becomes Powerful]
...
```mermaid
flowchart TD
    A[Blocked Practice] --> B[Fast Visible Progress]
    B --> C[Shallow Encoding]
    C --> D[Poor Transfer & Retention]
    E[Interleaved Practice] --> F[Slow Visible Progress]
    F --> G[Deep Encoding]
    G --> H[Strong Transfer & Retention]
...
```mermaid
flowchart LR
    A[Surface Analogy] --> B[Looks Similar]
    B --> C[Often Misleading]
    D[Structural Analogy] --> E[Works the Same Way]
    E --> F[Genuine Insight]
    G[Multiple Analogies] --> H[Deep Structure Visible]
    H --> I[Breakthrough Solution]
...
```mermaid
flowchart TD
    A[New Path] --> B{Experiment & Gather Evidence}
    B -->|Good Fit| C[Commit & Deepen]
    B -->|Poor Fit| D[Quit & Try Another]
    D --> A
    C --> E[High Match Quality]
    E --> F[Performance + Satisfaction]
    style E fill:#4a90d9,color:#fff
...
```mermaid
flowchart LR
    A[Giving Up] --> B[Abandoning Effort]
    B --> C[No Search Continues]
    D[Strategic Quitting] --> E[Reallocating Effort]
    E --> F[Better Match Found]
    F --> G[Higher Performance + Fulfilment]
    style C fill:#d94a4a,color:#fff
...
```mermaid
flowchart LR
    A[Try Possible Self A] --> B{Energy? Fit?}
    B -->|No| C[Try Possible Self B]
    C --> D{Energy? Fit?}
    D -->|No| E[Try Possible Self C]
    E --> F{Energy? Fit?}
    F -->|Yes| G[Commit & Build]
    B -->|Yes| G
    D -->|Yes| G
    style G fill:#4a90d9,color:#fff
```
...
```mermaid
flowchart TD
    A[Deep Expertise] --> B[Pattern Recognition]
    B --> C{Environment Change?}
    C -->|No| D[Expert Advantage]
    C -->|Yes| E[Einstellung Effect]
    E --> F[Familiar Solution Blocks Better One]
    C -->|Yes| G[Cognitive Entrenchment]
    G --> H[Cannot Reframe the Problem]
    F --> I[Expert Performs Worse Than Novice]
    H --> I
    style A fill:#4a90d9,color:#fff
...
flowchart LR
    subgraph Innovation
        direction TB
        A[Old Technology A] --> D[Novel Combination]
        B[Old Technology B] --> D
        C[Old Technology C] --> D
        D --> E[Breakthrough Product]
    end
    subgraph Failure
        direction TB
        F[Cutting-Edge Tech] --> G[Expensive & Fragile]
        G --> H[Commercial Irrelevance]
    end
    style E fill:#4a90d9,color:#fff
...
```mermaid
flowchart LR
    A[Curiosity-Driven Research] --> B[Broad Knowledge Base]
    B --> C[Unexpected Connection]
    C --> D[Applied Breakthrough]
    E[Application-Only Funding] --> F[Narrow Knowledge Base]
    F --> G[Incremental Improvement Only]
...
```mermaid
flowchart TB
    A[Range: Breadth of Experience] --> B[Analogical Thinking]
    A --> C[Match Quality Discovery]
    B --> D[Outsider Advantage]
    B --> E[Cross-Domain Innovation]
    C --> F[Fulfilled Career]
    D --> G[Novel Solutions to Stuck Problems]
    E --> G
    G --> H[Breakthrough Impact]
    F --> H
    style A fill:#4a90d9,color:#fff
    style H fill:#4a90d9,color:#fff
```
...
```mermaid
flowchart LR
    subgraph Kind["Kind Environments"]
        K1[Chess]
        K2[Golf]
        K3[Classical Music]
    end
    subgraph Wicked["Wicked Environments"]
        W1[Leadership]
        W2[Strategy]
        W3[Innovation]
    end
    Kind -->|Clear rules, instant feedback| S[Specialisation Wins]
    Wicked -->|Ambiguous rules, delayed feedback| R[Range Wins]
