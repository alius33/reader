...
date: 2026-03-25
type: book-summary
tags: [book-summary, edge, advantage, perception, bias, stereotypes, entrepreneurship, self-mastery, influence, career, mindset]
author: Laura Huang
...
```mermaid
flowchart LR
    A["ENRICH<br>(Know your value)"] --> B["DELIGHT<br>(Surprise positively)"]
    B --> C["GUIDE<br>(Shape perceptions)"]
    C --> D["EFFORT<br>(Work strategically)"]
    D --> E["YOUR EDGE"]
...
```mermaid
flowchart TD
    A["Hard Work + Talent"] --> B{"Does the world<br>work on merit alone?"}
    B -->|"Myth says YES"| C["Work harder<br>(leads to burnout)"]
    B -->|"Reality says NO"| D["Perception shapes<br>outcomes"]
    D --> E["Understand how<br>you're perceived"]
    E --> F["Strategically shape<br>those perceptions"]
    F --> G["Create Your Edge"]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A["Expected Pattern<br>(What everyone does)"] --> B["Pattern Interrupt<br>(Positive surprise)"]
    B --> C["Attention Shift<br>(Autopilot → Active)"]
    C --> D["Positive Association<br>(Attributed to you)"]
    D --> E["Your Value Gets<br>Full Attention"]
    A --> F["No Interrupt<br>(Meeting expectations)"]
    F --> G["Autopilot Evaluation<br>(Quick, shallow judgment)"]
    G --> H["Your Value<br>Gets Filtered"]
    style B fill:#27ae60,color:#fff
...
```mermaid
flowchart TD
    A["Perceived Constraint<br>(youth, gender, accent,<br>background)"] --> B{"Do you address it?"}
    B -->|"No — ignore it"| C["Constraint operates<br>in background"]
    C --> D["Others' biases<br>shape their evaluation"]
    D --> E["You lose control<br>of the narrative"]
    B -->|"Yes — acknowledge<br>and flip it"| F["Constraint becomes<br>visible and named"]
    F --> G["You reframe it<br>as an advantage"]
    G --> H["Others see your<br>self-awareness"]
    H --> I["Perceived weakness<br>becomes perceived strength"]
...
```mermaid
flowchart LR
    A["Hard Work<br>(Input)"] --> B{"Is effort<br>strategic?"}
    B -->|"No"| C["Brute effort<br>(Invisible, exhausting)"]
    C --> D["Work not seen<br>or valued"]
    B -->|"Yes"| E["Aligned with<br>what matters"]
    E --> F["Visible to<br>decision-makers"]
    F --> G["Timed for<br>maximum impact"]
    G --> H["Edge created<br>through effort"]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A["ENRICH<br>(Self-knowledge +<br>value to others)"] --> B["DELIGHT<br>(Positive surprise +<br>pattern interrupt)"]
    B --> C["GUIDE<br>(Shape perceptions +<br>flip constraints)"]
    C --> D["EFFORT<br>(Strategic, visible,<br>aligned work)"]
    D --> A
    A --> E["YOUR EDGE<br>(Sustainable advantage)"]
    B --> E
    C --> E
    D --> E
    style A fill:#2980b9,color:#fff
