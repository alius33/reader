...
date: 2026-03-25
type: book-summary
tags: [book-summary, focus, productivity, deep-work, knowledge-work, concentration, attention, habits]
author: Cal Newport
...
```mermaid
flowchart TD
    A["Knowledge Economy"] --> B["Demands rare,<br>valuable output"]
    C["Modern Workplace"] --> D["Fills days with<br>shallow tasks"]
    B --> E["Deep Work<br>is valuable"]
    D --> F["Deep Work<br>is rare"]
    E --> G["Massive opportunity<br>for those who<br>cultivate depth"]
    F --> G
...
```mermaid
flowchart LR
    A["No clear metrics<br>for deep work value"] --> B["Default to<br>proxy metrics"]
    B --> C["Reward visible<br>busyness"]
    C --> D["Shallow work<br>dominates"]
    D --> E["Deep work<br>becomes rare"]
    E --> F["Deep workers gain<br>massive advantage"]
...
```mermaid
flowchart TD
    A["Deep Work"] --> B["Neurological:<br>Shapes what you<br>pay attention to"]
    A --> C["Psychological:<br>Produces flow states"]
    A --> D["Philosophical:<br>Provides craftsman's<br>sense of meaning"]
    B --> E["A richer experience<br>of daily life"]
    C --> E
    D --> E
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart LR
    A["Lead Measure:<br>Hours of deep work"] --> B["Produces"]
    B --> C["Lag Measure:<br>Papers published,<br>code shipped,<br>chapters written"]
    D["Scoreboard:<br>Tally deep work hours<br>visibly"] --> A
    E["Weekly Review:<br>Did I hit my target?"] --> D
...
```mermaid
flowchart TD
    A["Identify your core<br>professional and personal<br>goals"] --> B["For each goal, identify<br>the 2-3 most important<br>activities"]
    B --> C["For each tool,<br>ask: does it substantially<br>help these activities?"]
    C -->|Yes| D["Keep the tool"]
    C -->|No / Marginal| E["Eliminate the tool"]
    C -->|Unclear| F["30-day trial removal"]
    F -->|Missed it| D
    F -->|Didn't notice| E
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A["Track daily<br>deep work hours"] --> B["Weekly review:<br>Did I hit my target?"]
    B -->|Yes| C["Raise the target<br>slightly"]
    B -->|No| D["Diagnose: What<br>prevented depth?"]
    D --> E["Structural problem?<br>(too many meetings,<br>open office)"]
    D --> F["Behavioural problem?<br>(phone checking,<br>email reflex)"]
    E --> G["Negotiate changes<br>with boss/team"]
    F --> H["Apply Rule 2:<br>Embrace Boredom"]
    G --> A
    H --> A
    C --> A
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    R1["Rule 1: Work Deeply<br>(Build rituals and<br>environments for depth)"] --> DW["Sustained<br>Deep Work"]
    R2["Rule 2: Embrace Boredom<br>(Train your concentration<br>muscle)"] --> DW
    R3["Rule 3: Quit Social Media<br>(Eliminate the biggest<br>attention drains)"] --> DW
    R4["Rule 4: Drain the Shallows<br>(Contain shallow work<br>within strict bounds)"] --> DW
    DW --> O1["Master hard<br>things quickly"]
    DW --> O2["Produce at<br>an elite level"]
    O1 --> V["Thrive in the<br>knowledge economy"]
    O2 --> V
