...
date: 2026-03-25
type: book-summary
tags: [book-summary, mindset, preparation, astronaut, space, resilience, teamwork, competence, fear, attitude, leadership, self-mastery]
...
```mermaid
flowchart TD
    A["Decide what you want"] --> B["Ask: What would that<br>person do right now?"]
    B --> C["Prepare relentlessly<br>— just in case"]
    C --> D["Enjoy the process<br>as an end in itself"]
    D --> E["When opportunity comes,<br>you're ready"]
    E --> F["When it doesn't,<br>you still lived well"]
    D --> F
...
```mermaid
flowchart LR
    A["Attitude<br>(Orientation)"] --> B["Monitor<br>constantly"]
    B --> C["Correct<br>if necessary"]
    C --> D["Stay on<br>course"]
    D --> E["Arrive — or don't<br>but you're intact"]
    B --> F["Lose attitude"]
    F --> G["Tumble and spin"]
    G --> H["Stray from course"]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A["Unknown threat"] --> B["Brain detects:<br>novelty + helplessness"]
    B --> C["FEAR response:<br>panic, freeze, flee"]
    A --> D["Studied and<br>simulated threat"]
    D --> E["Brain detects:<br>familiar + prepared"]
    E --> F["FOCUS response:<br>execute plan"]
...
```mermaid
flowchart TD
    A["Threat identified"] --> B{"Have you<br>prepared?"}
    B -->|Yes| C["Execute<br>rehearsed response"]
    B -->|No| D["Panic,<br>freeze, or improvise<br>badly"]
    C --> E["Threat managed<br>systematically"]
    D --> F["Outcome depends<br>on luck"]
    style A fill:#2980b9,color:#fff
    style E fill:#27ae60,color:#fff
...
```mermaid
flowchart LR
    A["Enter new<br>situation"] --> B["Aim to be<br>a zero"]
    B --> C["Observe, learn,<br>help quietly"]
    C --> D["Understand the<br>dynamics"]
    D --> E["Gradually become<br>a plus one"]
    A --> F["Try to be a<br>plus one immediately"]
    F --> G["Come across as<br>arrogant or disruptive"]
    G --> H["Become a<br>minus one"]
...
```mermaid
flowchart TD
    A["ISS Commander"] --> B["Facilitation<br>not dictation"]
    A --> C["Cultural<br>competence"]
    A --> D["Small stuff<br>attention"]
    A --> E["Emergency<br>readiness"]
    B --> F["Make quiet people<br>talk, loud people listen"]
    C --> G["Respect Russian<br>and American norms"]
    D --> H["Clean filters,<br>fix toilets, check systems"]
    E --> I["Regular drills,<br>sim reviews"]
    style A fill:#2980b9,color:#fff
    style F fill:#27ae60,color:#fff
```
...
```mermaid
flowchart TD
    A["Launch:<br>Violent acceleration"] --> B["Orbit:<br>166 days in<br>zero gravity"]
    B --> C["Re-entry:<br>15 explosions<br>+ car crash"]
    C --> D["Landing:<br>Kazakh steppe impact"]
    D --> E["Recovery:<br>Weeks of<br>rehabilitation"]
    E --> F["New normal:<br>Re-learn walking,<br>standing, living"]
...
```mermaid
flowchart TD
    A["Nine-year-old boy<br>watches Moon landing"] --> B["Asks: What would<br>an astronaut do?"]
    B --> C["21 years of<br>preparation"]
    C --> D["166 days<br>in space"]
    D --> E["Climbing down<br>the ladder"]
    E --> F["The journey was<br>the point all along"]
    C --> F
    style A fill:#2980b9,color:#fff
    style D fill:#27ae60,color:#fff
    style F fill:#27ae60,color:#fff
