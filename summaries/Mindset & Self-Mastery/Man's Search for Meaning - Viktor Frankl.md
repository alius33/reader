...
date: 2026-03-25
type: book-summary
tags: [book-summary, meaning, psychology, holocaust, resilience, logotherapy, philosophy, existentialism, suffering]
...
```mermaid
flowchart TD
    A["Suffering is unavoidable"] --> B{"Can you change<br>the situation?"}
    B -->|Yes| C["Change it<br>(Creative values)"]
    B -->|No| D["Change your attitude<br>(Attitudinal values)"]
    C --> E["MEANING"]
    D --> E
    F["Love and experience<br>(Experiential values)"] --> E
...
```mermaid
flowchart LR
    A["External World<br>(Stripped away)"] --> B["Possessions<br>Status<br>Health<br>Family<br>Dignity"]
    C["Internal World<br>(Remains)"] --> D["Memory<br>Love<br>Humour<br>Beauty<br>Attitude"]
    D --> E["Meaning"]
    B --> F["Gone"]
    style A fill:#e74c3c,color:#fff
...
```mermaid
flowchart TD
    A["Phase 1: Shock"] -->|Days| B["Phase 2: Apathy"]
    B -->|Months to years| C["Phase 3: Liberation"]
    C --> D["Depersonalisation<br>'Is this real?'"]
    D --> E["Emotional thawing<br>Sudden tears"]
    E --> F["Bitterness<br>No one understands"]
    F --> G{"Path forward?"}
    G -->|With meaning| H["Recovery and purpose"]
    G -->|Without meaning| I["Ongoing emptiness<br>or cruelty"]
    style A fill:#e74c3c,color:#fff
...
```mermaid
flowchart TD
    A["Three Sources<br>of Meaning"] --> B["Creative Values<br>(What you give)"]
    A --> C["Experiential Values<br>(What you receive)"]
    A --> D["Attitudinal Values<br>(How you face suffering)"]
    B --> E["Work, deeds,<br>contributions"]
    C --> F["Love, beauty,<br>truth, nature"]
    D --> G["Dignity, courage,<br>grace under fire"]
...
```mermaid
flowchart LR
    subgraph Maslow ["Maslow's Model"]
        A1["Self-Actualisation"] --> A2["Focus inward:<br>become your best self"]
    end
    subgraph Frankl ["Frankl's Model"]
        B1["Self-Transcendence"] --> B2["Focus outward:<br>task, person, cause"]
        B2 --> B3["Self-actualisation<br>follows naturally"]
    end
...
```mermaid
flowchart TD
    A["Fear of symptom"] --> B["Anticipatory anxiety"]
    B --> C["Symptom occurs"]
    C --> D["Fear confirmed<br>and intensified"]
    D --> A
    E["Paradoxical Intention<br>(Wish for the symptom)"] --> F["Anxiety loop broken"]
    F --> G["Symptom diminishes"]
    style A fill:#e74c3c,color:#fff
    style E fill:#27ae60,color:#fff
...
```mermaid
flowchart LR
    A["Statue of Liberty<br>(East Coast)"] --- C["Meaningful Life"]
    B["Statue of Responsibility<br>(West Coast)"] --- C
    A -.->|"Freedom FROM<br>constraints"| D["Without responsibility:<br>Existential Vacuum"]
    B -.->|"Freedom TO<br>serve meaning"| E["Without liberty:<br>Oppression"]
    style C fill:#27ae60,color:#fff
