...
date: 2026-03-25
type: book-summary
tags: [book-summary, stoicism, philosophy, epictetus, ethics, freedom, ancient-rome, self-mastery]
author: Epictetus
title: "Discourses"
year: 108
---
...
```mermaid
flowchart LR
    subgraph up ["Up to Us (Prohairetic)"]
        A["Judgments"]
        B["Desires"]
        C["Aversions"]
        D["Impulses to act"]
    end
    subgraph not ["Not Up to Us (Aprohairetic)"]
        E["Body & health"]
        F["Property & wealth"]
        G["Reputation"]
        H["Other people"]
    end
    up --> F1["Focus here → Freedom"]
    not --> F2["Attachment here → Slavery"]
...
```mermaid
flowchart TD
    A["External Event"] --> B["Impression Forms"]
    B --> C{"Prohairesis Engages"}
    C -->|"Assent"| D["Accept impression as true"]
    C -->|"Reject"| E["Dismiss impression as false"]
    C -->|"Suspend"| F["Withhold judgment"]
    D --> G["Action follows from judgment"]
    E --> H["No emotional reaction"]
    F --> I["Further examination"]
...
```mermaid
flowchart TD
    A["Three Disciplines of Stoic Practice"] --> B["Discipline of Desire<br>(Orexis)"]
    A --> C["Discipline of Action<br>(Hormē)"]
    A --> D["Discipline of Assent<br>(Sunkatathesis)"]
    B --> B1["Want only what is<br>in your power"]
    B --> B2["Be averse only to<br>what is in your power"]
    C --> C1["Act with reserve clause:<br>fate permitting"]
    C --> C2["Fulfil your roles<br>and duties"]
    D --> D1["Test every impression<br>before accepting it"]
    D --> D2["Never assent to<br>false judgments"]
    style A fill:#2980b9,color:#fff
    style B fill:#27ae60,color:#fff
    style C fill:#e7b800,color:#000
...
```mermaid
stateDiagram-v2
    [*] --> RawImpression: External event occurs
    RawImpression --> Pause: Prosoche (attention) activates
    Pause --> IsItUpToMe: Ask — Is this within my control?
    IsItUpToMe --> NotUpToMe: No — external matter
    IsItUpToMe --> UpToMe: Yes — prohairetic matter
    NotUpToMe --> Release: "This is nothing to me"
    UpToMe --> TestJudgment: Is my judgment accurate?
    TestJudgment --> Assent: Judgment confirmed
    TestJudgment --> Reject: Judgment found false
    Assent --> VirtuousAction: Act according to virtue
    Reject --> Release: Dissolve the false impression
    Release --> Ataraxia: Freedom from disturbance
    VirtuousAction --> Ataraxia

    note right of Pause: This gap is where<br/>all freedom lives
    note right of Ataraxia: The goal of<br/>Stoic practice
```
...
```mermaid
flowchart LR
    A["Possessions"] --> B["Dependency"]
    B --> C["Fear of loss"]
    C --> D["Slavery"]
    E["No possessions"] --> F["No dependency"]
    F --> G["No fear"]
    G --> H["Freedom"]
    style D fill:#e74c3c,color:#fff
    style H fill:#27ae60,color:#fff
...
```mermaid
flowchart TD
    A["Self<br>(Prohairesis)"] --> B["Family<br>(Immediate duties)"]
    B --> C["Community<br>(Civic obligations)"]
    C --> D["Humanity<br>(Cosmopolitan duty)"]
    D --> E["The Cosmos<br>(Living according to Nature)"]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A["Dichotomy of Control"] --> B["Discipline of Desire"]
    A --> C["Discipline of Action"]
    A --> D["Discipline of Assent"]
    B --> E["Want only what is<br>up to you"]
    C --> F["Act with reserve clause<br>and role awareness"]
    D --> G["Test every impression<br>before accepting"]
    E --> H["Ataraxia<br>(Freedom from disturbance)"]
    F --> H
    G --> H
    H --> I["Virtue<br>(Living according to Nature)"]
    style A fill:#2980b9,color:#fff
    style H fill:#27ae60,color:#fff
