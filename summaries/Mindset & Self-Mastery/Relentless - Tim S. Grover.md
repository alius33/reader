...
date: 2026-03-25
type: book-summary
tags: [book-summary, performance, mindset, intensity, competition]
author: Tim S. Grover
title: "Relentless"
year: 2013
---
...
```mermaid
flowchart LR
    A[Cooler] -->|adds skill & confidence| B[Closer]
    B -->|shifts identity & internal standards| C[Cleaner]
...
```mermaid
flowchart TD
    A[Dark Side] -->|controlled| B[Directed Fuel]
    A -->|uncontrolled| C[Destruction]
    B --> D[The Zone]
    B --> E[Relentless Drive]
    C --> F[Public Exposure]
    C --> G[Loss of Focus]
    F --> H[Shame & Scrutiny]
    H --> G
...
```mermaid
flowchart TD
    A[Emotion Arises] --> B{Type?}
    B -->|Fear, Excitement, Frustration| C[Eliminate from Performance Space]
    B -->|Cold Anger| D[Channel as Fuel]
    C --> E[The Zone]
    D --> E
    E --> F[Instinctive Execution]
...
```mermaid
flowchart LR
    A[Achievement] --> B[Brief High]
    B --> C[Hedonic Adaptation]
    C --> D[Restlessness]
    D --> E[Next Target]
    E --> A
