...
date: 2026-03-25
type: book-summary
tags: [book-summary, purpose, values, motivation, resource-allocation, strategy, integrity, relationships, culture]
...
```mermaid
flowchart TD
    A[Business Theory] --> B[Career Happiness]
    A --> C[Enduring Relationships]
    A --> D[Personal Integrity]
    B --> E[Motivation Theory]
    B --> F[Deliberate vs Emergent Strategy]
    B --> G[Resource Allocation]
    C --> H[Jobs to Be Done]
    C --> I[Capabilities Framework]
    C --> J[Culture Formation]
    D --> K[100% Principle]
    D --> L[Purpose Framework]
...
```mermaid
flowchart LR
    A[Searching] -->|"Hygiene + motivators found"| B[Commit]
    A -->|"Still exploring"| C[Emergent Mode]
    C -->|"Test hypotheses"| A
    B --> D[Deliberate Mode]
    D -->|"Conditions change"| A
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart LR
    A[Dell: Full Capabilities] -->|"Outsource manufacturing"| B[Dell: Design + Brand]
    B -->|"Outsource supply chain"| C[Dell: Brand Only]
    C -->|"Asus launches own brand"| D[Dell: Hollow Shell]
    A -.->|"Capabilities transfer"| E[Asus: Components]
    E --> F[Asus: Manufacturing]
    F --> G[Asus: Design + Manufacturing]
    G --> H[Asus: Full Competitor]
...
```mermaid
flowchart TD
    A[Problem Arises] --> B[Solution Proposed]
    B --> C[Solution Tried]
    C -->|Works| D[Used Again]
    C -->|Fails| A
    D --> E[Repeated Many Times]
    E --> F[Becomes Instinctive]
    F --> G[Culture: 'How We Do Things']
...
```mermaid
flowchart TD
    A[First Compromise] -->|"Marginal cost: tiny"| B[Boundary Eroded]
    B -->|"Next situation: easier to cross"| C[Second Compromise]
    C -->|"Pattern established"| D[Serial Compromises]
    D -->|"Full cost arrives"| E[Identity Destroyed]
    A2[100% Commitment] -->|"No negotiation needed"| F[Boundary Holds]
    F -->|"Every future case: already decided"| G[Identity Preserved]
...
```mermaid
flowchart TD
    A[Purpose] --> B[Likeness]
    A --> C[Commitment]
    A --> D[Metrics]
    B --> E["Concrete image of who you want to become"]
    C --> F["Daily adherence, especially under pressure"]
    D --> G["Individual impact, not aggregate status"]
    style A fill:#2980b9,color:#fff
```
