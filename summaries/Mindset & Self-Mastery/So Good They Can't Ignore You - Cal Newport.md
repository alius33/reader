...
date: 2026-03-25
type: book-summary
tags: [book-summary, career-capital, deliberate-practice, craftsman-mindset, mission, autonomy]
author: Cal Newport
...
```mermaid
flowchart LR
    A[Rule 1: Reject Passion] --> B[Rule 2: Build Career Capital]
    B --> C[Rule 3: Invest in Control]
    C --> D[Rule 4: Discover Mission]
...
```mermaid
flowchart TD
    A[Build Skill] --> B[Competence Grows]
    B --> C[Autonomy Increases]
    C --> D[Relatedness Deepens]
    D --> E[Work Feels Like a Calling]
    B --> E
    C --> E
...
```mermaid
flowchart TD
    A[Identify Market Type] --> B{Winner-Take-All?}
    B -->|Yes| C[One skill dominates]
    C --> D[Focus obsessively on that skill]
    B -->|No: Auction| E[Many skills have value]
    E --> F[Build unique combination]
    F --> G[Walk through open gates]
    D --> H[Career Capital Grows]
    G --> H
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A[Identify Skill Gap] --> B[Design Stretch Activity]
    B --> C[Focused Practice Session]
    C --> D[Get Feedback]
    D --> E{Performance Improved?}
    E -->|Yes| F[Raise the Bar]
...
```mermaid
flowchart TD
    A[You Want More Control] --> B{Do you have career capital?}
    B -->|No| C[First Trap: Premature Autonomy]
    B -->|Yes| D{Are employers resisting?}
    D -->|No| E[Safe to proceed]
    D -->|Yes| F[Second Trap: Push forward]
    C --> G[Build more capital first]
...
```mermaid
flowchart LR
    A[Build Capital] --> B[Reach Cutting Edge]
    B --> C[See Adjacent Possible]
    C --> D[Identify Mission]
    D --> E[Make Little Bets]
    E --> F{Feedback}
    F -->|Works| G[Double Down]
    F -->|Fails| E
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A[Mission-Driven Project] --> B{Is it remarkable?}
    B -->|No| C[No traction regardless of venue]
    B -->|Yes| D{Is the venue remarkable?}
    D -->|No| E[Hidden masterpiece — no spread]
    D -->|Yes| F[Outsized career returns]
    style C fill:#e74c3c,color:#fff
...
```mermaid
flowchart TD
    A[Rule 1: Reject Passion Hypothesis] --> B[Clear the mental ground]
    B --> C[Rule 2: Adopt Craftsman Mindset]
    C --> D[Build Career Capital via Deliberate Practice]
    D --> E[Rule 3: Invest Capital in Control]
    E --> F[Navigate the Two Traps]
    F --> G[Rule 4: Discover Mission]
    G --> H[Use Little Bets + Remarkability]
    style A fill:#e74c3c,color:#fff
    style H fill:#27ae60,color:#fff
```
