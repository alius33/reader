...
date: 2026-03-25
type: book-summary
tags: [book-summary, learning, neuroscience, math, study-techniques, focus, chunking, memory, procrastination]
...
flowchart LR
    subgraph focused ["Focused Mode"]
        F1["Concentrated attention"]
        F2["Sequential reasoning"]
        F3["Familiar patterns"]
    end
    subgraph diffuse ["Diffuse Mode"]
        D1["Relaxed awareness"]
        D2["Big-picture connections"]
        D3["Novel insights"]
    end
    focused -- "take a break" --> diffuse
    diffuse -- "return to work" --> focused
    style focused fill:#3498db,color:#fff
    style diffuse fill:#27ae60,color:#fff
```
...
```mermaid
flowchart TD
    A["New skill or concept"] --> B["Step 1: Focus<br>(undivided attention)"]
    B --> C["Step 2: Understand<br>(grasp the connections)"]
    C --> D["Step 3: Practice<br>(repeated retrieval)"]
    D --> E{"Automatic?"}
    E -->|Not yet| D
    E -->|Yes| F["Chunk formed!<br>Freed working memory"]
    F --> G["Can be combined<br>with other chunks"]
    G --> H["Expertise:<br>library of<br>interconnected chunks"]
...
```mermaid
flowchart TD
    A["Cue<br>(see textbook)"] --> B["Pain response<br>(insular cortex)"]
    B --> C{"Routine choice"}
    C -->|Old routine| D["Avoid: phone,<br>snack, email"]
    D --> E["Reward:<br>temporary relief"]
    E --> F["Belief reinforced:<br>'I'm not disciplined'"]
    F -->|Next encounter| A
    C -->|New routine| G["Start Pomodoro:<br>25 min timer"]
    G --> H["Reward:<br>genuine break +<br>accomplishment"]
    H --> I["Belief updated:<br>'I can do this'"]
...
```mermaid
flowchart TD
    A["Encounter<br>difficult problem"] --> B["Anxiety<br>activates"]
    B --> C["Working memory<br>partially occupied<br>by anxiety"]
    C --> D["Reduced capacity<br>for the problem"]
    D --> E["Poor performance"]
    E --> F["'I'm bad at this'<br>belief strengthens"]
    F --> G["Next encounter<br>triggers MORE anxiety"]
    G --> B
...
```mermaid
flowchart TD
    A["New information<br>enters working memory"] --> B{"Transfer method?"}
    B -->|Rereading| C["Information stays<br>in working memory"]
    C --> D["Feels familiar<br>(illusion of competence)"]
    D --> E["Fades within hours<br>No long-term memory"]

    B -->|Retrieval practice| F["Brain actively searches<br>long-term memory"]
    F --> G{"Found it?"}
    G -->|Yes| H["Synapse strengthened<br>Memory more durable"]
    G -->|No| I["Brain flags gap<br>Re-learning is deeper"]
    I --> H
    H --> J["Lasting memory<br>Retrievable weeks later"]
...
```mermaid
flowchart LR
    subgraph EASY ["Feels Easy (Ineffective)"]
        R["Rereading"]
        H["Highlighting"]
        LS["Looking at solutions"]
        BP["Blocked practice"]
    end
    subgraph HARD ["Feels Hard (Effective)"]
        RP["Retrieval practice"]
        SP["Spaced repetition"]
        IL["Interleaving"]
        GN["Struggling first"]
    end
    EASY -->|"High confidence<br>Low retention"| BAD["Poor long-term<br>learning"]
    HARD -->|"Low confidence<br>High retention"| GOOD["Excellent long-term<br>learning"]
...
```mermaid
flowchart TD
    A["Encounter new material"] --> B["Focused Mode<br>(concentrated study)"]
    B --> C["Struggle with problems<br>(before seeing answers)"]
    C --> D["Take a break<br>(walk, nap, exercise)"]
    D --> E["Diffuse Mode<br>(unconscious processing)"]
    E --> F["Return to material"]
    F --> G["Retrieval Practice<br>(test yourself)"]
    G --> H{"Can you recall<br>the key ideas?"}
    H -- "No" --> B
    H -- "Yes" --> I["Chunk formed<br>Space the repetition"]
    I --> J["Interleave with<br>other topics"]
    J --> K["Sleep<br>(consolidation)"]
    K --> L["Review after delay<br>(spaced retrieval)"]
    L --> H
```
