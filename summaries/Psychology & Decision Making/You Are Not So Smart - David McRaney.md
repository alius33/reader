...
date: 2026-03-25
type: book-summary
tags: [book-summary, cognitive-bias, psychology, self-deception, rationality, heuristics, logical-fallacy, taxonomy]
...
```mermaid
mindmap
  root((You Are Not So Smart))
    SelfDeception["Self-Deception"]
      ConfirmationBias["Confirmation Bias"]
      Confabulation["Confabulation"]
      SelfServingBias["Self-Serving Bias"]
      IntrospectionIllusion["Introspection Illusion"]
    ...
    Shortcuts["Mental Shortcuts"]
      ...
      AffectHeuristic["Affect Heuristic"]
      Priming["Priming"]
      RepresentativenessHeuristic["Representativeness"]
    SocialErrors["Social Errors"]
      BystanderEffect["Bystander Effect"]
      Groupthink["Groupthink"]
      SpotlightEffect["Spotlight Effect"]
      ThirdPersonEffect["Third Person Effect"]
      Conformity["Conformity"]
    PersistenceErrors["Persistence Errors"]
      SunkCost["Sunk Cost Fallacy"]
      Procrastination["Procrastination"]
      LearnedHelplessness["Learned Helplessness"]
      NormalcyBias["Normalcy Bias"]
...
```mermaid
flowchart LR
    A["Low Competence"] --> B["Can't Evaluate<br>Own Performance"]
    B --> C["Overestimates<br>Ability"]
    D["High Competence"] --> E["Task Feels Easy"]
    E --> F["Assumes Everyone<br>Finds It Easy"]
    F --> G["Underestimates<br>Own Ability"]
...
```mermaid
flowchart TD
    A["Self-Deception Cluster"] --> B["Confirmation Bias"]
    A --> C["Confabulation"]
    A --> D["Hindsight Bias"]
    A --> E["Self-Serving Bias"]
    B -->|"Filters input"| F["Reinforced<br>World View"]
    C -->|"Generates fake<br>explanations"| F
    D -->|"Rewrites memory"| F
    E -->|"Assigns credit/blame"| F
    F --> G["Unshakeable but<br>Unfounded Confidence"]
...
```mermaid
flowchart TD
    A["Event Occurs"] --> B{"How vivid/emotional<br>is the event?"}
    B -->|Very vivid| C["Easy to recall"]
    B -->|Mundane| D["Hard to recall"]
    C --> E["Overestimate<br>probability"]
    D --> F["Underestimate<br>probability"]
    E --> G["Fear rare but<br>vivid threats"]
    F --> H["Ignore common but<br>boring threats"]
...
```mermaid
flowchart LR
    A["Expectation<br>Forms"] --> B["Behaviour<br>Changes"]
    B --> C["Environment<br>Responds"]
    C --> D["Outcome Matches<br>Expectation"]
    D --> E["Expectation<br>Reinforced"]
    E --> A
    style A fill:#2980b9,color:#fff
    style D fill:#27ae60,color:#fff
```
...
```mermaid
flowchart TD
    A["Free Dinner"] --> B["Weekend Retreat"]
    B --> C["Join Community"]
    C --> D["Cut Outside Ties"]
    D --> E["Full Commitment"]
    A -.->|"Sunk Cost<br>Grows"| E
    B -.->|"Identity<br>Shifts"| E
...
```mermaid
flowchart TD
    A["Cohesive Group"] --> B["Leader Signals Preference"]
    B --> C["Members Self-Censor"]
    C --> D["Silence = Agreement"]
    D --> E["Illusion of Unanimity"]
    E --> F["Warnings Dismissed"]
    F --> G["Catastrophic Decision"]
    style A fill:#2980b9,color:#fff
    style G fill:#e74c3c,color:#fff
```
...
```mermaid
flowchart LR
    A["Task Triggers<br>Negative Emotion"] --> B["Present Self<br>Avoids Task"]
    B --> C["Future Self<br>Bears Consequence"]
    C --> D["Deadline<br>Approaches"]
    D --> E["Fear of Failure<br>Overrides Avoidance"]
    E --> F["Panic-Driven<br>Action"]
    style A fill:#e74c3c,color:#fff
...
```mermaid
flowchart TD
    A["Initial Investment"] --> B["Returns Disappointing"]
    B --> C{"Continue or<br>Stop?"}
    C -->|"Rational"| D["Evaluate FUTURE<br>value only"]
    C -->|"Emotional"| E["Can't waste PAST<br>investment"]
    D --> F["Cut losses<br>early"]
    E --> G["Invest more<br>to justify past"]
    G --> H["Losses compound"]
    H --> C
    style F fill:#27ae60,color:#fff
...
```mermaid
flowchart TD
    A["Evolutionary<br>Environment"] --> B["Simple threats<br>Small groups<br>Immediate consequences"]
    B --> C["Brain develops<br>heuristics & shortcuts"]
    C --> D["Fast decisions<br>in simple environments"]
    D --> E["High survival value"]

    F["Modern<br>Environment"] --> G["Complex threats<br>Massive groups<br>Delayed consequences"]
    G --> H["Same heuristics<br>applied to new problems"]
    H --> I["Systematic errors<br>in complex environments"]
    I --> J["Cognitive biases"]
...
```mermaid
flowchart LR
    A["Identify<br>the Bias"] --> B["Recognise<br>the Trigger"]
    B --> C["Apply<br>Structural Safeguard"]
    C --> D["Seek<br>Disconfirmation"]
    D --> E["Accept<br>Uncertainty"]
    style A fill:#2980b9,color:#fff
    style E fill:#27ae60,color:#fff
```
