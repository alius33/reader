...
date: 2026-03-25
type: book-summary
tags: [book-summary, psychology, neuroscience, expectations, placebo, nocebo, mindset, prediction, stress, ageing, willpower, self-fulfilling-prophecy]
author: David Robson
...
```mermaid
flowchart TD
    A["Brain generates<br>PREDICTION"] --> B{"Does reality<br>match?"}
    B -->|Yes| C["Prediction confirmed<br>No adjustment needed"]
    B -->|No| D{"Which adjusts?"}
    D -->|Often| E["Brain adjusts<br>PERCEPTION & PHYSIOLOGY<br>to match prediction"]
    D -->|Sometimes| F["Brain updates<br>the PREDICTION"]
    E --> G["Expectation becomes<br>self-fulfilling"]
    F --> H["Learning occurs"]
...
```mermaid
flowchart TD
    A["Negative age belief:<br>'Decline is inevitable'"] --> B["Withdraw from<br>challenging activities"]
    B --> C["Less cognitive &<br>physical stimulation"]
    C --> D["Actual measurable<br>decline occurs"]
    D --> E["'See? I was right.<br>I AM declining.'"]
    E --> A
...
```mermaid
flowchart LR
    A["Index case:<br>One person develops<br>symptoms"] --> B["Social observation:<br>Others see/hear<br>about symptoms"]
    B --> C["Prediction primed:<br>'This could happen<br>to me'"]
    C --> D["Under stress:<br>Brain generates<br>similar symptoms"]
    D --> E["Media coverage:<br>Amplifies awareness"]
    E --> B
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TB
    subgraph limited ["'Willpower Is Limited' Belief"]
        L1["Exert self-control"] --> L2["Expect depletion"]
        L2 --> L3["Brain reduces motivation<br>for next task"]
        L3 --> L4["Performance drops"]
        L4 --> L5["'See? I was depleted'"]
    end
    subgraph unlimited ["'Willpower Is Self-Generating' Belief"]
        U1["Exert self-control"] --> U2["Expect energisation"]
        U2 --> U3["Brain maintains or<br>increases motivation"]
        U3 --> U4["Performance sustained"]
        U4 --> U5["'I'm on a roll'"]
    end
    style L4 fill:#e74c3c,color:#fff
    style U4 fill:#27ae60,color:#fff
```
...
```mermaid
flowchart LR
    subgraph pygmalion ["Pygmalion Effect (Social Placebo)"]
        P1["Authority expects<br>high performance"] --> P2["Provides warmth,<br>challenge, feedback"]
        P2 --> P3["Student absorbs<br>positive signals"]
        P3 --> P4["Performance rises"]
    end
    subgraph golem ["Golem Effect (Social Nocebo)"]
        G1["Authority expects<br>low performance"] --> G2["Provides less warmth,<br>easier material"]
        G2 --> G3["Student absorbs<br>negative signals"]
        G3 --> G4["Performance drops"]
    end
...
```mermaid
flowchart TD
    A["Stressor detected:<br>Heart rate up,<br>palms sweat"] --> B{"How do you<br>interpret this?"}
    B -->|"I'm anxious.<br>This is bad."| C["THREAT RESPONSE"]
    B -->|"I'm energised.<br>This is useful."| D["CHALLENGE RESPONSE"]
    C --> E["Vasoconstriction<br>High cortisol<br>Narrowed attention"]
    D --> F["Vasodilation<br>High DHEA<br>Enhanced performance"]
    E --> G["Long-term:<br>Health damage"]
    F --> H["Long-term:<br>Resilience & growth"]
...
```mermaid
flowchart TD
    A["Personal beliefs"] --> D["Your Prediction<br>Machine"]
    B["Social transmission<br>(doctors, teachers,<br>peers, media)"] --> D
    C["Cultural narratives<br>(ageing, stress,<br>intelligence)"] --> D
    D --> E["Generates<br>expectations"]
    E --> F["Shapes physiology,<br>perception, behaviour"]
    F --> G["Creates outcomes<br>that confirm<br>expectations"]
    G --> D
