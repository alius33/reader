...
date: 2026-03-25
type: book-summary
tags: [book-summary, finance, psychology, money, investing, behavioural-economics, wealth, compounding, behaviour]
...
```mermaid
flowchart TD
    A["Your unique life experience"] --> B["Your mental model of money"]
    B --> C["Your financial decisions"]
    C --> D{"Outcome"}
    D -->|Good| E["Often attributed to SKILL"]
    D -->|Bad| F["Often attributed to BAD LUCK"]
    G["But LUCK and RISK<br>are siblings"] --> D
...
```mermaid
flowchart LR
    A["Same skill,<br>same effort"] --> B{"Random events"}
    B -->|"Favourable"| C["Success<br>(attributed to skill)"]
    B -->|"Unfavourable"| D["Failure<br>(attributed to bad luck)"]
    C --> E["We study and<br>imitate them"]
    D --> F["We dismiss and<br>ignore them"]
...
```mermaid
flowchart TD
    A["100 investments"] --> B["~65 lose money"]
    A --> C["~32 earn modest returns"]
    A --> D["~3 earn extraordinary returns"]
    D --> E["These 3 drive nearly<br>ALL total returns"]
    B --> F["These 65 are irrelevant<br>if the tails hit"]
...
```mermaid
flowchart LR
    A["Income earned"] --> B{"Visible or invisible?"}
    B -->|"Spent on visible things"| C["Rich<br>(current lifestyle)"]
    B -->|"Saved/invested invisibly"| D["Wealthy<br>(future options)"]
    C --> E["Others see and admire"]
    D --> F["Others see nothing"]
    E --> G["But admiration fades<br>and money is gone"]
    F --> H["But options grow<br>and compound"]
...
```mermaid
flowchart TD
    A["Financial Plan"] --> B{"Has room for error?"}
    B -->|"Yes"| C["Survives market crash"]
    B -->|"Yes"| D["Survives job loss"]
    B -->|"Yes"| E["Survives health crisis"]
    C --> F["Compounding continues<br>uninterrupted"]
    D --> F
    E --> F
    B -->|"No"| G["Optimised for<br>one scenario"]
    G --> H["Any surprise = ruin"]
    H --> I["Compounding stops<br>permanently"]
...
```mermaid
flowchart LR
    A["Every financial<br>decision"] --> B{"Does this increase<br>my independence?"}
    B -->|"Yes"| C["Do it"]
    B -->|"No"| D["Don't do it"]
    B -->|"Unclear"| E["Default to saving"]
    C --> F["Freedom grows"]
    D --> G["Freedom preserved"]
    E --> F
...
```mermaid
flowchart TD
    A["Post-WWII boom<br>1945-1975"] --> B["Broadly shared<br>prosperity"]
    B --> C["Expectation:<br>Everyone gets richer<br>together"]
    C --> D["Late 1970s:<br>Pattern breaks"]
    D --> E["Growth continues but<br>gains flow to the top"]
    E --> F["Gap between<br>expectations and reality"]
    F --> G["Response: borrow to<br>maintain lifestyle"]
    G --> H["Debt, anxiety, and<br>financial fragility"]
