...
date: 2026-03-28
type: book-summary
tags: [book-summary, psychology, decision-making, cognitive-bias, heuristics, behavioural-economics, two-systems, prospect-theory, Nobel-Prize]
...
```mermaid
flowchart TD
    A["Stimulus / Question"] --> B["System 1<br>Fast, automatic, effortless"]
    B --> C{"System 2<br>Engaged?"}
    C -->|"Usually no<br>(lazy endorsement)"| D["Accept System 1's<br>intuition as truth"]
    C -->|"Sometimes yes<br>(effortful check)"| E["Deliberate analysis<br>Logic, statistics, doubt"]
    E --> F["Override or<br>confirm intuition"]
    D --> G["Belief / Decision / Action"]
    F --> G
    B --> H["WYSIATI:<br>Builds story from<br>available evidence only"]
    H --> D
...
```mermaid
flowchart LR
    A["Start with<br>BASE RATE<br>(average outcome)"] --> B["Your impression<br>of THIS case"]
    B --> C{"How well does<br>your evidence<br>predict?"}
    C -->|"Strongly<br>(rare)"| D["Predict close<br>to your impression"]
    C -->|"Weakly<br>(common)"| E["Stay close<br>to the base rate"]
    D --> F["Calibrated<br>Prediction"]
    E --> F
...
```mermaid
quadrantChart
    title When Can You Trust Expert Intuition?
    x-axis Low Regularity --> High Regularity
    y-axis Low Feedback --> High Feedback
    quadrant-1 TRUST intuition
    quadrant-2 FALSE confidence
    quadrant-3 Guessing
    quadrant-4 Slow learning
    Stock Pickers: [0.2, 0.35]
    Political Pundits: [0.15, 0.25]
    Chess Masters: [0.85, 0.9]
    Firefighters: [0.75, 0.85]
    ER Doctors: [0.7, 0.75]
    Therapists: [0.3, 0.3]
```
...
```mermaid
graph TD
    subgraph "Prospect Theory Value Function"
    A["Reference Point<br>(current state)"] --> B["GAINS<br>Concave curve<br>Diminishing returns"]
    A --> C["LOSSES<br>Convex curve<br>Steeper slope (~2x)"]
    B --> D["Moderate pleasure<br>from $100 gain"]
    C --> E["Intense pain<br>from $100 loss"]
    end
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A["System 1<br>Fast, automatic thinking"] --> B["Heuristics<br>Availability, representativeness,<br>anchoring, affect"]
    B --> C["Biases<br>Base rate neglect,<br>conjunction fallacy,<br>regression blindness"]
    C --> D["Overconfidence<br>Illusion of validity,<br>planning fallacy,<br>narrative fallacy"]
    D --> E["Prospect Theory<br>Loss aversion, framing,<br>narrow framing,<br>mental accounting"]
    E --> F["Two Selves<br>Experiencing vs remembering,<br>peak-end rule,<br>duration neglect"]
    A --> G["System 2<br>Slow, lazy, depleted"]
    G -->|"Usually endorses<br>without checking"| B
    G -->|"Occasionally overrides<br>when engaged"| H["Better decisions<br>(but effortful)"]
    style A fill:#2980b9,color:#fff
...
flowchart LR
    ...
    subgraph "Part II"
    B["Heuristics<br>& Biases"]
    end
    subgraph "Part III"
    C["Over-<br>confidence"]
    end
    subgraph "Part IV"
    D["Choices"]
    end
    ...
    A -->|"System 1's shortcuts<br>produce..."| B
    B -->|"Biases aggregate<br>into..."| C
    C -->|"Overconfident minds<br>make flawed..."| D
    D -->|"Choices are evaluated<br>by..."| E
    E -->|"Memory biases<br>reinforce..."| A
    style A fill:#2980b9,color:#fff
