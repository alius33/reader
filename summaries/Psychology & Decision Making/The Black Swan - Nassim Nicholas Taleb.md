...
date: 2026-03-28
type: book-summary
tags: [book-summary, uncertainty, probability, black-swan, risk, extremistan, cognitive-bias, prediction, philosophy, nassim-taleb]
...
```mermaid
flowchart TD
    BS["BLACK SWAN<br>Rare + Extreme Impact +<br>Retrospective Predictability"]
    BS --> B1["We can't predict it<br>(it's an outlier)"]
    BS --> B2["It changes everything<br>(extreme impact)"]
    BS --> B3["We explain it afterward<br>(narrative fallacy)"]
    B1 --> BLIND["SYSTEMATIC<br>BLINDNESS"]
    B2 --> BLIND
    B3 --> BLIND
    BLIND --> C1["Confirmation Bias"]
    BLIND --> C2["Narrative Fallacy"]
    BLIND --> C3["Silent Evidence"]
    BLIND --> C4["Ludic Fallacy"]
    BLIND --> C5["Tunneling"]
    style BS fill:#e74c3c,color:#fff
    style BLIND fill:#2c3e50,color:#fff
```
...
```mermaid
mindmap
  root((Five Sources of Black Swan Blindness))
    Confirmation Bias
      Seek confirming evidence
      Ignore disconfirming data
      White swans prove nothing
    Narrative Fallacy
      Stories compress randomness
      Memory rewrites history
      Hindsight creates certainty
    Silent Evidence
      Dead can't write memoirs
      Survivors distort the picture
      The cemetery is invisible
    Ludic Fallacy
      Casino rules ≠ reality
      Models assume known unknowns
      Real uncertainty is unmapped
    Tunneling
      Focus on known risks
      Miss unknown unknowns
...
```mermaid
flowchart LR
    subgraph Forward["FORWARD PROCESS<br>(Relatively Easy)"]
        ICE["🧊 Ice Cube<br>(Known Starting Point)"] --> PUDDLE["💧 Puddle<br>(Predictable Shape)"]
    end
    subgraph Backward["BACKWARD PROCESS<br>(Effectively Impossible)"]
        PUDDLE2["💧 Puddle<br>(Observed Outcome)"] --> Q1["🧊 Ice Cube A?"]
        PUDDLE2 --> Q2["🧊 Ice Cube B?"]
        PUDDLE2 --> Q3["🥤 Spilled Drink?"]
        PUDDLE2 --> Q4["🌧️ Rain?"]
        PUDDLE2 --> Q5["∞ Infinite<br>Possibilities"]
    end
    style Forward fill:#27ae60,color:#fff
    style Backward fill:#e74c3c,color:#fff
```
...
flowchart TD
    subgraph BARBELL["THE BARBELL STRATEGY"]
        direction LR
        SAFE["85-90%<br>MAXIMALLY SAFE<br>Treasury Bills<br>Cash Under Mattress<br>Zero Black Swan Risk"]
        NOTHING["NOTHING<br>IN THE<br>MIDDLE<br>❌ 'Medium Risk'<br>is a fraud"]
        SPEC["10-15%<br>MAXIMALLY SPECULATIVE<br>Venture Capital<br>Options / Long Shots<br>Unlimited Upside"]
    end
    SAFE --- NOTHING --- SPEC
    style SAFE fill:#27ae60,color:#fff
    style NOTHING fill:#e74c3c,color:#fff
...
flowchart TD
    RAND["ALL RANDOMNESS"]
    RAND --> MED["MEDIOCRISTAN<br>Gaussian / Bell Curve<br>Mild Randomness"]
    RAND --> EXT["EXTREMISTAN<br>Wild Randomness"]
    EXT --> GRAY["GRAY SWANS<br>Mandelbrotian Fractals<br>Rare but Modelable"]
    EXT --> BLACK["BLACK SWANS<br>Unknown Unknowns<br>Not Modelable"]
    style MED fill:#27ae60,color:#fff
    style GRAY fill:#f39c12,color:#fff
    style BLACK fill:#e74c3c,color:#fff
    style RAND fill:#2c3e50,color:#fff
```
...
```mermaid
flowchart LR
    subgraph GAUSSIAN["GAUSSIAN WORLD<br>(The Fraud)"]
        G1["Extremes are<br>'impossible'"]
        G2["Risk is<br>computable"]
        G3["Models give<br>precise answers"]
    end
    subgraph FRACTAL["FRACTAL WORLD<br>(Gray Swans)"]
        F1["Extremes are<br>rare but real"]
        F2["Risk is roughly<br>estimable"]
        F3["Models give<br>approximate answers"]
    end
    subgraph REAL["REAL WORLD<br>(Black Swans)"]
        R1["Extremes are<br>unknown unknowns"]
        R2["Risk is<br>not computable"]
        R3["No model<br>captures it"]
    end
    GAUSSIAN -->|"Better than"| FRACTAL
    FRACTAL -->|"Better than"| REAL
    style GAUSSIAN fill:#e74c3c,color:#fff
    style FRACTAL fill:#f39c12,color:#fff
    style REAL fill:#2c3e50,color:#fff
```
...
```mermaid
flowchart TD
    subgraph P1["PART ONE: How We Seek Validation<br>(Cognitive Foundations)"]
        C1["Ch 1: Autobiography<br>Triplet of Opacity"]
        C2["Ch 2: Yevgenia's<br>Black Swan"]
        C3["Ch 3: Mediocristan<br>vs. Extremistan"]
        C4["Ch 4: Turkey Problem<br>Problem of Induction"]
        C5["Ch 5: Confirmation<br>Bias & Falsification"]
        C6["Ch 6: Narrative<br>Fallacy"]
        C7["Ch 7: Antechamber<br>of Hope"]
        C8["Ch 8: Silent<br>Evidence"]
        C9["Ch 9: Ludic<br>Fallacy"]
    end
    subgraph P2["PART TWO: We Just Can't Predict<br>(Empirical & Philosophical Case)"]
        C10["Ch 10: Scandal of<br>Prediction"]
        C11["Ch 11: Serendipity &<br>Poincare's Limit"]
        C12["Ch 12: Epistemocracy<br>& Future Blindness"]
        C13["Ch 13: Barbell<br>Strategy"]
    end
    subgraph P3["PART THREE: Gray Swans of Extremistan<br>(Mathematical Case)"]
        C14["Ch 14: Preferential<br>Attachment"]
        C15["Ch 15: Bell Curve<br>Fraud"]
        C16["Ch 16: Mandelbrot &<br>Fractals"]
        C17["Ch 17: Nobel Prize<br>Scandal / LTCM"]
        C18["Ch 18: Fourth<br>Quadrant"]
    end
    subgraph P4["PART FOUR + POSTSCRIPT<br>(What To Do)"]
        C19["Ch 19: Half<br>and Half"]
        PS["Postscript:<br>10 Principles<br>Amor Fati<br>Seeds of Antifragile"]
    end
    P1 --> P2 --> P3 --> P4
