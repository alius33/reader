...
date: 2026-03-25
type: book-summary
tags: [book-summary, career-strategy, sponsorship, perception, networking, risk-taking, voice]
...
```mermaid
flowchart LR
    A[Day 1-30: Master Basics] --> B[Day 31-60: Learn Unspoken Rules]
    B --> C[Day 61-90: Build Key Relationships]
    C --> D[Day 91+: Articulate Accomplishments]
    D --> E[Proactive Feedback Loop]
    E --> F[Adjust & Repeat]
```
...
```mermaid
flowchart TD
    A[Career Relationship] --> B{Level of Investment}
    B -->|Low: one-off questions| C[Adviser]
    B -->|Medium: ongoing guidance| D[Mentor]
    B -->|High: spends political capital| E[Sponsor]
    C --> F[Isolated data points]
    D --> G[Helps you think clearly]
    E --> H[Makes things happen in the room]
...
```mermaid
flowchart TD
    A[Your Network] --> B[Upward: Senior Leaders]
    A --> C[Lateral: Peers & Assistants]
    A --> D[Downward: Junior Colleagues]
    A --> E[External: Outside Organisation]
    B --> F[Access to decision-makers & sponsorship]
    C --> G[Skills transfer & early warning systems]
    D --> H[Fresh perspectives & future pipeline]
...
```mermaid
flowchart TD
    A{Your Working Style} --> B[Workhorse]
    A --> C[Political Maniac]
    B --> D{Boss's Style?}
    C --> E{Boss's Style?}
    D -->|Workhorse Boss| F[Aligned: Valued AND Rewarded]
    D -->|Political Maniac Boss| G[Misaligned: Valued but NOT Rewarded]
    E -->|Political Maniac Boss| H[Aligned: Valued AND Rewarded]
    E -->|Workhorse Boss| I[Misaligned: Valued but NOT Rewarded]
