...
date: 2026-03-25
type: book-summary
tags: [book-summary, personal-brand, reputation, politics, leadership, organisational-behaviour]
author: David D'Alessandro
title: "Career Warfare: 10 Rules for Building a Successful Personal Brand"
year: 2004
---
...
> - One act of indiscretion destroyed the entire partnership
...
> - Cerner's stock dropped 22% in three days
...
> - He would never have discovered it without asking
...
> - The boss discovered the betrayal and drop-kicked Doug into obscurity
...
The first four types each offer something worth extracting. The last two are dead ends.
...
```mermaid
flowchart TD
    A[Decision Pending] --> B{Your Input}
    B --> C[Before Decision: Counsel]
    B --> D[After Decision: Execution]
    C --> E[Present evidence and alternatives]
    E --> F[Accept outcome either way]
    D --> G[Execute faithfully]
    D --> H{Illegal or Unethical?}
    H -->|Yes| I[Resist regardless]
    H -->|No| G
...
> - The new boss repeated this to the old boss
> - The executive spent two years in professional purgatory as punishment
...
> - When O'Neal replaced David Komansky, he systematically removed anyone perceived as a Komansky loyalist
...
```mermaid
flowchart LR
    A[Your Brand] --> B[Boss Loyalty]
    A --> C[Institutional Value]
    A --> D[Cross-Functional Allies]
    B --> E[Credit Upward]
    C --> F[Independent Reputation]
    D --> G[Multiple Power Centres]
    E --> H[Survive Regime Change]
    F --> H
    G --> H
    style A fill:#2980b9,color:#fff
...
- Let incompetent rivals self-destruct
...
- Why social events are so dangerous:
  ...
  - Informality encourages oversharing
  - Relaxation creates the illusion that the usual rules do not apply
  - And then, the next morning, everyone remembers everything
...
> - The bankers thought they were celebrating
...
> - They thought they were enjoying a private moment
> - There is no private moment at a corporate event
...
- When you are promoted, people start treating you differently:
  - They laugh at your jokes more readily
  - They agree with your ideas more quickly
  - They stop telling you when you are wrong
...
> - Jack Welch, the legendary CEO of General Electric, upon retirement discovered that his compensation package included details that looked excessive when made public — private jet access, a luxury apartment, courtside tickets
> - Welch had not thought twice about these perks while in power
> - Inside the bubble, they seemed normal; outside it, they looked outrageous
...
> - The campaign cost him personally: death threats, social ostracism, bitter criticism
...
```mermaid
flowchart TD
    A[Someone Attacks You] --> B{Type of Attack?}
    B --> C[Open Disagreement]
    B --> D[Covert Undermining]
    C --> E[Respect It — Respond with Evidence]
    D --> F[Identify the Source]
    F --> G[Respond with Facts]
    G --> H{Will Fighting Confirm a Negative Brand?}
    H -->|Yes| I[Outrun Them Instead]
    H -->|No| J[Fight Decisively]
...
> - He refused to answer questions, hid behind legal counsel, and eventually gave a television interview so evasive that it destroyed whatever remained of his credibility
...
> - His brand survived; his career continued
...
```mermaid
flowchart LR
    A[Daily Competence] --> B[Goodwill Reservoir]
    C[Generosity] --> B
    D[Genuine Engagement] --> B
    B --> E{Crisis Hits}
    E -->|Full Reservoir| F[Scandal Becomes Subhead]
    E -->|Empty Reservoir| G[Scandal Becomes Career End]
...
```mermaid
flowchart TD
    A[Personal Brand] --> B[Five Cornerstones]
    A --> C[Boss as Coauthor]
    A --> D[Daily Transactions]
    B --> E[Revenue + Truth + Discretion + Promises + Talent]
    C --> F[Loyalty as Strategy]
    C --> G[Bridge, Not Retainer]
    D --> H[Halo Effect]
    D --> I[Pattern Awareness]
    F --> J[Goodwill Reservoir]
    G --> J
    H --> J
    I --> J
    J --> K[Survive Crises]
    J --> L[Sustain Advancement]
    style A fill:#2980b9,color:#fff
    style J fill:#27ae60,color:#fff
```
