...
date: 2026-03-25
type: book-summary
tags: [book-summary, skill-mastery, soft-skills, professional-development]
author: Maya Grossman
title: "Invaluable: Master the 10 Skills You Need to Skyrocket Your Career"
year: 2023
---
...
```mermaid
flowchart LR
    A[Owner Mentality] --> B[Lifelong Learning]
    B --> C[Impact Focus]
    C --> D[Opportunity-Digging]
    D --> E[Fixer Mentality]
    E --> F[Influence]
    F --> G[Relationships]
    G --> H[Accountability]
    H --> I[Career Planning]
    I --> J[Success Tracking]
    J --> A
...
```mermaid
flowchart LR
    A[Small Meaning] --> B[Small Action]
    B --> C[Small Win]
    C --> D[Motivation]
    D --> E[Bigger Action]
    E --> F[Bigger Win]
...
```mermaid
flowchart TD
    A[Define Your Purpose] --> B[Identify Their Barrier]
    B --> C[Identify Their Benefit]
    C --> D[Flip the Script]
    D --> E[Call for Specific Action]
    style D fill:#27ae60,color:#fff
    style A fill:#2980b9,color:#fff
```
...
```mermaid
flowchart TD
    A[Define Destination] --> B[Outline the Path]
    B --> C[Check Fuel Levels]
    C --> D[Decompose into Weekly Actions]
    D --> E[Execute & Review]
    E -->|Annual revisit| A
    style A fill:#2980b9,color:#fff
    style D fill:#27ae60,color:#fff
...
```mermaid
flowchart TD
    A[Owner Mentality] --> B[Notice Problems]
    B --> C[Fixer Mentality]
    C --> D[Need Influence to Implement]
    D --> E[Influence Formula]
    E --> F[Need Relationship Context]
    F --> G[Managing Up & Across]
    G --> H[Need to Know What Matters]
    H --> I[Impact Focus]
    I --> J[Need to Eliminate Low-Value Work]
    J --> K[First-Principle Thinking]
    K --> L[Need Knowledge of Alternatives]
    L --> M[Lifelong Learning]
    M --> A
    style A fill:#27ae60,color:#fff
