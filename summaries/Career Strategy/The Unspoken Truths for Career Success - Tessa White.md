...
date: 2026-03-25
type: book-summary
tags: [book-summary, career-reality, corporate-navigation, promotability, leverage, negotiation, politics, feedback]
author: Tessa White
title: "The Unspoken Truths for Career Success"
year: 2022
---
...
```mermaid
flowchart TD
    A[Your Ask] --> B{Can you translate it into company value?}
    B -->|Yes| C[Frame as ROI / risk reduction]
    B -->|No| D[Rework or abandon the ask]
    C --> E{Does it align with manager's incentives?}
    E -->|Yes| F[Manager advocates]
    E -->|No| G[Manager stalls or blocks]
    F --> H{Does HR see legal/financial risk?}
    H -->|No risk| I[Ask approved]
    H -->|Risk detected| J[Ask modified or denied]
```
...
```mermaid
flowchart LR
    A[Identify the Gap] --> B[State Consequences]
    B --> C[Hand Off with Good Intent]
    C --> D[Listen to Response]
    D --> E[Problem-Solve Together]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart LR
    A[Stage 1: Doer] --> B[Stage 2: Achiever]
    B --> C[Stage 3: Collaborator]
    C -->|Most careers stall here| D[Stage 4: Builder]
    D --> E[Stage 5: Expander]
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A[Map Company Fingerprint] --> B[Map Your Operating Style]
    B --> C{Aligned?}
    C -->|Yes| D[Leverage natural fit]
    C -->|No| E{Can you adapt?}
    E -->|Yes| F[Adapt your style]
    E -->|No| G{Pocket available?}
    G -->|Yes| H[Transfer internally]
    G -->|No| I[Consider leaving]
    style A fill:#2980b9,color:#fff
    style D fill:#27ae60,color:#fff
...
```mermaid
flowchart LR
    A[Observe the Culture] --> B[Read Decision Patterns]
    B --> C[Align Your Behaviour]
    C --> D[Package Work Correctly]
    D --> E[Advance]
    F[Resent the Culture] --> G[Resist or Withdraw]
    G --> H[Stagnate]
    style E fill:#27ae60,color:#fff
    style H fill:#e74c3c,color:#fff
```
