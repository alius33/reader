...
```mermaid
flowchart LR
    A[Right Stuff<br>Skills + Knowledge + Behaviour] -->|Numerator| C[Career Trajectory]
    B[Wrong Stuff<br>Derailers] -->|Denominator| C
    C --> D{High Trajectory?}
    D -->|Small denominator| E[Advancement]
    D -->|Large denominator| F[Derailment]
...
```mermaid
flowchart TD
    A[Initiative & Learning] --> D[Compounding Returns]
    B[Relationships & EQ] --> D
    C[Drive & Perseverance] --> D
    A -->|Opens new opportunities| B
    B -->|Provides support & influence| C
    C -->|Produces results justifying further opportunity| A
...
```mermaid
flowchart LR
    A[Managing Self] -->|Solo Flier risk| B[Managing Others]
    B -->|Strategic thinking needed| C[Managing Functions]
    C -->|One-Trick Pony risk| D[Managing Business Units]
    D -->|Power motive critical| E[Managing Enterprises]
    style A fill:#2980b9,color:#fff
    style E fill:#27ae60,color:#fff
```
...
```mermaid
flowchart LR
    A[Moderate Trait<br>Asset] -->|Stress amplifies| B[Overdriven Trait<br>Liability]
    B -->|Self-awareness| C[Early Warning<br>System]
    C -->|Anticipate & plan| A
...
```mermaid
flowchart TD
    A[Identify Top 3 Strengths] --> B[Core Quadrant Analysis<br>Strength → Shadow → Balance → Allergy]
    A --> C[Informal 360<br>What should I change?]
    A --> D[Motives Ranking<br>Self vs. Others]
    B --> E[Derailer Diagnosis]
    C --> E
    D --> E
