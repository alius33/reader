...
date: 2026-03-25
type: book-summary
tags: [book-summary, workplace-navigation, soft-skills, managing-up, office-politics, negotiation, self-promotion]
author: "Dennis Mark & Michael Dam"
title: "Thriving at Work: What School Doesn't Teach You"
year: 2022
---
...
```mermaid
flowchart TD
    A[Credibility] --> D[Disproportionate Influence]
    B[Visibility] --> D
    C[Expertise] --> D
    A -- "Makes visibility<br>believable" --> B
    B -- "Puts expertise<br>in front of deciders" --> C
    C -- "Builds credibility<br>through results" --> A
...
```mermaid
flowchart LR
    A[Understand True Issues] --> B[Assess Trade-offs]
    B --> C[Set Walkaway Value]
    C --> D[Estimate Their Walkaway]
    D --> E[Negotiate with Intelligence]
...
```mermaid
flowchart LR
    A[Your Work] --> B[Evidence Package]
    B --> C[Your Manager]
    C --> D[Calibration Meeting]
    D --> E[Your Rating]
    F[Other Managers] --> D
    G[Their Evidence] --> F
    style A fill:#2980b9,color:#fff
...
```mermaid
flowchart TD
    A[Help Others Succeed] --> B[Goodwill Deposits]
    B --> C[Relationship Accounts]
    C --> D{Need Arises}
    D -->|"Favour needed"| E[Withdraw Goodwill]
    D -->|"No need yet"| F[Continue Depositing]
    F --> B
    E --> G[Support Received]
...
```mermaid
flowchart TB
    A[Formal Decision-Maker] --> D[Decision]
    B[Informal Influencers] --> A
    C[Access Controllers] --> A
    C --> B
